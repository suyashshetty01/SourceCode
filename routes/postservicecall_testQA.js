...
//let sales_marketing_material = require('../models/sales_marketing_material'); Need to uncomment
//let sales_material_tracking = require('../models/sales_material_tracking'); Need to uncomment
var emailtocc = {
    "Product Input Page": {
            ...
            niva_med_Que.find({ "application_id": applicationId, "CRN": crn }, function (err, data) {
                if (err) {
                    ...
                    if (data && data.length > 0 && data[0] && data[0].hasOwnProperty('_doc') && data[0]['_doc'].hasOwnProperty('redirect_url') && data[0]['_doc']['redirect_url']) {
                        ...
                        var client = new Client();
                        process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = 0;
                        let service_url = config.environment.name === 'Production' ? '' : 'https://rules1.nivabupa.com/api/v1/survey/get_uw_form';
                        client.post(service_url, args, function (data, response) {
                            try {
                                if (data.urls) {
...
router.post("/kyc_details/oriental_fetch_kyc_details", function (req, res) {
    try {
        ...
        let kyc_create_response = ObjRequest.details ? ObjRequest.details : "";
        let LM_Data = {
            "KYC_Doc_No": (ObjRequest.Document_ID === undefined || ObjRequest.Document_ID === "" || ObjRequest.Document_ID === null) ? "" : ObjRequest.Document_ID,
            "KYC_Number": ObjRequest.details.ckycNo ? ObjRequest.details.ckycNo : "",
            "KYC_FullName": ObjRequest.details.insuredName ? ObjRequest.details.insuredName : cust_name,
            "KYC_Ref_No": ObjRequest.details.ckycNo ? ObjRequest.details.ckycNo : "",
            ...
            "mobileNumber": kyc_create_response ? kyc_create_response : ObjRequest.Proposal_Request.mobile,
            "DOB": ObjRequest.details.dob ? ObjRequest.details.dob : ObjRequest.Proposal_Request.birth_date,
            "PAN": ObjRequest.details.individualPAN ? ObjRequest.details.individualPAN : ObjRequest.Proposal_Request.pan,
            'Product_Id': (ObjRequest.Product_Id === undefined || ObjRequest.Product_Id === "" || ObjRequest.Product_Id === null) ? 0 : ObjRequest.Product_Id - 0,
            'User_Data_Id': udid,
            'Email': (proposal_request.email === undefined || proposal_request.email === "" || proposal_request.email === null) ? "" : proposal_request.email,
            'Proposal_Id': ObjRequest.hasOwnProperty('Proposal_Id') ? ObjRequest.Proposal_Id : ""
        ...
        let saveObj = {
            ...
            'KYC_Ref_No': ObjRequest.details.ckycNo ? ObjRequest.details.ckycNo : "",
            'Full_Name': cust_name,
            'KYC_Full_Name': ObjRequest.details.insuredName ? ObjRequest.details.insuredName : cust_name,
            'proposal_page_url': ObjRequest.hasOwnProperty('proposal_url') ? ObjRequest.proposal_url : "",
            'service_log_id': proposal_request.hasOwnProperty('slid') ? proposal_request.slid : ""
        };
        ...
        kyc_detail.findOne({ 'PB_CRN': crn, 'Insurer_Id': insurer_id }).sort({ Modified_On: -1 }).exec(function (err, db_save_kyc_detail) {
            if (err) {
                ...
                if (db_save_kyc_detail) {
                    kyc_detail.update({ "PB_CRN": crn, "Insurer_Id": insurer_id }, { $set: LM_Data }, function (err, db_update_kyc_detail) {
                        if (err) {
                            res.json({ "Msg": err, "Status": "FAIL" });
                        } else {
                    ...
                    kyc_detail1.save(saveObj, function (err, users) {
                        if (err) {
                            ...
                            if (ObjRequest.hasOwnProperty("status") && ObjRequest.status === "auto_approved") {
                                LM_Data['KYC_Status'] = "VERIFY_SUCCESS";
                                LM_Data['ckyc_remarks'] = "NA";
                            } else {
                                LM_Data['KYC_Status'] = "VERIFY_FAIL";
                                LM_Data['KYC_Redirect_URL'] = ObjRequest['KYC_Redirect_URL'] ? ObjRequest['KYC_Redirect_URL'] : "";
                                LM_Data['ckyc_remarks'] = ObjRequest.status ? ObjRequest.status : "KYC not created";
                            }
                            let req_txt = {
                    ...
                    fatakpay_details.find({ PB_CRN: objRequest.crn, User_Data_Id: objRequest.udid }, function (err, fatakPay_data) {
                        if (err) {
                            ...
                            if (fatakPay_data.length > 0) {
                                ...
                                LM_Data['Status'] = eligibility_response['data'].hasOwnProperty('eligibility_status') && eligibility_response['data'].eligibility_status  === true ? "Eligible" : "Pending";
