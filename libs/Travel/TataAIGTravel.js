...
//        this.prepared_request['___proposal_id___'] = this.lm_request['insurer_customer_identifier_2'];
//        this.processed_request['___proposal_id___'] = this.prepared_request['___proposal_id___'];
        ...
        if (this.lm_request.hasOwnProperty('visiting_countries') && this.lm_request['visiting_countries']) {
            ...
            this.processed_request['___visiting_countries___'] = JSON.stringify(this.prepared_request['___visiting_countries___']);
        }
    ...
    if (this.lm_request['method_type'] === 'Verification') {
            this.prepared_request['payment_id'] = this.insurer_master.service_logs.pb_db_master.Insurer_Response.proposal_res.data[0].payment_id;
    ...
    for (member = 1; member <= this.lm_request['adult_count']; member++) {
        ...
        if (this.lm_request['method_type'] === 'Proposal') {
            ...
            this.prepared_request['___member_' + member + '_nominee_name___'] = this.lm_request['member_' + member + '_nominee_name'];
            this.processed_request['___member_' + member + '_nominee_name___'] = this.prepared_request['___member_' + member + '_nominee_name___'];
            
            this.prepared_request['___member_' + member + '_nominee_relation___'] = this.lm_request['member_' + member + '_nominee_relation'];
            this.processed_request['___member_' + member + '_nominee_relation___'] = this.prepared_request['___member_' + member + '_nominee_relation___'];

            if (this.lm_request['member_' + member + '_ped'] && typeof (this.lm_request['member_' + member + '_ped']) === "object") {
                this.prepared_request['___member_' + member + '_ped___'] = this.lm_request['member_' + member + '_ped'][0]['id'] - 0 === 0 ? false : true;
                this.processed_request['___member_' + member + '_ped___'] = this.prepared_request['___member_' + member + '_ped___'];

                this.prepared_request['___member_' + member + '_ped_desc___'] = this.lm_request['member_' + member + '_ped'][0]['id'] - 0 === 0 ? "" : this.lm_request['member_' + member + '_other_diseases'];
             ...
             this.prepared_request['___member_' + member + '_age___'] = this.lm_request['member_' + member + '_age_1'];
            this.processed_request['___member_' + member + '_age___'] = this.prepared_request['___member_' + member + '_age___'];
    ...
    for (member = 3; member <= this.lm_request['child_count'] + 2; member++) {
        ...
        txt_replace_with += txt_replace.replaceAll('___member_array', '___member_' + member).replaceAll('____member_count___', '_' + (adultCount + (member - 2)));;
        if (this.lm_request['method_type'] === 'Proposal') {
    ...
    if (this.method_content[0] !== '<') {// for json
        ...
        let Total_Count
        if (['Premium', 'Customer'].includes(this.lm_request['method_type'])) {
...
TataAIGTravel.prototype.customer_response_handler = function (objResponseJson) {
    ...
    try {
        ...
        if (Error_Msg === 'NO_ERR') {
            var Customer = {};
            Customer = {
                        'insurer_customer_identifier_2': objResponseJson["data"][0]["proposal_id"]
            };
            objServiceHandler.Insurer_Transaction_Identifier = objResponseJson["data"][0]["quote_no"];
...
TataAIGTravel.prototype.proposal_response_handler = function (objResponseJson) {
    ...
    try {
        ...
        } else if (paymentResponseData && paymentResponseData.hasOwnProperty('status') && paymentResponseData.status === 200 && paymentResponseData.hasOwnProperty('message_txt') && paymentResponseData.message_txt.toLowerCase().includes("success")) {
        } else {
              if (objResponseJson && objResponseJson.proposal_res && proposalResponseData.hasOwnProperty('message_txt') && proposalResponseData.message_txt !== "Proposal submitted successfully") {
                Error_Msg = proposalResponseData.hasOwnProperty('message_txt') ? proposalResponseData.message_txt : proposalResponseData;
            } else if (objResponseJson && objResponseJson.payment_res) {
                Error_Msg = paymentResponseData.hasOwnProperty('message_txt') ? paymentResponseData.message_txt : paymentResponseData;
            } else {
                Error_Msg = paymentResponseData.hasOwnProperty('message_txt') ? paymentResponseData.message_txt : objResponseJson;
            }
        }

        if (Error_Msg === 'NO_ERR') {
            let objPremiumVerification = this.premium_verification(this.lm_request['final_premium'], proposalResponseData.data[0][['total_premium']]);
...
//        Insurer Payment response handling 
        if (Error_Msg === 'NO_ERR') {
            let payment_data = paymentResponseData['data'];
            var pg_data = {
                orderID: payment_data.orderID,
                uniqueIdentifier:payment_data.uniqueIdentifier
            };
            objServiceHandler.Payment.pg_data = pg_data;
            objServiceHandler.Payment.pg_url = payment_data.paymentLink_web;
            objServiceHandler.Payment.pg_redirect_mode = 'POST';
            objServiceHandler.Insurer_Transaction_Identifier = proposalResponseData["data"][0]["proposal_no"];
            objServiceHandler.insurer_customer_identifier = proposalResponseData["data"][0]["payment_id"];
        }
...
//Asia Guard
        ...
        {'area': 'Asia', 'plan_code_txt': "Gold", 'plan_code': 'G-Asia', "plan_id": 1110, 'trip_type': "SINGLE", 'GeneralPurpose': "ASIA", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Asia Guard'},
        {'area': 'Asia', 'plan_code_txt': "Silver", 'plan_code': 'S-Asia', "plan_id": 1111, 'trip_type': "SINGLE", 'GeneralPurpose': "ASIA", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Asia Guard'},
        {'area': 'Asia', 'plan_code_txt': "Silver Plus", 'plan_code': 'SP-Asia', "plan_id": 1121, 'trip_type': "SINGLE", 'GeneralPurpose': "ASIA", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Asia Guard'},
        {'area': 'Asia', 'plan_code_txt': "Gold", 'plan_code': 'G', "plan_id": 1101, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Asia', 'plan_code_txt': "Platinum", 'plan_code': 'P', "plan_id": 1102, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Asia', 'plan_code_txt': "Silver", 'plan_code': 'S', "plan_id": 1103, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Asia', 'plan_code_txt': "Silver with family", 'plan_code': 'SWF', "plan_id": 1104, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Asia', 'plan_code_txt': "Silver Plus", 'plan_code': 'SP', "plan_id": 1105, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Asia', 'plan_code_txt': "Silver Plus with family", 'plan_code': 'SPWF', "plan_id": 1106, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Asia', 'plan_code_txt': "Senior", "plan_id": 1107, 'plan_code': 'Senior', 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 71, 'max_age': 86, 'selected_product': 'Travel Guard'},
        {'area': 'Asia', 'plan_code_txt': "Gold", 'plan_code': 'G-SL', "plan_id": 1112, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Asia', 'plan_code_txt': "Platinum", 'plan_code': 'P-SL', "plan_id": 1113, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Asia', 'plan_code_txt': "Silver", 'plan_code': 'S-SL', "plan_id": 1114, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Asia', 'plan_code_txt': "Silver with family", 'plan_code': 'SWF-SL', "plan_id": 1115, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Asia', 'plan_code_txt': "Silver Plus", 'plan_code': 'SP-SL', "plan_id": 1116, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Asia', 'plan_code_txt': "Silver Plus with family", 'plan_code': 'SPWF-SL', "plan_id": 1117, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding AmerUSA/Canadaicas", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Asia', 'plan_code_txt': "Senior", 'plan_code': 'Senior-SL', 'trip_type': "SINGLE", "plan_id": 1118, 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 71, 'max_age': 86, 'selected_product': 'Travel Guard Without Sublimit'},
        
//Africa
        {'area': 'Africa', 'plan_code_txt': "Gold", 'plan_code': 'G', "plan_id": 1101, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Africa', 'plan_code_txt': "Platinum", 'plan_code': 'P', "plan_id": 1102, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Africa', 'plan_code_txt': "Silver", 'plan_code': 'S', "plan_id": 1103, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Africa', 'plan_code_txt': "Silver with family", 'plan_code': 'SWF', "plan_id": 1104, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Africa', 'plan_code_txt': "Silver Plus", 'plan_code': 'SP', "plan_id": 1105, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Africa', 'plan_code_txt': "Silver Plus with family", 'plan_code': 'SPWF', "plan_id": 1106, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Africa', 'plan_code_txt': "Senior", "plan_id": 1107, 'plan_code': 'Senior', 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 71, 'max_age': 86, 'selected_product': 'Travel Guard'},
        {'area': 'Africa', 'plan_code_txt': "Gold", 'plan_code': 'G-SL', "plan_id": 1112, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Africa', 'plan_code_txt': "Platinum", 'plan_code': 'P-SL', "plan_id": 1113, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Africa', 'plan_code_txt': "Silver", 'plan_code': 'S-SL', "plan_id": 1114, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Africa', 'plan_code_txt': "Silver with family", 'plan_code': 'SWF-SL', "plan_id": 1115, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Africa', 'plan_code_txt': "Silver Plus", 'plan_code': 'SP-SL', "plan_id": 1116, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Africa', 'plan_code_txt': "Silver Plus with family", 'plan_code': 'SPWF-SL', "plan_id": 1117, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding AmerUSA/Canadaicas", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Africa', 'plan_code_txt': "Senior", 'plan_code': 'Senior-SL', 'trip_type': "SINGLE", "plan_id": 1118, 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 71, 'max_age': 86, 'selected_product': 'Travel Guard Without Sublimit'},

//Europe
        {'area': 'Europe', 'plan_code_txt': "Gold", 'plan_code': 'G', "plan_id": 1101, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Europe', 'plan_code_txt': "Platinum", 'plan_code': 'P', "plan_id": 1102, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Europe', 'plan_code_txt': "Silver", 'plan_code': 'S', "plan_id": 1103, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Europe', 'plan_code_txt': "Silver with family", 'plan_code': 'SWF', "plan_id": 1104, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Europe', 'plan_code_txt': "Silver Plus", 'plan_code': 'SP', "plan_id": 1105, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Europe', 'plan_code_txt': "Silver Plus with family", 'plan_code': 'SPWF', "plan_id": 1106, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 0.5, 'max_age': 71, 'selected_product': 'Travel Guard'},
        {'area': 'Europe', 'plan_code_txt': "Senior", "plan_id": 1107, 'plan_code': 'Senior', 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 71, 'max_age': 86, 'selected_product': 'Travel Guard'},
        {'area': 'Europe', 'plan_code_txt': "Gold", 'plan_code': 'G-SL', "plan_id": 1112, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Europe', 'plan_code_txt': "Platinum", 'plan_code': 'P-SL', "plan_id": 1113, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Europe', 'plan_code_txt': "Silver", 'plan_code': 'S-SL', "plan_id": 1114, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Europe', 'plan_code_txt': "Silver with family", 'plan_code': 'SWF-SL', "plan_id": 1115, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Europe', 'plan_code_txt': "Silver Plus", 'plan_code': 'SP-SL', "plan_id": 1116, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Europe', 'plan_code_txt': "Silver Plus with family", 'plan_code': 'SPWF-SL', "plan_id": 1117, 'trip_type': "SINGLE", 'GeneralPurpose': "Worldwide Excluding AmerUSA/Canadaicas", 'min_age': 56, 'max_age': 71, 'selected_product': 'Travel Guard Without Sublimit'},
        {'area': 'Europe', 'plan_code_txt': "Senior", 'plan_code': 'Senior-SL', 'trip_type': "SINGLE", "plan_id": 1118, 'GeneralPurpose': "Worldwide Excluding USA/Canada", 'min_age': 71, 'max_age': 86, 'selected_product': 'Travel Guard Without Sublimit'},
...
//        lm_request['travelling_to_area'] = 'WWExUSCanada';
//    };
    if(lm_request['trip_type'] === "MULTI" && lm_request['member_1_age'] > 70){
        return false;
    } else if (lm_request['travelling_to_area'] === 'Asia' && (lm_request['member_1_age'] < 18 || lm_request['member_1_age'] > 71) && ["G-Asia", "S-Asia","SP-Asia"].indexOf(planCode) > -1) {
        return false;
    } else if (lm_request['travelling_to_area'] === 'Asia' && (lm_request['member_1_age'] > 71) && ["Senior-SL","Senior"].indexOf(planCode) === -1) {
        return false;
    }else if (lm_request['travelling_to_area'] === 'Asia' && (lm_request['member_1_age'] < 56 || lm_request['member_1_age'] > 71) && ["G-SL","P-SL","S-SL","SWF-SL","SP-SL","SPWF-SL"].indexOf(planCode) > -1) {
        return false;
    } else if (['WWExUSCanada','Europe','Africa'].includes(lm_request['travelling_to_area']) && ["G-Asia", "S-Asia","SP-Asia"].indexOf(planCode) > -1) {
        return false;
    } else if (lm_request['travelling_to_area'] === 'Asia' && total_days > 30 &&  ["G-Asia", "S-Asia","SP-Asia"].indexOf(planCode) > -1) {
        return false;
    } else if (lm_request['trip_type'] === "SINGLE" && ["AG", "AP", "AG-SL", "AP-SL"].indexOf(planCode) > -1) {
        return false;
    } else if ((lm_request['travelling_to_area'] === 'WorldWide') && ["G-Asia", "S-Asia","SP-Asia"].indexOf(planCode) > -1) {
        return false;
    } else if (lm_request['trip_type'] === "MULTI" && ['60', '90'].includes(lm_request['maximum_duration'])) {
        return false;
    } else if (lm_request['travel_insurance_type'] === "individual" && ["SWF", "SPWF", "SWF-SL", "SPWF-SL"].indexOf(planCode) > -1) {
        return false;
    } else if (lm_request['trip_type'] === "MULTI" && lm_request['travelling_to_area'] === 'WWExUSCanada') {
        return false; 
    ...
    else if (lm_request['trip_type'] === "SINGLE" && lm_request['adult_count'] - 0 &&  ["G-SL", "P-SL", "S-SL", "SP-SL", "SWF-SL", "SPWF-SL"].indexOf(planCode) > -1 ) {
        for (let member = 1; member <= lm_request['adult_count']; member++) {
            if (lm_request['member_' + member + '_age'] < 56 || lm_request['member_' + member + '_age'] > 71) {
                return false;
            }
        }
        for (let member = 3; member <= lm_request['child_count'] + 2; member++) {
           if (lm_request['member_' + member + '_age'] < 56 || lm_request['member_' + member + '_age'] > 71) {
                return false;
            } 
        }
        return true;
    }else if (lm_request['trip_type'] === "SINGLE" && lm_request['child_count'] - 0) {
        for (let member = 3; member <= lm_request['child_count'] + 2; member++) {
            var difference = moment().diff(lm_request['member_' + member + '_birth_date'], 'months');
            if (difference < 6) {
                return false;
            }
        }
