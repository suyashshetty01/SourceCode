...
function verify_Token(req, res, next) {
    ...
    try {
        if(req.hasOwnProperty('headers') && req.headers.hasOwnProperty('authorization')) {
            ...
            let decode = jwt.verify(token, jwt_ACCESS_TOKEN);
            console.log(decode);
            return next();
        } else {
            return next();
...
router.post('/premium_initiate', verify_Token, master_process, function (req, res, next) {
    try {
        ...
        if ((parseInt(objRequestCore['product_id']) === 1) && (objRequestCore['vehicle_insurance_subtype'] === '3CH_0TP')) {
            ...
            if ([1, 10, 12].indexOf(objRequestCore['product_id']) > -1) {
                ...
                if (objRequestCore['vehicle_registration_type'] === "corporate") {
                    // arr_msg.push('Due to KYC implementation, Company Own Insurance is temporary discontinued.');
                }
                ...
                objClient.get(config.environment.weburl + '/clients', {}, function (data, response) {
                    if (data) {
                        if (data.hasOwnProperty(objRequestCore.client_key) &&
                            ...
                            if (objRequestCore.product_id === 2 && objRequestCore.hasOwnProperty('addon_selected')
                                && objRequestCore.addon_selected === 'yes' && objRequestCore.hasOwnProperty('addon_srn')
                                && objRequestCore.addon_srn) {
                                requestUniqueId = objRequestCore['addon_srn'].split('_')[0];
                                userDataId = objRequestCore['addon_srn'].split('_')[1];
                                objBase.request_unique_id = requestUniqueId;
                            } else {
                            ...
                            if (objRequestCore.product_id === 2 && objRequestCore.hasOwnProperty('addon_selected') && objRequestCore.addon_selected === 'yes') {
                                objUserData.Modified_On = new Date();
                                User_Data.findOneAndUpdate({ 'User_Data_Id': userDataId }, { $set: objUserData }, { new: true }, function (err, objDbUserData) {
                                    if (err) {
                                        console.error('objDbPospSave', 'user_data_not_updated', err);
                                    } else {
                                        console.log(objDbUserData);
                                        if (userDataId != '' && userDataId != undefined && userDataId != null && userDataId) {
                                            objBase['udid'] = userDataId - 0;
                                            temp_udid = userDataId - 0;
                                            objRequestCore['udid'] = userDataId - 0;
                                        } else {
                ...
                if (objDbUserData) {
                    ...
                    if (arr_sale.indexOf(Last_Status) > -1) {
                        ...
                        if (['addon', 'addon_quote', 'addon_proposal_agent', 'addon_proposal_customer'].indexOf(data_type) > -1) {
                            ...
                            if (objDbUserData.Product_Id === 2 && [20, 38].indexOf(objRequestCore['InsurerId']) > -1) {
                                let insId = objRequestCore.hasOwnProperty('InsurerId') && objRequestCore['InsurerId'];
                                let planId = objRequestCore.hasOwnProperty('PlanId') && objRequestCore['PlanId'];

                                if (objDbUserData['_doc'].hasOwnProperty('Addon_Request') && objDbUserData.Addon_Request) {
                                    addon_req_in_db = objDbUserData.Addon_Request;
                                    Health_Addon_request = addon_req_in_db;
                                }
                                if (!Health_Addon_request[insId]) {
                                    Health_Addon_request[insId] = {};
                                }
                                Health_Addon_request[insId][planId] = Addon_Request;
                                objUserData.Addon_Request = Health_Addon_request;
...
function send_payment_link_handler(reminder, objRequestCore, dbUserData, res) {
    ...
    if ((dbUserData.Premium_Request['ss_id'] - 0) > 0) {
        email_agent = dbUserData.Premium_Request['posp_email_id'] ? dbUserData.Premium_Request['posp_email_id'].toString() : "0";
        objRequestCore['agent_mobile'] = dbUserData.Premium_Request['posp_mobile_no'] ? dbUserData.Premium_Request['posp_mobile_no'].toString() : "";
        objRequestCore['agent_name'] = dbUserData.Premium_Request['posp_first_name'] ? dbUserData.Premium_Request['posp_first_name'].toString() : "";
...
function reCaptcha(req, res, next) {
    try {
        ...
        if ((req.body['ss_id'] - 0) <= 0) {
            ...
            var Client = require('node-rest-client').Client;
            var client = new Client();
            client.post("https://www.google.com/recaptcha/api/siteverify", reqObj_args, (responseData, apiResponse) => {
                try {
                    if (responseData && responseData.success) {
                        req.body.isCaptchaVerified = true;
                        next();
                    } else {
                        req.body.isCaptchaVerified = false;
                        return res.status(403).send('Captcha verification failed, please retry.');
                    }
                } catch (err) {
...
router.post('/vehicle_info', reCaptcha, pre_vehicle_info, function (req, res, next) {
...
function validateCustomerEmail_Handler(obj_validation, req, res, next) {
    ...
    var Email = require('../models/email');
    var objModelEmail = new Email();
    console.error('Niva Health Renew LINE7', JSON.stringify(req.User_Data));
    if (req.User_Data['Product_Id'] === 5 || req.User_Data['Product_Id'] === 16 || req.User_Data['Product_Id'] === 17 || req.User_Data['Product_Id'] === 18 || req.User_Data['Product_Id'] === 4 || req.User_Data['Product_Id'] === 8) {
        ...
        console.error('Niva Health Renew LINE8');
        if (req.User_Data['Premium_Request'] && req.User_Data['Premium_Request']['health_policy_type'] && req.User_Data['Premium_Request']['health_policy_type'] === "renew" && req.User_Data && req.User_Data['Premium_Response'] &&
            req.User_Data['Premium_Response']['Member_details'] && req.User_Data['Premium_Response']['Member_details'][0] && req.User_Data['Premium_Response']['Member_details'][0]['email'] !== "") {
        console.error('Niva Health Renew LINE9');
                ...
                if (obj_validation['proposal_list'] === 'FAIL') {
                    ...
                    objModelProposal.save(function (err, objDbProposal) {
                        ...
                        if (err) {
                            ...
                            // for razorpay start				
                            if ([1, 10, 12, 2].indexOf(dbUserData['Product_Id']) > -1 && [3, 6, 46, 38, 17, 11].indexOf(objRequestCore['insurer_id'] - 0) > -1) {
                                var Client = require('node-rest-client').Client;
                                var client = new Client();
                                let obj_pg_rzr = {
...
function validateCustomerEmail(req, res, next) {
    try {
        ...
        User_Data.findOne(ud_cond, function (err, dbUserData) {
            ...
            } else if ([2].indexOf(objRequestCore['product_id']) > -1 && dbUserData['Premium_Request'].hasOwnProperty('renewal') && dbUserData['Premium_Request']['renewal'] === 'yes') {
                console.error('Niva Health Renew LINE10');
                var mobile_customer = "9999999999";//dbUserData['Premium_Request']['mobile'];
            } else {
            ...
            if ([1, 10, 12].indexOf(dbUserData['Product_Id']) > -1) {
                if ([3, 6, 46, 17].indexOf(objRequestCore['insurer_id'] - 0) > -1 && objRequestCore['method_type'] === 'Proposal') {
                    //obj_validation['pre_pg_process_validation'] = 'PENDING';
                    if (objRequestCore['insurer_id'] - 0 === 17) {
                        obj_validation['pre_pg_process_validation'] = 'PENDING';
                    }
                }
                if (dbUserData['Premium_Request']['vehicle_insurance_type'] === "renew") {
...
router.get('/car/:vehicle_no', (req, res) => {
    try {
        let RegistrationNumber = req.params.vehicle_no || "";
		let vehicleNcbCurrent = "";
		let policy_exist = "";
        let NoClaimBonusStat = "";
        let break_in_case = "";
        if (RegistrationNumber === "") {
            res.send("VEHICLE NUMBER IS MANDATORY");
        } else {
            let vehicle_info_args = {
                data: {
                    "secret_key": "SECRET-HZ07QRWY-JIBT-XRMQ-ZP95-J0RWP3DYRACW",
                    "client_key": "CLIENT-CNTP6NYE-CU9N-DUZW-CSPI-SH1IS4DOVHB9",
            ...
            console.error("vehicle_info_args", vehicle_info_args, config.environment.weburl + '/quote/vehicle_info');

            let Client = require('node-rest-client').Client;
            let client = new Client();

            client.post(config.environment.weburl + '/quote/vehicle_info', vehicle_info_args, (vehicle_info_data, vehicle_info_res) => {
                try {
					console.error("vehicle_info_data", vehicle_info_data);
                    let vehicle_info_obj = vehicle_info_data;
                    let is_allow = false;
                    if (vehicle_info_obj) {
                        is_allow = true;
                        let registration_date_year = vehicle_info_obj.Registration_Date;
                        let dateParts = registration_date_year && registration_date_year.split("/") || "";
                        let registration_year = dateParts ? (new Date(dateParts[2], dateParts[1] - 1, dateParts[0]).getFullYear()) : "";


                        
                        let vehicleInsuranceSubtype;
                        let ageDiff = moment().format('YYYY') - registration_year;
                        let ProductTypeNewrenew = 'renew';
                        let tp_policy_exists = "no";
                        
                        let policy_exp_date = vehicle_info_obj.Pyp_Policy_Expiry_Date ? vehicle_info_obj.Pyp_Policy_Expiry_Date : "";
                        ...
                        let status;
                        let ss_id = req.params.ssid;
                        let Pre_expiry_date = vehicle_info_obj.Pyp_Policy_Expiry_Date ? vehicle_info_obj.Pyp_Policy_Expiry_Date : "";
                        let Pre_insurerid = vehicle_info_obj.Pyp_Insurer_Id;
                        let date_Parts = Pre_expiry_date.split('/');
                        let expiry = `${date_Parts[2]}-${date_Parts[1]}-${date_Parts[0]}`;
                        ...
                        let current_date = new Date(DateStr);
                        let Exp_date = new Date(expiry);

                        let timeDifference = current_date - Exp_date;
                        let daysDifference = Math.floor(timeDifference / (1000 * 60 * 60 * 24));
                        if (Pre_insurerid == "" && Pre_expiry_date == "") {
                            status = "Don't have policy";
                            policy_exist = "no";
                            break_in_case = "yes";
                            NoClaimBonusStat = "no";
                            vehicleNcbCurrent = "0";
                            ...
                            if (daysDifference <= 0) {
                                ...
                                status = "Expired within 90 days";
                                policy_exist = "yes";
                                break_in_case = 'yes';
                            } else if (daysDifference > 90) {
                                status = "Expired more than 90 days";
                                policy_exist = "yes";
                                NoClaimBonusStat = "no";
                                vehicleNcbCurrent = "0";
                                break_in_case = 'yes';
                        ...
                        //Vehicle insurance subtype
                        if (vehicle_info_obj.Product_Id == 1) {
                            if (ageDiff < 3) {
                                vehicleInsuranceSubtype = '1OD_0TP';
                                tp_policy_exists = "yes"

                            }
                            if (ageDiff >= 3) {
                                vehicleInsuranceSubtype = '1CH_0TP';
                            }
                        } else if (vehicle_info_obj.Product_id == 10) {
                            if (ageDiff < 5) {
                                vehicleInsuranceSubtype = '1OD_0TP';
                            }
                            if (ageDiff >= 5) {
                        ...
                        } else if (vehicle_info_obj.Product_id == 12) {
                            vehicleInsuranceSubtype = '1CH_0TP'
                        } else {
                        ...
                        //vehicleNcbCurrent
                        if (status == "Don't have policy" || status == "Expired more than 90 days" || ageDiff <= 1) {
                            vehicleNcbCurrent = "0";
                        } else if (ageDiff == 2) {
                            ...
                            return "Issue related to vehicle age difference";
                        ...
                        //is_financed
                        let is_financed;
                        if (vehicle_info_obj.Financier_Name == "") {
                            is_financed = "no";
                        } else {
                            is_financed = "yes";
                        }
                        ...
                        //regno_rtocode
                        let regno_rtocode;
                        let registration_number = vehicle_info_obj.Registration_Number;
                        let regno_formated = registration_number ? registration_number.replace(/^([A-Z]{2})(\d{2}).*$/, "$1-$2") : "";
                        regno_rtocode = regno_formated;


                        //for Name
                        let Ownername = vehicle_info_obj.Owner_Name;
                        let nameParts = Ownername ? Ownername.trim().split(/\s+/) : "";
                        let firstName = nameParts[0] || "";
                        let middleName = nameParts && nameParts.slice(1, -1).join(" ") || "";
                        let lastName = nameParts[nameParts.length - 1] || "";

                    } else {
                        res.send("NO DATA AVAILABLE IN VEHICLE INFO API", vehicle_info_obj);
                    }
                    //premium initiate request
                    if (is_allow) {
                        let premium_args = {
                            data: {
                                "product_id": vehicle_info_obj.Product_Id ? vehicle_info_obj.Product_Id : "",
                                "vehicle_id": vehicle_info_obj.Variant_Id ? vehicle_info_obj.Variant_Id : "",
                                "client_id": 2,
                                "rto_id": vehicle_info_obj.CityofRegitrationId ? vehicle_info_obj.CityofRegitrationId : "",
                                "vehicle_insurance_type": "renew",
                                "vehicle_registration_date": vehicle_info_obj.Registration_Date ? moment(vehicle_info_obj.Registration_Date, "DD-MM-YYYY").format("YYYY-MM-DD") : "",
                                "vehicle_manf_date": vehicle_info_obj.Registration_Date ? moment(vehicle_info_obj.Registration_Date, "DD-MM-YYYY").format("YYYY-MM") + "-01" : "",
                                "policy_expiry_date": vehicle_info_obj.Pyp_Policy_Expiry_Date ? moment(vehicle_info_obj.Pyp_Policy_Expiry_Date, "DD-MM-YYYY").format("YYYY-MM-DD") : "",
                                "prev_insurer_id": vehicle_info_obj.Pyp_Insurer_Id ? vehicle_info_obj.Pyp_Insurer_Id : "",
                                "vehicle_registration_type": "individual",
                                "vehicle_ncb_current": vehicleNcbCurrent ? vehicleNcbCurrent : "",
                                "is_claim_exists": NoClaimBonusStat,
                                "method_type": "Premium",
                                "execution_async": "yes",
                                "electrical_accessory": "0",
                                "non_electrical_accessory": "0",
                                "registration_no": vehicle_info_obj.Registration_Number ? vehicle_info_obj.Registration_Number.replace(/([A-Z]{2})(\d{2})([A-Z]{2})(\d{4})/, "$1-$2-$3-$4") : "",
                                "regno_rtocode": regno_rtocode ? regno_rtocode : "",
                                "is_llpd": "no",
                                "is_antitheft_fit": "no",
                                "is_external_bifuel": "no",
                                "is_aai_member": "no",
                                "fuel_type": vehicle_info_obj.Fuel_Type ? vehicle_info_obj.Fuel_Type : "",
                                "external_bifuel_type": "",
                                "external_bifuel_value": "0",
                                ...
                                "vehicle_insurance_subtype": vehicleInsuranceSubtype ? vehicleInsuranceSubtype : "",
                                "secret_key": "SECRET-HZ07QRWY-JIBT-XRMQ-ZP95-J0RWP3DYRACW",
                                "client_key": "CLIENT-CNTP6NYE-CU9N-DUZW-CSPI-SH1IS4DOVHB9",
                                "is_breakin": break_in_case ? break_in_case : "",
                                "is_inspection_done": "no",
                                "is_policy_exist": policy_exist,
                                ...
                                "ui_source": "UI22",
                                "is_quick_quote": "no",
                                "chassis_number": vehicle_info_obj.Chassis_Number ? vehicle_info_obj.Chassis_Number : "",
                                "engin_number": vehicle_info_obj.Engin_Number ? vehicle_info_obj.Engin_Number : "",
                                "customer_name": Ownername ? Ownername : "",
                                "permanent_pincode": vehicle_info_obj.Permanent_Pincode ? vehicle_info_obj.Permanent_Pincode : "",
                                "is_fastlane_manufacture": "yes",
                                "is_fastlane_registration": "yes",
                                "is_tp_policy_exists": tp_policy_exists,
                                "visitor_id": "",
                                "search_from": "diy",
                                "addon_applied": "no"
                            },
                        ...
                        client.post(config.environment.weburl + '/quote/premium_initiate', premium_args, (premium_initiate_data, premium_initiate_res) => {
                            try {
                                let premium_initiate_obj = premium_initiate_data;

                                if (premium_initiate_obj && premium_initiate_obj.Summary && premium_initiate_obj.Summary.Request_Unique_Id) {
                                    let product_id_obj = {
                                        1: "car-insurance",
                                        2: "two-wheeler-insurance",
                                        12: "commercial-vehicle-insurance"
                                    };
                                    let product_id = premium_initiate_obj.Request && premium_initiate_obj.Request.product_id && product_id_obj[premium_initiate_obj.Request.product_id] || "";
                                    redirect_url = 'https://www.policyboss.com/' + product_id + '/quotes?SID=' + premium_initiate_obj.Summary.Request_Unique_Id + '&ClientID=' + premium_initiate_obj.Request.client_id;
                                    return res.redirect(redirect_url);
                                } else {
                                    res.send(premium_initiate_obj.Msg || "INVALID REQUEST");
                                }
                            } catch (e) {
                                res.send("EXCEPTION IN API" + e.stack);
                            }
                        });
                    } else {
                        res.send("ERROR IN VEHICLE INFO API", vehicle_info_obj);
                    }
                } catch (e) {
                    res.send("EXCEPTION IN VEHICLE INFO API" + e.stack);
                }
            });
        }
    } catch (e) {
        res.send("EXCEPTION IN MAIN API" + e.stack);
    }
});
