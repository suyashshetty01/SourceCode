...
//			      let url_api = "http://horizon.policyboss.com:5000/sync_contacts/calculate_relationship_score/"+ss_id+"/"+mobile+"/"+update_data;
    ...
    app.post('/sync_contact/sync_contact_category_wrapper', function (req, res) { // amish sir api call - single ss_id/fba_id 
        let objSummary = {};
        try {
            var Client = require('node-rest-client').Client;
            var client = new Client();
            ...
            if (objRequest && objRequest.user_id && objRequest.mobile) {
                ...
                let sync_contact_add_api = "http://pbnext.policyboss.com/sync-contact/sync-contact-add-data/";
                let sync_contact_add_args = {
                    data: sync_contact_add_obj,
                    headers: {
                        'Content-Type': 'application/json'
                    }
                };
                let fetch_args = {
                    'mobileno': objRequest.mobile
                };
                if (objRequest.ss_id) {
                    fetch_args['ss_id'] = objRequest.ss_id - 0;
                    log_file_name = objRequest.ss_id;
                }
                if (objRequest.fba_id) {
                    fetch_args['fba_id'] = objRequest.fba_id - 0;
                    log_file_name = objRequest.fba_id;
                }
                client.post(sync_contact_add_api, sync_contact_add_args, function (sync_contact_add_data, sync_contact_add_res) {
                    let is_category_synced = 2;
                    if (sync_contact_add_data && sync_contact_add_data.status && sync_contact_add_data.status === "success") {
                        is_category_synced = 1;
                    }
                    let update_args = {
                        "Is_Category_Synced": is_category_synced,
                        "Is_Category_Response_Received": 0,
                        "Category_Synced_On": new Date()
                    };
                    Sync_Contact.findOneAndUpdate(fetch_args, {$set: update_args}, function (sync_contact_err, catagoryWrapperData) {
                        if (sync_contact_err) {
                            objSummary['Status'] = 'FAIL';
                            objSummary['Msg'] = 'Is_Category_Synced updation fail';
                            objSummary['Data'] = sync_contact_err;
                            ...
                            objSummary['Data'] = sync_contact_add_data;
                            return res.json(objSummary);
                        }
                    });
                    try {
                        ...
                        let log_write_data = {objRequest: sync_contact_add_obj, responseData: sync_contact_add_data};
                        if (!fs.existsSync(appRoot + "/tmp/log/Sync_Category")) {
                            fs.mkdirSync(appRoot + "/tmp/log/Sync_Category");
                        }
                        fs.appendFile(appRoot + "/tmp/log/Sync_Category/" + log_file_name + "_sync_category_" + today_str + ".log", JSON.stringify(log_write_data) + "\r\n", function (log_write_err) {
                            if (log_write_err) {
                                //return console.log(err);
                            }
                            console.log("The file was saved!");
    ...
    //25-07-2024 - remove from postservicecall & add here
    app.get('/sync_contact/all_contact_category_sync', function (req, res) { // all sync contact category sync at time of user login with ss_id & erp_id
        try {
            let objSummary = {
                "Data": {
                    "total_batch_call": 0,
                    "total_sync_contact_category_count": 0,
                    "total_sync_contact_category_fail": 0,
                    "total_sync_contact_category_success": 0,
                    "exception": []
                }
            };
            let Client = require('node-rest-client').Client;
            ...
            let erp_id = objRequest.erp_id || 0;
            let error_msg = '';
            let is_allow = true;
            let arr_msg = [];
            if (ss_id === 0) {
                arr_msg.push('SS_ID :: MISSING');
            }
            if (erp_id === 0) {
                arr_msg.push('ERP_ID :: MISSING');
            }
            if (arr_msg.length > 0) {
                is_allow = false;
            ...
            if (is_allow) {
                let query = {"ss_id": ss_id, $or: [{Is_Category_Synced: {$exists: false}}, {Is_Category_Synced: 0}]};
                Sync_Contact.find(query).exec(function (sync_cont_err, sync_cont_data) {
                    try {
                        if (sync_cont_err) {
                            objSummary['Status'] = 'FAIL';
                            objSummary['Msg'] = 'Error While Fetching sync contact category data from sync contact';
                            objSummary['Data'] = sync_cont_err;
                            return res.json(objSummary);
                        } else {
                            if (sync_cont_data.length > 0) {
                                objSummary['Data']['total_sync_contact_category_count'] = sync_cont_data.length;
                                for (let k = 0; k < sync_cont_data.length; k++) {
                                    let sync_cont_category_doc = sync_cont_data[k]._doc;
                                    let add_sync_contact_category_reqObj = {
                                        "ss_id": ss_id,
                                        "user_id": erp_id,
                                        "name": sync_cont_category_doc.name || "",
                                        "email": sync_cont_category_doc.email || "",
                                        "mobile": sync_cont_category_doc.mobileno || ""
                                    };
                                    let add_sync_contact_category_args = {
                                        data: add_sync_contact_category_reqObj,
                                        headers: {
                                            'Content-Type': 'application/json'
                                        }
                                    };
                                    client.post(config.environment.weburl + '/sync_contact/add_sync_contact_category', add_sync_contact_category_args, function (category_data, category_resp) {
                                        try {
                                            objSummary['Data']['total_batch_call']++;
                                            if (category_data.Status && category_data.Status === 'SUCCESS') {
                                                if (category_data.Data && category_data.Data.status === 'success') {
                                                    objSummary['Data']['total_sync_contact_category_success']++;
                                                } else {
                                                    objSummary['Data']['total_sync_contact_category_fail']++;
                                                }
                                            } else {
                                                objSummary['Data']['total_sync_contact_category_fail']++;
                                            }
                                        } catch (err) {
                                            console.error('/sync_contact/all_contact_category_sync', err.stack);
                                            objSummary['Data']['exception'].push(err.stack);
                                        }
                                        if (objSummary['Data']['total_batch_call'] === objSummary['Data']['total_sync_contact_category_count']) {
                                            objSummary['Status'] = 'SUCCESS';
    ...
    app.post('/sync_contact/add_sync_contact_category', function (req, res) { // amish sir api call - create log & save in sync_contact db
        let objSummary = {};
        try {
            var Client = require('node-rest-client').Client;
            var client = new Client();
            req.body = JSON.parse(JSON.stringify(req.body));
            let objRequest = req.body;
            let user_id = objRequest.user_id || '';
            let mobile = objRequest.mobile || '';
            let ss_id = objRequest.ss_id || '';
            let name = objRequest.name || '';
            let error_msg = '';
            let is_allow = true;
            let arr_msg = [];
            if (user_id === '') {
                arr_msg.push('ERPID :: INVALID');
            }
            if (mobile === '') {
                arr_msg.push('Mobile Number :: INVALID');
            }
            if (ss_id === '') {
                arr_msg.push('SSID :: INVALID');
            }
            if (name === '') {
                arr_msg.push('Name :: INVALID');
            }
            if (arr_msg.length > 0) {
                is_allow = false;
                error_msg = arr_msg.join(', ');
            }
            let update_sync_category_obj = {
                'ss_id': objRequest.ss_id - 0 || "",
                'mobileno': objRequest.mobile || "",
                'is_category_sync': ''
            };
            if (is_allow) {
                ...
                let log_file_obj = {
                    'objRequest': sync_contact_add_obj,
                    'responseData': ''
                };
                if (!fs.existsSync(appRoot + '/tmp/log/Sync_Category')) {
                    fs.mkdirSync(appRoot + '/tmp/log/Sync_Category');
                }
                let sync_contact_add_url = 'http://pbnext.policyboss.com/sync-contact/sync-contact-add-data/';
                let sync_contact_add_args = {
                    data: JSON.stringify(sync_contact_add_obj),
                    headers: {
                        'Content-Type': 'application/json',
                        'accept': '*/*'
                    },
                ...
                let sync_contact_api_call_obj = client.post(sync_contact_add_url, sync_contact_add_args, function (sync_contact_add_data, sync_contact_add_res) {
                    try {
                        let today = moment().utcOffset('+05:30');
                        let today_str = moment(today).format('YYYYMMD');
                        log_file_obj['responseData'] = sync_contact_add_data;
                        fs.appendFile(appRoot + '/tmp/log/Sync_Category/' + objRequest.ss_id + '_sync_category_' + today_str + '.log', JSON.stringify(log_file_obj) + '\r\n', function (log_write_err) {
                            if (log_write_err) {
                            }
                        });
                    } catch (e) {
                        console.error('Category Sync_Contact add logs not created', e.stack);
                    }
                    let is_category_synced = 2;
                    if (sync_contact_add_data && sync_contact_add_data.status && sync_contact_add_data.status === 'success') {
                        is_category_synced = 1;
                    }
                    update_sync_category_obj['is_category_sync'] = is_category_synced;
                    try {
                        let update_sync_category_args = {
                            data: update_sync_category_obj,
                            headers: {
                                'Content-Type': 'application/json',
                                'accept': '*/*'
                            }
                        };
                        client.post(config.environment.weburl + '/sync_contact/update_sync_contact_category', update_sync_category_args, (update_sync_contact_data, update_sync_contact_res) => {
                        });
                    } catch (e) {
                        console.error('/sync_contact/add_sync_contact_category', e.stack);
                        objSummary['Data'] = e.stack;
                    }
                    objSummary['Status'] = 'SUCCESS';
                    objSummary['Msg'] = 'Add Sync Category Successfully.';
                    objSummary['Data'] = sync_contact_add_data;
                    return res.json(objSummary);
                });
                sync_contact_api_call_obj.on('requestTimeout', function (req_timeout) {
                    try {
                        let today = moment().utcOffset('+05:30');
                        let today_str = moment(today).format('YYYYMMD');
                        log_file_obj['responseData'] = 'API Request Timeout';
                        fs.appendFile(appRoot + '/tmp/log/Sync_Category/' + objRequest.ss_id + '_sync_category_' + today_str + '.log', JSON.stringify(log_file_obj) + '\r\n', function (log_write_err) {
                            if (log_write_err) {
                            }
                        });
                    ...
                    update_sync_category_obj['is_category_sync'] = 2;
                    try {
                        let update_sync_category_args = {
                            data: update_sync_category_obj,
                            headers: {
                    ...
                    objSummary['Status'] = 'FAIL';
                    objSummary['Msg'] = 'Add API Request timeout';
                    return res.json(objSummary);
                });
                sync_contact_api_call_obj.on('responseTimeout', function (res_timeout) {
                    try {
                        let today = moment().utcOffset('+05:30');
                        let today_str = moment(today).format('YYYYMMD');
                        log_file_obj['responseData'] = 'API Response Timeout';
                        fs.appendFile(appRoot + '/tmp/log/Sync_Category/' + objRequest.ss_id + '_sync_category_' + today_str + '.log', JSON.stringify(log_file_obj) + '\r\n', function (log_write_err) {
                            if (log_write_err) {
                            }
                        });
                    ...
                    objSummary['Status'] = 'FAIL';
                    objSummary['Msg'] = 'Add API Response timeout';
                    return res.json(objSummary);
                });
            } else {
                update_sync_category_obj['is_category_sync'] = 3;
                try {
                    let update_sync_category_args = {
                        data: update_sync_category_obj,
                        headers: {
            ...
            console.error('/sync_contact/add_sync_contact_category', err.stack);
            objSummary['Status'] = 'FAIL';
    ...
    app.post('/sync_contact/update_sync_contact_category', function (req, res) { // Update sync contact category data
        try {
            req.body = JSON.parse(JSON.stringify(req.body));
            let objRequest = req.body;
            let objSummary = {};
            if (objRequest.ss_id && objRequest.mobileno && objRequest.is_category_sync) {
                let fetch_args = {
                    'ss_id': objRequest.ss_id,
                    'mobileno': objRequest.mobileno
                };
                let update_args = {
                    'Is_Category_Synced': objRequest.is_category_sync,
                    'Is_Category_Response_Received': 0,
                    'Category_Synced_On': new Date()
                };
                Sync_Contact.findOneAndUpdate(fetch_args, {$set: update_args}, function (sync_contact_err, catagoryWrapperData) {
                    if (sync_contact_err) {
                        objSummary['Status'] = 'FAIL';
                        objSummary['Msg'] = 'Category Sync_Contact add updation fail';
                        objSummary['Data'] = sync_contact_err;
                        return res.json(objSummary);
                    } else {
                        objSummary['Status'] = 'SUCCESS';
                        objSummary['Msg'] = 'Category Sync_Contact add successfully';
                        return res.json(objSummary);
                    }
                });
            } else {
                objSummary['Status'] = 'FAIL';
                objSummary['Msg'] = 'Sync Category update field missing';
                return res.json(objSummary);
            }
        } catch (e) {
            console.error('/sync_contact/update_sync_contact_category', e.stack);
            objSummary['Status'] = 'FAIL';
            objSummary['Msg'] = 'Exception in update_sync_contact_category';
            objSummary['Data'] = e.stack;
            return res.json(objSummary);
        }
    });
    app.post('/sync_contact/sync_contact_category', function (req, res) {
        let objSummary = {};
        try {
            req.body = JSON.parse(JSON.stringify(req.body));
            let objRequest = req.body;
            ...
            let error_msg = "";
            let is_allow = true;
            if (["", null, undefined].indexOf(objRequest) > -1) {
                arr_msg.push("SYNC RESPONSE :: INVALID :: " + objRequest);
            }
            if (["", null, undefined, 0].indexOf(objRequest.user_id) > -1) {
                arr_msg.push("USER ID :: INVALID :: " + objRequest.user_id);
            }
            if (["", null, undefined].indexOf(objRequest.mobile) > -1) {
                arr_msg.push("MOBILE :: INVALID :: " + objRequest.mobile);
            }
            if (["", null, undefined, 0].indexOf(objRequest.ss_id) > -1) {
                arr_msg.push("SS_ID :: INVALID :: " + objRequest.ss_id);
            }
            if (arr_msg.length > 0) {
                is_allow = false;
                error_msg = arr_msg.join(", ");
            ...
            if (is_allow) {
                let ssid = objRequest.ss_id - 0;
                let mobileNo = (objRequest.mobile - 0).toString();
                let update_args = {
                    "Sync_Category_Response": objRequest,
                    "Is_Category_Response_Received": 1,
                    "Relation": objRequest.relation || "",
                    "Profession": objRequest.profession || "",
                    "Designation": objRequest.designation || "",
                    "Category_Response_Received_On": new Date()
                };
                Sync_Contact.findOneAndUpdate({ss_id: ssid, mobileno: mobileNo}, {$set: update_args}, function (catagoryErr, catagoryData) {
                    if (catagoryErr) {
                        objSummary['Status'] = 'FAIL';
                        objSummary['Msg'] = 'RECORD NOT FOUND';
                        objSummary['Data'] = catagoryErr;
                        res.send(objSummary);
                    } else {
                        if (catagoryData) {
                            objSummary['Status'] = 'SUCCESS';
                            objSummary['Msg'] = 'RECORD FOUND AND UPDATED SUCCESSFULLY';
            ...
            objSummary['Status'] = 'FAIL';
            objSummary['Msg'] = 'EXCEPTION IN /sync_contact/sync_contact_category';
            objSummary['Data'] = err.stack;
            res.send(objSummary);
        }
    });
    
    app.get('/sync_contacts/emp_data_piyush', agent_details_pre, function (req, res) {
        var obj_summary = {
            "Status":"",
            "Msg":"",
            "Data":""
...
//            }
            ...
            objRequest = objRequest_new;
            var url_api = 'http://lerpci.policyboss.com/RBServices.svc/getSyncData?MobileNo=7666020532&POSPID=1076022222&fbaid=54063333';
            if (erp_id === 0 && fba_id === 0) {
                console.error('DBG', 'call_erp_sync', 'ss_id', Ss_Id, 'url', url_api);
            }
            let Client = require('node-rest-client').Client;
            ...
            client.get(url_api, function (data, response) {
                try {
                    ...
                    fs.appendFile(appRoot + "/tmp/log/erp_fetch_new" + today_str + ".log", JSON.stringify(objLog) + "\r\n", function (err) {
                        if (err) {
                            return console.log(err);
                        }
                    });
                    ...
                    if (data) {
                        if (data.hasOwnProperty('getSyncDataResult')) {
                            if (data['getSyncDataResult'] !== '' && data['getSyncDataResult'].length > 0){
                                ...
                                let edata = data['getSyncDataResult'];
                                for (let k in edata){
                                    saveObj['Erp_Qt'] = edata['erp_QT'] || "";
                                    saveObj['Request'] = url_api || "";
                                    saveObj['Response'] = edata || {};
                                    let args = {
                                        data: saveObj,
                                        headers: {
                                    ...
                                    client1.post('http://localhost:3000' + '/sync_contacts/save_prospect_posp_erp', args, function (data, response) {
    ...
    app.post('/sync_contacts/save_prospect_posp_erp', function (req, res) {
        try {
            req.body = JSON.parse(JSON.stringify(req.body));
            let prospect_posp_erp = require('../models/prospect_posp_erp');
            let objRequest = req.body || {};
            let ss_id = (objRequest.Ss_Id && objRequest.Ss_Id - 0) || 0;
            let saveObj = {
                "Ss_Id": ss_id,
                ...
                "Erp_Qt": objRequest.Erp_Qt,
                "Request": objRequest.Request,
                "Response": objRequest.Response
            };
            saveObj['Created_On'] = new Date();
            saveObj['Modified_On'] = new Date();
            let prospect_posp_erp_obj = new prospect_posp_erp(saveObj);
            prospect_posp_erp_obj.save(function (prospect_posp_err, prospect_posp_data) {
                if (prospect_posp_err) {
                    res.json({'Status': 'FAIL', 'Msg': 'Error occurred while saving in prospect_posp_erp collection', 'Data': prospect_posp_err});
                } else {
                    res.json({'Status': 'SUCCESS', 'Msg': 'Data Saved', 'Data': prospect_posp_data});
                }
            });
        } catch (e) {
            console.error('Exception', '/sync_contacts/save_prospect_posp_erp', e.stack);
            return res.json({'Status': 'SUCCESS', 'Msg': 'Exception Occurred', 'Data': ''});
        }
    });
};
...
function agent_details_pre(req, res, next) {
    ...
    if (ss_id > 0) {
        var Client = require('node-rest-client').Client;
        var client = new Client();
        client.get('https://horizon.policyboss.com:5443' + '/posps/dsas/view/' + ss_id.toString(), {}, function (data, response) {
            if (data['status'] === 'SUCCESS') {
                req.agent = data;
            } else {
                //res.send('AGENT_NOT_EXIST<br>' + '<pre>' + JSON.stringify(req.query, undefined, 2) + '</pre>');
