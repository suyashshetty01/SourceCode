...
function get_payment_data(arn, count) {
    ...
    $.get('/TwoWheelerInsurance/Proposal_Details?request_unique_id=' + srn + '&service_log_unique_id=' + arn + '&client_id=' + ClientID + '&customerRefNo=' + crn + '&productId=' + product_id + '', function (res) {
        ...
        if (response_json.Status === 'complete') {
            error_specification = response_json.Error.Error_Specific.toString();
...
function Raise_Ticket() {
    var objdata = {
        "Ticket_Id": "",
        "Product": ProductID = "1" ?"Car" : "Bike",
        "Category": "Proposal",
        "SubCategory" : "Proposal_Auto_Generated_SubCategory",
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            console.log(response);
            $('.ticketAlert').show();
            $('#txt_ticket_res').text(response['Msg']);
        }
    });
}
