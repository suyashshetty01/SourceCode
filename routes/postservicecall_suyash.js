...
//var bitly_access_token = config.environment.arr_bitly_access_token[Math.floor(Math.r zandom() * config.environment.arr_bitly_access_token.length)];
        ...
        kyc_history.findOne(query).sort({ Created_On: -1 }).exec((err, data) => {
            try {
                if (err) {
                    ...
                    if (data && data.hasOwnProperty('_doc') && data['_doc'].hasOwnProperty('KYC_Status') && data['_doc']['KYC_Status'] !== "VERIFY_FAIL" && data['_doc']['KYC_Status'] !== "FETCH_FAIL" && [2, 4].indexOf(ObjRequest.product_id) === -1 && [30].indexOf(ObjRequest.insurer_id) === -1) {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            for (let i in files) {
                if (files[i] !== null && files[i] !== "" && files[i] !== undefined) {
                    ...
                    if (status !== "Fail") {
                        var file_sys_loc_horizon = appRoot + (kyc_doc ? (('/tmp/kyc_documents/' + (crndata + "_" + ObjRequest.document_id + "_" + ObjRequest.insurer_id)).replace(/#| |"|'|`|%20|%22|%27|%2C/g, '')) : ("/tmp/onboarding_docs/" + crndata + "/" + file_name_prefix[document_type_data] + "" + crndata)) + '.' + extension;
...
router.post('/car_info_lead', (req, res) => {
    try {
        ...
        var api_url = "https://policyboss.bitrix24.in/rest/2/fuiywb2evfxvo7b5/crm.deal.add.json" +
            ...
            "&FIELDS[UF_CRM_1693205467749]=https://www.policyboss.com" + (objectRequest.quote_url || "");//Pass quotation link here;
...
router.post('/calculatePayoutCarTw', function (req, res) {
    try {
        ...
        let objRequest = req.body || {};
        const User_Data_Id = objRequest['udid'] - 0;
        let response_obj;
        var crn;
        let payout_model = require('../models/payout');
        let payout_data = new payout_model();
        ...
        payout_model.find({ User_Data_Id: User_Data_Id, Insurer_Id: objRequest['insurer_id'], Status: "Success" }).sort({ _id: -1 }).exec((err, data) => {
            if (err) {
                console.error('payout', err);
            }
            if (data && data.length > 0) {
                let payout_amount = data[0]._doc && data[0]._doc.Point ? Math.round(data[0]['_doc']['Point']) : 0;
                res.json({ "Status": "Success", Payout: payout_amount, Request: data[0]['_doc']['Request'], Response: data[0]['_doc']['Response'] });
            } else {
                request(getOptions, function (err, data) {
                    try {
                        if (data && JSON.parse(data.body).length > 0) {
                            ...
                            crn = data.PB_CRN;
                            var plan = plan_obj[data['Premium_Request']['vehicle_insurance_subtype']];
                            var premium_od = 0;
                            var premium_add_on = 0;
                            var premium_tp = 0;
                            var tariff = 0;
                            ...
                            if (data.Premium_List && data.Premium_List.Response && data.Premium_List.Response.length > 1) {
                                for (var i = 0; i < data.Premium_List.Response.length; i++) {
                                    if (data.Premium_List.Response[i]["Insurer_Id"] == objRequest['insurer_id'] && data.Premium_List.Response[i]["Premium_Breakup"]) {
                                        const premiumBreakup = data.Premium_List.Response[i]["Premium_Breakup"];
                                        const premiumRate = data.Premium_List.Response[i]["Premium_Rate"];
                                        premium_od = premiumBreakup["own_damage"]["od_final_premium"].toFixed(2);
                                        premium_tp = premiumBreakup["liability"]["tp_final_premium"].toFixed(2);
                                        ...
                                        if (premiumRate && premiumRate["own_damage"] && premiumRate["own_damage"]["od_disc"]) {
                                            tariff = premiumRate["own_damage"]["od_disc"].toFixed(2);
                                        }
                                        if (data.Addon_Request && Object.keys(data.Addon_Request.addon_standalone).length > 0) {
                                            for (var h = 0; h < Object.keys(data.Addon_Request.addon_standalone).length; h++) {
                                                const addonKey = Object.keys(data.Addon_Request.addon_standalone)[h];
                            ...
                            var fuel = data['Master_Details']["vehicle"]["Fuel_Name"];
                            var product_sub_type;
                            if (data['Premium_Request']["product_id"] == 1) {
                                product_sub_type = 'Car';
                            } else if (data['Premium_Request']["product_id"] == 10) {
                                product_sub_type = data['Master_Details']["vehicle"]["Vehicle_Segmant"] === "Scooter" ? "Scooter" : "Bike";
                            } else if (data['Premium_Request']["product_id"] == 12) {
                            ...
                            var req_obj = {
                                ...
                                "vehicle_cc": Math.round(data['Master_Details']["vehicle"]["Cubic_Capacity"] - 0), //125,
                                ...
                                "product_name": data['Premium_Request']["product_id"] == 10 ? "Two Wheeler" : "Motor", //"Two Wheeler",
                            ...
                            request(options, function (error, response) {
                                if (error) {
                                    ...
                                    var Msg = JSON.parse(response.body);
                                    if (Msg.status && Msg.status === true && Msg.data && Msg.data.calculated_payout_amount) {
                                        let Payout = Math.round(Msg.data.calculated_payout_amount);
                                        response_obj = { "Status": "Success", Payout: Payout, Request: req_obj, Response: Msg };
                                    } else {
                                        response_obj = { "Status": "Fail", "Msg": Msg, Request: req_obj, Response: Msg };
                                    }
...
//            if (req.body.search_by === 'Make_Model' && req.body.search_byvalue && req.body.search_byvalue !== '') {
//                filter["Make_Model"] = new RegExp(req.body['search_byvalue'], 'i');
//            }
//            if (req.body.search_by === 'Insurer_Id' && req.body.search_byvalue && req.body.search_byvalue !== '') {
//                filter["Insurer_Id"] = req.body['search_byvalue'] - 0;
//            }
//            if (req.body.search_by === 'Product_Name' && req.body.search_byvalue && req.body.search_byvalue !== '') {
//                filter["Product_Name"] = req.body['search_byvalue'] - 0;
//            }
//        }
...
router.get('/getBlogs', function (req, res) {
    try {
        ...
        var content_management = require('../models/content_management');
        content_management.find({ Type: type, Is_Active: 1, Status: 'Publish' }, { _id: 0 }).sort({ Created_On: -1 }).exec(function (err, blogs) {
            if (err) {
...
//                var separator = "";
//                var dynamicSeparator = new RegExp(separator, 'g');
//                var fileNameParts = pdf_file_name.split('.');
//                var extension = fileNameParts.pop();
//                var baseName = fileNameParts.join('.');
//                pdf_file_name = baseName.replace(/ /g, '').replace(dynamicSeparator, '') + "." + extension;
...
router.post("/renewal", recalculate, (req, res) => {
    try {
        var reqObj = req.body;
...
router.post("/call_premium_list_db", (req, res) => {
    try {
                ...
                try {
                    if (premium_initiate_res.hasOwnProperty('Status') && premium_initiate_res['Status'] === "VALIDATION") {
                        ...
                        if (premium_initiate_res && premium_initiate_res.Summary && premium_initiate_res.Summary.Request_Unique_Id) {
                            ...
                            client.post(config.environment.weburl + "/postservicecall/call_premium_list_db", call_premium_list_db_args, (premium_list_db_res) => {
                                if (premium_list_db_res) {
...
//                //                var path = "/var/www/Production/hcdn/website/UI22/images/blog_images/" + fields['category'];//Live
//                var path = appRoot + "/tmp/blog_images/";//local
//                var pdf_sys_loc_horizon = path + '/' + pdf_file_name;
//                var oldpath = files.file_1.path;
//                if (!fs.existsSync(path)) {
//                    fs.mkdirSync(path);
...
//                var pdf_file_name = fileList.name;
//                var separator = "";
//                var dynamicSeparator = new RegExp(separator, 'g');
//                var fileNameParts = pdf_file_name.split('.');
//                var extension = fileNameParts.pop();
...
//                } else if (save_type === "edit_save") {
//                    objData.Content_Id = autoIndex;
//                    if (req.body['old_status_val'] == "Draft" || true) {
//                        var findobj = {
//                            "URL": req.body['old_url_val'],
//                            "Content_Id": parseInt(req.body['ContentId'])
//                        };
...
router.get('/content_management/change_blog_status/:content_id', (req, res) => {
    try {
        ...
        MongoClient.connect(config.db.connection + ':27017/' + config.db.name, function (err, db) {
            ...
            content_management.findOne(Query_obj, function (err, db_data) {
                if (db_data) {
                    if (db_data['Type'] === "BLOG") {

                        var status = "Publish";
                        if (['Publish', 'UnPublish'].indexOf(status) > -1) {
                            res.json({ "Msg": "Expected format is Publish or UnPublish", "Status": "Fail" })
                        }
                        else {
                            if (status === "Publish") {
                                var update = {
                            ...
                            content_management.updateOne(Query_obj, update, function (err, result) {
                                if (err) {
                                    console.error("Error updating the Status");
                                    res.json({ "Msg": "Error updating the Status", "Status": "Fail" });
                                } else {
                                    res.json({ "Msg": result, "Status": "Success" });
                                }
                            })
                        }
                    }
                    ...
                    res.json({ "Msg": "Content id not found", "Status": "Fail" });
                }
            })
        })
    } catch (e) {
 ...
 router.get('/content_management/change_blog_status/:content_id', (req, res) => {
     try {
         ...
         if (['Publish', 'UnPublish'].indexOf(status) > -1) {
             ...
             content_management.findOneAndUpdate(Query_obj,{ $set: update_Obj }, function (err, db_data) {
                     if (err) {
                         console.error("Error updating the Status");
                         res.json({ "Msg": "Error updating the Status", "Status": "FAIL" });
                     } else {
             ...
             res.json({"Msg":"Format should be Publish or UnPublish","Status":"FAIL"})
         }
     }
     catch (e) {
 ...
 router.get('/onboarding/remove_all_documents',function(req,res){
    try{
        let posp_user = require('../models/posp_users.js');
        ...
        let docs = ["PAN","AADHAAR","AADHAAR_BACK","POSP_ACC_DOC","QUALIFICATION","PROFILE"];
        ...
        let document_types = docs;
        let current_doc_fields = fields ; 
        let posp_log_args = {
            "Verified_By_API": "",
            "Status": "",
            "Doc_URL": "",
...
router.post('/posp/posp_users', LoadSession, function (req, res) {
    try {
        var posp_user=require('../models/posp_users')
        var objBase = new Base();
        var obj_pagination = objBase.jqdt_paginate_process(req.body);
        var optionPaginate = {
            select: '',
        ...
        if (req && req.body && req.body['search'] && req.body['search']['value'] !== '') {
            ...
            if ((req.obj_session.user.role_detail.role.indexOf('SuperAdmin') > -1) || req.obj_session.user.ss_id == 7973 || req.obj_session.user.ss_id == 7644) {//added Ronald and Sandeep ssid  to show all data
...
router.post('/get_posp_enquiry_field_visitors', function (req, res) {
    try {
        var Base = require('../libs/Base');
        var objBase = new Base();
        var optionPaginate = {
        ...
        var posp_enquiry_field_visitor = require('../models/posp_enquiry_field_visitor.js');
        posp_enquiry_field_visitor.paginate({}, optionPaginate).then(function (user_datas) {
            res.json(user_datas);
        });
    }
    catch (e) {
...
router.post('/employees/update_employee_details', function (req, res) {
    try {
        let users = require('../models/user');
        let hr_employee = require('../models/hr_employee');
        let user_find_query = {
            'Ss_Id': (req.body.ss_id && req.body.ss_id - 0) || 0,
            'UID': (req.body.uid && req.body.uid - 0) || 0
        };
        let hr_employee_find_query = {
            'uid': (req.body.uid && req.body.uid - 0) || 0
        };
        let user_update_obj = {
        ...
        let hr_employee_update_obj = {
            'full_name': req.body.name || '',
            'contact_nos': req.body.phone_no || '',
            'personal_email': req.body.email || ''
        };
        users.findOneAndUpdate(user_find_query, {$set: user_update_obj}, {new : true, select: "UID Ss_Id Employee_Name Email Phone"}, function (db_user_err, db_user_data) {
            try {
                if (db_user_err) {
                    res.json(db_user_err);
                } else {
                    //res.json(db_user_data);
                    hr_employee.findOneAndUpdate(hr_employee_find_query, {$set: hr_employee_update_obj}, {new : true, select: "uid full_name personal_email contact_nos"}, function (db_hr_employee_err, db_hr_employee_data) {
                        try {
                            if (db_hr_employee_err) {
                                res.json(db_user_err);
                            } else {
                                res.json({'Status': 'SUCCESS', 'Msg': 'DATA UPDATED SUCCESSFULLY', 'User': db_user_data, 'HR_Employee': db_hr_employee_data});
                            }
                        } catch (ex) {
                            console.error('Exception in -/employees/update_employee_details', ex.stack);
                            res.json({'Status': 'FAIL', 'Msg': 'EXCEPTION OCCURS AFTER UPDATING HR_EMPLOYEE DETAILS', data: ex.stack});
                        }
                    });
                }
            } catch (ex) {
                console.error('Exception in -/employees/update_employee_details', ex.stack);
                res.json({'Status': 'FAIL', 'Msg': 'EXCEPTION OCCURS AFTER UPDATING EMPLOYEE DETAILS', data: ex.stack});
            }
        });
    } catch (ex) {
        console.error('Exception in -/employees/update_employee_details', ex.stack);
        res.json({'Status': 'FAIL', 'Msg': 'EXCEPTION IN UPDATING EMPLOYEE DETAILS', data: ex.stack});
    }
});
        ...
        agent_coin.find(agent_coin_query, function (db_agent_coin_err, db_agent_coin_data) {
            try {
                if (db_agent_coin_err) {
                    res.json(db_agent_coin_err);
                } else {
                    console.error('db_agent_coin_data - ', db_agent_coin_data);
...
router.get('/sync_contacts/update_lead_count', function (req, res) {
    try {
        const agent_coin = require('../models/agent_coin');
        const objRequest = req.query || {};
        let agent_coin_query = {
            'Ss_Id': (objRequest.ss_id && objRequest.ss_id - 0) || 0
        };
        let agent_coin_lead_update = {
            'Total_Lead_Count': (objRequest.total_lead_count && objRequest.total_lead_count - 0) || 0,
            'Lead_Buy_Count': (objRequest.lead_buy_count && objRequest.lead_buy_count - 0) || 0
        };
        console.log('objRequest',objRequest)
        agent_coin.update(agent_coin_query, {$set: agent_coin_lead_update}, function (update_lead_count_err, update_lead_count_data) {
            if (update_lead_count_err) {
                res.json(update_lead_count_err);
            } else {
                if (update_lead_count_data && update_lead_count_data.nModified === 1) {
                    res.json({'Status': 'SUCCESSS', 'Msg': 'LEAD COUNTS UPDATED SUCCESSFULLY',Data:update_lead_count_data});
                } else {
                    res.json({'Status': 'SUCCESSS', 'Msg': 'LEAD COUNTS ALREADY UPDATED',Data:update_lead_count_data});
                }
            }
        });
    } catch (ex) {
        console.error('Exception in -/update_lead_count', ex.stack);
        res.json({'Status': 'FAIL', 'Msg': 'EXCEPTION WHILE UPDATING LEAD COUNTS', data: ex.stack});
    }
});
                            ...
                            if (update_buy_lead_data && update_buy_lead_data.nModified === 1) {
                                ...
                                agent_coin_history_obj.save(function (agent_coin_history_err, agent_coin_history_data) {
                                    if (agent_coin_history_err) {
                                        ...
                                        client.get(config.environment.weburl + '/razorpay_payment/update_coins_transaction_status/' + transaction_id+'/'+'Success', {}, function (data, response) {
                                            if(data && data.Status === 'Success'){
                                                res.json(objSummary);
                                            }else{
                                                objSummary["Msg"] = "BUY LEAD COUNTS UPDATED SUCCESSFULLY BUT NOT UPDATED IN RAZORPAY";
                                                res.json(objSummary);
                                            }
                                        });
                                    }
...
////            
////            var databaseColumnIndex = "Category";
...
//        content_management.paginate(filter, optionPaginate).then(function (dbTicket) {
//           
////               dbTicket.docs.sort((a,b)=>{
////                   return a.Title.localeCompare(b.Title) * sortOrder;
////               });
//           
//            res.json({'DATA':dbTicket,'COLUMN_NO':orderColumnIndex,'COLUMN_ORDER':orderDirection});
////            res.json(dbTicket);
//        });
//        
//        
...
//            var orderColumnIndex = req.body.order[0].column;
//            var orderDirection = req.body.order[0].dir;
//            var columnName=['Action','SS_ID','Type','Category','Title','URL','Keywords','Description','Status','Author','Source','Image_Url'];
////            var databaseColumnIndex = "Status";
...
//            
////            if(column==='URL'){
////                optionPaginate.sort = getCustomSortAggregation(orderDirection);
////            }
////            else{
//            optionPaginate.sort[databaseColumnIndex] = (orderDirection === 'asc') ? 1 : -1;
////        }
...
//        var filter = obj_pagination.filter;
//        var ss_id = parseInt(objRequest["ss_id"]);
//        if ([8067].indexOf(ss_id) > -1) {/** for santosh sir marketing team */
//            filter['$or'] = [
//                {'Status': 'Publish'},
//                {'Status': 'UnPublish'}
//            ];
...
//                    
//                    for (var k in erpqt_data) {
//                        qtdata[k] = data['___' + k + '___'];
//                    }
//                    qt_data["policy_number"] = getData[0]['_doc']['Transaction_Data']  ? getData[0]['_doc']['Transaction_Data']["policy_number"] : "";
//                    res.json(qtdata);
//                } else {
//                    res.json({'msg': 'No data found'});
//                }
...
//                    let data = {
//                        "___salutation___": "MR",
//                        "___contact_name___": "PAWAN KUMAR KANOAGIA",
//                        "___first_name___": "PAWAN",
//                        "___middle_name___": "KUMAR",
//                        "___last_name___": "KANOAGIA",
//                        "___birth_date___": "1997-05-20",
//                        "___email___": "pkanoagia@gmail.com",
//                        "___mobile___": "9569395291",
//                        "___mobile_update___": "9569395291",
//                        "___permanent_address_1___": "GRAM - GUDBAD",
//                        "___permanent_address_2___": "228142-SULTANPUR",
//                        "___permanent_address_3___": "",
//                        "___is_reg_addr_comm_addr_same___": "yes",
//                        "___communication_address_1___": "GRAM - GUDBAD",
//                        "___communication_address_2___": "228142-SULTANPUR",
//                        "___communication_address_3___": "",
//                        "___nominee_name___": "XYZ MEMBER",
//                        "___nominee_relation___": "Son",
//                        "___nominee_birth_date___": "2002-01-12",
//                        "___registration_no___": "MH-02-CZ-5258",
//                        "___registration_no_1___": "MH",
//                        "___registration_no_2___": "02",
//                        "___registration_no_3___": "CZ",
//                        "___registration_no_4___": "5258",
//                        "___engine_number___": "L15A74129869",
//                        "___chassis_number___": "MAKGM252DDN404049",
//                        "___is_financed___": "no",
//                        "___financial_agreement_type___": "",
//                        "___financial_institute_name___": "",
//                        "___financial_institute_code___": "",
//                        "___financial_institute_city___": "SULTANPUR"
//                    }
//                    var erpqt_data = {
//                        "salutation": "",
//                        "contact_name": "",
...
router.get('/digitResp', function (req, res) {
    var reqObj = {
        "person": [
            {
                "communications": [
                    {
                        ...
                        "communicationId": "manaskumardas6@gmail.com"
                    },
                    {
                        "isPrefferedCommunication": true,
                        "communicationType": "MOBILE",
                        "communicationId": "8697475313"
                    }
                ]
            }
        ]
    }
    var digit_url_api = 'https://preprod-digitpolicyissuance.godigit.com/endo/v1/g91/endorsement/D600261182?source=CD';
//    var pdf_request = userDataRes['Pdf_Request'];
    var args = {
        data: reqObj,
        headers: {
    ...
    var Client = require('node-rest-client').Client;
    var client = new Client();
    client.post(digit_url_api, args, function (data, response) {
        if (data) {
...
router.get('/test_service', function (req, res) {
    res.json({'Status' : 'Success'});
});
...
router.post('/fetch_campaign_leads_data', LoadSession, function (req, res, next) {     //Piyush Service
    try {
        var Base = require('../libs/Base');
        var arr_ssid=[];
        var objBase = new Base();
        ...
        if (req.obj_session.user.role_detail.role.indexOf('SuperAdmin') > -1 || [10859].indexOf(req.obj_session.user.ss_id) > -1) {/* santosh singh ssid /
             ...
             */
            arr_ssid.push(req.obj_session.user.ss_id);
//            filter['Lead_Assigned_Ss_Id'] = {$in: arr_ssid}; // Need to uncomment this
        }
        ...
        if(objRequest.Disposition_Status && objRequest.Sub_Status){
            filter['Disposition_Status'] = objRequest.Disposition_Status;
...
//            filter['Disposition_Status'] = {$in:dispositionStatus};
//        }
//        if(objRequest.Disposition_Status === "DISPOSED"){
//            filter['Disposition_Status'] = {$nin:dispositionStatus};
//        }
//        if(objRequest.Disposition_Status && ["PENDING","DISPOSED"].indexOf(objRequest.Disposition_Status) === -1){
//            filter['Disposition_Status'] = objRequest.Disposition_Status;
//        }
...
router.get('/get_campaign_leads_disposition_data/:id', function (req, res) {
    try {
        ...
        var user_disposition = require('../models/posp_disposition');
        user_disposition.find({"Disposition_Id": id, Disposition_Source: Disposition_Source}, function (dbuser_disposition_err, dbuser_disposition) {
            if (dbuser_disposition_err) {
                res.json(dbuser_disposition_err);
            } else {
...
router.get('/fetch_all/campaign_leads_disposition_data', function (req, res) {
    try {
        var user_disposition = require('../models/posp_disposition');
        var Disposition_Source=(req.query.Disposition_Source!=="undefined")?req.query.Disposition_Source : "";
        let agg = [
            {"$group": {
                    "_id": {"Disposition_Id": "$Disposition_Id", "Disposition_Source": Disposition_Source},
                    "Status": {"$last": "$Status"},
...
// router.post('/customer_tracking',(req,res)=>{
//     var customer_tracking = require('../models/customer_tracking');
//     var ObjReq=req.body;
//     // var customer_tracking_data = {
//     // 'User_Id': ObjReq.User_Id,
//     // 'Product_Id': ObjReq.Product_Id,
//     // 'Language': ObjReq.Language,
//     // 'Type_Of_Content':ObjReq.Type_Of_Content,
//     // 'Request_Core':ObjReq.Request_Core,
//     // 'Created_On': new Date(),
//     // 'Modified_On': new Date(),
//     // };
//     // console.log(JSON.stringify(customer_tracking_data));
//     let customer_tracking_log = new customer_tracking(ObjReq);
//     customer_tracking_log.save(function (err, res1) {
//         res.json({ "Msg": "Record Created Successfully.", "Status": "Success" });
//     })
// })
...
//    var ObjReq = req.body;
//    try{
//// {"app_version":"policyboss-1.4.0.2","product_id":"2","device_code":"e73cb6c8dd4be83c","fbaid":"95403","ssid":"124970"}
//    var customer_tracking_obj=
//        {
//            "User_Id":ObjReq.ssid ||"",
//            "Product_Id":ObjReq.product_id || "", 
//            "Language":ObjReq.language || "",
...
//        };
//    
//    var customer_tracking_log = new customer_tracking(customer_tracking_obj);
//    
//    customer_tracking_log.save(function (err, savedRecord) {
//        
//        if (err) { 
//            console.log("Error saving customer tracking data:", err);
//            res.json({ "Msg": "Failed to save record.", "Status": "Fail" });
//        } else {     
//            res.json({ "Msg": "Record Created Successfully.", "Status": "Success" });
//        }
//    });
//}
//catch(er){
//    console.error("Error saving customer tracking data:", er);
//    res.json({ "Msg": er.stack, "Status": "Error" });
//}
//});

router.post('/customer_tracking', (req, res) => {
    let customer_tracking = require('../models/customer_tracking');
    let ObjReq = req.body;
    try {
        let customer_tracking_obj = {
            "User_Id": ObjReq.ssid || "",
            "Product_Id": ObjReq.product_id || "",
            "Language": ObjReq.language || "",
            "Type_Of_Content": ObjReq.type_of_content || "",
        ...
        let Customer_Tracking_Log = new customer_tracking(customer_tracking_obj);
        Customer_Tracking_Log.save(function (err, customer_tracking_data) {
            if (err) {
                res.json({"Status": "FAIL", "Msg": "Failed to save record", "Data": err});
            } else {
                res.json({"Status": "SUCCESS", "Msg": "Record Created Successfully", "Data": customer_tracking_data});
            }
        });
    } catch (e) {
        res.json({"Status": "ERROR", "Msg": "Exception In /customer_tracking", "Data": e.stack});
    }
});

//var const_static_page_UI22 = JSON.parse(fs.readFileSync(config.environment.horizon_app_path + "/tmp/cms_urls.json", { encoding: 'utf8', flag: 'r' }))["routes"];
//
//router.get(const_static_page_UI22, function (req, res) {
//    console.log('working');
//    var Client = require('node-rest-client').Client;
//    var client = new Client();
//    //let content_URL = req.path.replace(/\//g,'_');
//    let content_URL = req.path.substring(1);
//    //res.locals.dynamic_url =  req.fullUrl;
//    if(content_URL.indexOf('UI22') > -1){
//            
//    }
//    else{
//            content_URL = 'UI22/'+ content_URL;
//    }
//    let Ui_Version = 'UI22';
//    client.get(config.environment.weburl + '/content_managements/get_content?content_URL=' + content_URL, function (data) {
//            if (data) {
//                    try{
//                        if(config.environment.name === "Production"){
//                            res.locals.title = data.Title;
//                            res.locals.keyword = data.Keywords;
//                            res.locals.description = data.Description;
//                            res.locals.date=moment(data.Blog_Date).format('DD MMM, YYYY');
//                            res.locals.source=date.Source;
//                            //res.locals.dynamic_url = content_URL;
//                            let content_URL_ver = content_URL.replace(/\//g,'_') +'_ver_'+data.Version;
//                            
//                            if(fs.existsSync('/var/www/Production/Admin/views/website/cms22/'+content_URL_ver+'.ejs') === false){
//                                    let cms_content = data.Content;
//                                    cms_content = cms_content.replace(/src\=\"images\//g,'src="https://origin-cdnh.policyboss.com/website/UI22/images/');
//                                    cms_content += "<!-- created_on : "+ moment(data.Created_On).utcOffset("+05:30").format('YYYY-MM-DD HH:mm:ss') +" ,cached_on : "+ moment().utcOffset("+05:30").format('YYYY-MM-DD HH:mm:ss') +" , cached_version : "+data.Version+" -->";
//                                    fs.writeFile('/var/www/Production/Admin/views/website/cms22/'+content_URL_ver+'.ejs', cms_content, function (err) {
//                                            if (err) {
//                                                    return console.log(err);
//                                            }                                        
//                                            
//                                            res.render('website/cms22/'+content_URL_ver, {layout: 'UI22/website'});
//                                                                                            
//                                    });
//                            }
//                            else{
...
//                    }else{
//                    let content_URL_ver = content_URL.replace(/\//g, '_') + '_ver_' + data.Version;
//                    if (data && data.Type && data.Type === 'BLOG') {
//                        if (fs.existsSync(appRoot + '/Admin/views/website/cms22/' + content_URL_ver + '.ejs') === false) {
//                            let cms_content = data.Content;
//                            cms_content = cms_content.replace(/src\=\"images\//g, 'src="https://origin-cdnh.policyboss.com/website/UI22/images/');
//                            cms_content += "<!-- created_on : " + moment(data.Created_On).utcOffset("+05:30").format('YYYY-MM-DD HH:mm:ss') + " ,cached_on : " + moment().utcOffset("+05:30").format('YYYY-MM-DD HH:mm:ss') + " , cached_version : " + data.Version + " -->";
//                            fs.writeFile(appRoot + '/Admin/views/website/cms22/' + content_URL_ver + '.ejs', cms_content, function (err) {
//                                if (err) {
//                                    return console.log(err);
//                                }
//
//                                res.render('website/cms22/' + content_URL_ver, {layout: false});
//
//                            });
//                        } else {
//                            res.render('website/cms22/' + content_URL_ver, {layout: false});
...
//                                }
//
//                                res.render('website/cms22/' + content_URL_ver, {layout: 'website/product/UI22/news-landing-detail'});
//
//                            });
//                        } else {
//                            res.render('website/cms22/' + content_URL_ver, {layout: 'website/product/UI22/news-landing-detail'});
...
//    let kycInfo = require('../models/kyc_history');
//    kycInfo.find
//})
//router.get('/getErrorReport', (req, res) => {
//    
//    // Find documents with KYC_Status = "FETCH_FAIL"
//    var data=kyc_history.find({ KYC_Status: "FETCH_FAIL" })
//     
//          res.send({'Msg':'SUCCESS','Data':data});
      

      // Count error codes for each insurer
//      const errorCounts = {};
//      documents.forEach(record => {
//        const insurerId = record.Insurer_Id;
...
//          errorCounts[insurerId][errorCode] = 0;
//        }
//
//        errorCounts[insurerId][errorCode]++;
//      });
//
//      client.close();
//      res.json({ records: documents, errorCounts: errorCounts });
//    });
...
router.get('/getErrorReport', (req, res) => {
    // Find documents with KYC_Status = "FETCH_FAIL"
    kyc_history.find({ KYC_Status: "FETCH_FAIL" }).exec((err, data) => {
        if (err) {
        ...
        data.forEach(record => {
            const insurerId = record._doc.Insurer_Id ? record._doc.Insurer_Id : "";
            const errorCode = record._doc.Error_Code ? record._doc.Error_Code : "";
            const errorMsg = record._doc.Error_Msg ? record._doc.Error_Msg.trim() : ""; // Trim the error message
            if(errorCode!==""){
            if (!errorCounts[insurerId]) {
                errorCounts[insurerId] = {};
            }
            ...
            if (!errorCounts[insurerId][errorCode]) {
                errorCounts[insurerId][errorCode] = { count: 0, errorMsg: [] }; // Initialize count and errorMsg array
            }

            // Increment count for the error code
            errorCounts[insurerId][errorCode].count++;

            // Add the error message to the errorMsg array if it's not empty
            if (errorMsg !== "") {
...
router.post('/sync_contact_total_count', function (req, res) {
    let objSummary = {};
    try {
        req.body = JSON.parse(JSON.stringify(req.body));
        let objRequest = req.body;
        ...
        objSummary['Status'] = 'FAIL';
        objSummary['Msg'] = 'Exception in sync_contact_total_count service';
...
router.post('/sync_contact_dashboard_category', function (req, res) {
    ...
    try {
        ...
        let query = { [key] :objRequest[key] - 0 , $or: [{Is_Category_Synced: {$exists: false}}, {Is_Category_Synced: 0}], mobileno: {$in: objRequest['mobileNo']} };
        Sync_Contact.find(query).exec(function (err, sync_cont_data) {
            if (err) {
                objSummary['Status'] = 'FAIL';
                objSummary['Msg'] = 'Data not found';
        ...
        if(objRequest && objRequest.user_id && objRequest.mobile) {
            ...
            client.post(url, args, function (data, resp) {
                if (data && data.hasOwnProperty('data') && data.data.includes('success')) {
                    ...
                    let query = { [key]: objRequest[key] - 0, mobileno: objRequest.mobile };
                    Sync_Contact.findOneAndUpdate(query, { $set: { "Is_Category_Synced": 1 } }, function (err, catagoryWrapperData) {
                        if (err) {
                            objSummary['Status'] = 'FAIL';
