...
module.exports.controller = function (app) {
    ...
    app.get('/sms/campaign', function (req, res, next) {
        var Sms_Campaign = require('../models/sms_campaign');
        let cond = {Is_Sms_Sent: {$exists: false}};
        let camp = (req.query.hasOwnProperty('camp')) ? req.query['camp'] : '';
        let is_test = (req.query.hasOwnProperty('is_test')) ? req.query['is_test'] : '';
        let limit = (req.query.hasOwnProperty('limit')) ? req.query['limit'] - 0 : 5;
        if (is_test === 'yes') {
            cond = {'Is_Test': 'yes'};
        }
        Sms_Campaign.find(cond).limit(limit).exec(function (err, dbSms_Campaigns) {
            let SmsLog = require('../models/sms_log');
            let objSmsLog = new SmsLog();
            let Msg = 'Worried about Low Insurance Coverage? Now double your health insurance cover for as little as Rs. 1000/yr and secure your family\'s health and wellness better. To Avail, Give Missed Call on 8506885068 . T&CA.PolicyBoss';

            for (let k in dbSms_Campaigns) {
                let ind_camp = dbSms_Campaigns[k]._doc;
                let Obj_Sms_Campaign = {
                    'Is_Sms_Sent': 1,
                    'Sms_Sent_On': new Date()
                };
                let Customer_Mobile = ind_camp['Customer_Mobile'];
                Sms_Campaign.update({'_id': dbSms_Campaigns[k]._id}, {$set: Obj_Sms_Campaign}, function (err, numAffected) {
                    if ((numAffected.nModified - 0) > 0) {
                        objSmsLog.send_sms(Customer_Mobile.toString(), Msg, camp);
                    }
                });
            }
        });
        return res.json({'status': 'success', 'details': 'Message Received'});
    });
...
Help Finish";
                ...
                objModelSms.save(function (err, objDbSms) {
                    ...
                    if (Request_Type === 'GOLD') {
                        var Sms_Campaign = require('../models/sms_campaign');
                        Sms_Campaign.find({'Customer_Mobile': objDbSms.Sender}).sort({'Sms_Sent_On': -1}).limit(1).exec(function (err, dbSms_Campaign) {
                            if (dbSms_Campaign && dbSms_Campaign._doc['User_Data_Id'] > 0) {
                                var User_Data = require('../models/user_data');
                                User_Data.findOne({'User_Data_Id': dbSms_Campaign._doc['User_Data_Id']}, function (err, dbUserData) {
                                    if (dbUserData) {
                                        dbUserData = dbUserData._doc;
                                        let Visited_History = dbSms_Campaign._doc.hasOwnProperty('Visited_History') ? dbSms_Campaign._doc.Visited_History : [];
                                        Visited_History.unshift(new Date());
                                        let Visited_Count = dbSms_Campaign._doc.hasOwnProperty('Visited_Count') ? dbSms_Campaign._doc.Visited_Count : 0;
                                        Visited_Count = Visited_Count + 1;
                                        let Obj_Sms_Campaign = {
                                            'City': dbUserData['Erp_Qt_Request_Core']['___permanent_city___'],
                                            'State': dbUserData['Erp_Qt_Request_Core']['___permanent_state___'],
                                            'Visited_History': Visited_History,
                                            'Visited_Count': Visited_Count,
                                            'Visited_On': new Date()
                                        };
                                        Sms_Campaign.update({'_id': dbSms_Campaign._id}, {$set: Obj_Sms_Campaign}, function (err, numAffected) {
                                        });
                                    }
                                });
                            } else {
                                ...
                                Visited_History.unshift(new Date());
                                let Obj_Sms_Campaign = {
                                    'Customer_Mobile': objDbSms.Sender,
                                    'Is_Sms_Sent': 0,
                                    'Visited_History': Visited_History,
                                    'Visited_Count': 1,
                                    'Visited_On': new Date()
                                };
                                var objModel_Sms_Campaign = new Sms_Campaign(Obj_Sms_Campaign);
                                objModel_Sms_Campaign.save(function (err, numAffected) {
                                });
                            }
                        });
                        return res.json({'status': 'success', 'details': 'Message Received'});
