$(document).ready(function () {
    ...
    for (var i = 1; i <= 4; i++) {
        ...
        $(_kid).datepicker({
            ...
            changeYear: true,
            yearRange: 'c-23:c',
            minDate: new Date((new Date()).getFullYear() - 25, (new Date()).getMonth(), 1),
            maxDate: '-25d',
            dateFormat: 'dd-mm-yy'
        });
    }
    ...
    function FillPriority(_policyFor) {
        ...
        $.get('/HealthInsuranceIndia/GetInsurancePriority?policyfor=' + _policyFor, function (response) {
            var Priority = $.evalJSON(response);
...
function ValidateForm() {
    ...
    var _FatherSelected = $("#FatherSelect").is(":checked");
    var _MotherSelected = $("#MotherSelect").is(":checked");
    ...
    var _PolicyFor = $("input[name=PolicyFor]:checked").val();
    ...
    if (_selected == 0) {
        _error += "- Please select InsurancePriority .\n";
    }
    ...
    if (_City == "") {
        ...
        //if (!(_SelfSelected || _SpouseSelected || _FatherSelected || _MotherSelected)) {
    ...
    if (_PolicyFor != 'Parents') {
        ...
        if (!(_FatherSelected || _MotherSelected)) {
            _error += "- Please select minimum 1 adult insured.\n";
        }
        else
        {
            if (_FatherSelected) {
                if (_FatherDOB == "") {
                    _error += "- Please select father dob.\n";
                }
            }
            if (_MotherSelected) {
                if (_MotherDOB == "") {
                    _error += "- Please select mother dob.\n";
                }
            }
        }
    }
