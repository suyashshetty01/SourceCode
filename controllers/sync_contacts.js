                    ...
                    Sync_Contact.find({ss_id: Ss_Id, fba_id: Fba_Id}, function (err, dbSync_Contacts) {
                        ...
                        if (dbSync_Contacts.length > 0) {
                            ...
                            for (let x in dbSync_Contacts) {
                                ...
                                if (dbSync_Contacts[x]._doc.hasOwnProperty('sync_date')) {
                                    sync_lead_matched_count++;
                                }
                            }
                            sync_contact_erp_data.find({ss_id: Ss_Id, fba_id: Fba_Id, 'product': 'MOTOR', 'Is_Valid': 1}).sort({'policy_expiry_date': 1}).exec(function (err, dbSync_contact_erp_datas) {
                                ...
                                Lead.find({ss_id: Ss_Id, lead_type: 'sync_contacts'}, function (lead_err, db_lead_data) {
                                    ...
                                    if (lead_err) {
                                        ...
                                        contact_res['Sync_Lead_Matched_Count'] = sync_lead_matched_count;
                                        contact_res['Sync_Pending_Match_Lead'] = (contact_res['Total_Contact_Count'] - contact_res['Sync_Lead_Matched_Count']);
...
//			      let url_api = "http://horizon.policyboss.com:5000/sync_contacts/calculate_relationship_score/"+ss_id+"/"+mobile+"/"+update_data;
    ...
    app.get('/sync_contacts/get_sync_prospect_posp_erp_data', function (req, res) {
        try {
            let obj_summary = {
                "Status": "",
                "Msg": "",
            ...
            let mobileno = req.query['mobile'] || ""; //Erp_Id //Fba_Id //Ss_Id
            let ss_id = req.query['ss_id'] || 0;
            let fba_id = req.query['fba_id'] || 0;
            let erp_id = req.query['erp_id'] || 0;
            if (erp_id && fba_id && mobileno) {
//                var url_api = 'http://lerpci.policyboss.com/RBServices.svc/getSyncData?MobileNo=' + mobileno + '&POSPID=' + erp_id + '&fbaid=' + fba_id;
                ...
                client.get(url_api, function (data, response) {
                    try {
                        ...
                        fs.appendFile(appRoot + "/tmp/log/prospect_posp_erp_fetch_" + today_str + ".log", JSON.stringify(objLog) + "\r\n", function (err) {
                            if (err) {
                                return console.log(err);
                            }
                        });
                        ...
                        if (data) {
                            if (data.hasOwnProperty('getSyncDataResult')) {
                                if (data['getSyncDataResult'] !== '' && data['getSyncDataResult'].length > 0) {
                                    obj_summary['Data']['Total_Count'] = data['getSyncDataResult'].length;
                                    let edata = data['getSyncDataResult'];
                                    for (let k in edata) {
                                        saveObj['Erp_Qt'] = edata[k]['erp_QT'] || "";
                                        saveObj['YOM'] = edata[k]['YOM'] || "";
                                        saveObj['SubModel'] = edata[k]['SubModel'] || "";
                                        saveObj['RegistrationNo'] = edata[k]['RegistrationNo'] || "";
                                        saveObj['RegistrationDate'] = edata[k]['RegistrationDate'] || "";
                                        saveObj['RTO_State'] = edata[k]['RTO_State'] || "";
                                        saveObj['RTO_City'] = edata[k]['RTO_City'] || "";
                                        saveObj['ProductName'] = edata[k]['ProductName'] || "";
                                        saveObj['Product_Id'] = ((edata[k]['ProductName'] && productList[edata[k]['ProductName']]) || 0);
                                        saveObj['Phone'] = edata[k]['Phone'] || "";
                                        saveObj['Model'] = edata[k]['Model'] || "";
                                        saveObj['ManufacturingDate'] = edata[k]['ManufacturingDate'] || "";
                                        saveObj['Make'] = edata[k]['Make'] || "";
                                        saveObj['InsuranceType'] = edata[k]['InsuranceType'] || "";
                                        saveObj['ExpiryDate'] = edata[k]['ExpiryDate'] || "";
                                        saveObj['ClientName'] = edata[k]['ClientName'] || "";
                                        saveObj['City'] = edata[k]['City'] || "";
                                        var args = {
                                            data: saveObj,
                                            headers: {
                                                "Content-Type": "application/json"
                                        ...
                                        var prospect_posp_erp_save_url = config.environment.weburl + '/sync_contacts/save_prospect_posp_erp_data';
                                        var Client = require('node-rest-client').Client;
                                        var client = new Client();
                                        client.post(prospect_posp_erp_save_url, args, function (data, response) {
                                            if (data.Status === 'SUCCESS') {
                                                obj_summary['Data']['Total_Success_Count']++;
                                            } else {
                                                obj_summary['Data']['Total_Fail_Count']++;
                                            }
                                            if (obj_summary['Data']['Total_Count'] === obj_summary['Data']['Total_Success_Count'] + obj_summary['Data']['Total_Fail_Count']) {
                                                obj_summary['Status'] = 'Success';
                                                obj_summary['Msg'] = data.Msg;
                                                res.json(obj_summary);
                                            }
                                        });
                                    }
                                } else {
                                    obj_summary['Status'] = 'FAIL';
                                    obj_summary['Msg'] = 'NoDetails';
                                    res.json(obj_summary);
                                }
                            } else {
                                obj_summary['Status'] = 'FAIL';
                                obj_summary['Msg'] = 'Data_Node_Missing';
                                res.json(obj_summary);
                            }
                        } else {
                            obj_summary['Status'] = 'FAIL';
                            obj_summary['Msg'] = 'Data_Null';
                            res.json(obj_summary);
                        }
                    } catch (ex) {
                        console.error('Exception', 'get_sync_prospect_posp_erp_data', ex.stack);
                        obj_summary['Status'] = 'FAIL';
                        obj_summary['Msg'] = 'Exception occured';
                        obj_summary['Data'] = ex.stack;
                        res.json(obj_summary);
                    }
                });
            } else {
                obj_summary['Status'] = 'FAIL';
                obj_summary['Msg'] = 'Fba_Id or erp_Id or mobile number is missing';
                res.json(obj_summary);
            }
        } catch (ex) {
            console.error('Exception', 'get_sync_prospect_posp_erp_data', ex.stack);
...
//    app.post('/sync_contacts/save_prospect_posp_erp_data', function (req, res) {
//        try {
//            req.body = JSON.parse(JSON.stringify(req.body));
...
//                 "YOM": objRequest.Erp_Qt,
//                 "SubModel": objRequest.Erp_Qt,
//                 "RegistrationNo": objRequest.Erp_Qt,
//                 "RegistrationDate": objRequest.Erp_Qt,
//                 "RTO_State": objRequest.Erp_Qt,
//                 "RTO_City": objRequest.Erp_Qt,
//                 "ProductName": objRequest.Erp_Qt,
//                 "Phone": objRequest.Erp_Qt,
//                 "Model": objRequest.Erp_Qt,
//                 "ManufacturingDate": objRequest.Erp_Qt,
//                 "Make": objRequest.Erp_Qt,
//                 "InsuranceType": objRequest.Erp_Qt,
//                 "ExpiryDate": objRequest.Erp_Qt,
//                 "ClientName": objRequest.Erp_Qt,
//                 "City": objRequest.Erp_Qt,
//                 "Created_On": objRequest.Erp_Qt,
//                 "Modified_On": objRequest.Erp_Qt
//                 };
//                 */
//                for (const key in objRequest) {
//                    //if(['Ss_Id','Fba_Id','Erp_Id','Product_Id'])
//                    saveObj[key] = objRequest[key] || "";
//                }
//                saveObj['Created_On'] = new Date();
//                saveObj['Modified_On'] = new Date();
//                let prospect_posp_erp_obj = new prospect_posp_erp(saveObj);
//                prospect_posp_erp_obj.aggregate([
//                    { $match: { "Erp_Id": objRequest.Erp_Id, "Fba_Id": objRequest.Fba_Id, "Erp_Qt": objRequest.Erp_Qt} }
//                ]).exec((err,dbData) => {
//                    if(err){
//                        res.json({ 'Status': 'FAIL', 'Msg': 'Error occurred during aggregation', 'Data': err });
//                    }
//                    else if(dbData.length > 0){
//                        res.json({'Status':'SUCCESS','Msg':'Data already present'});
//                    }
//                    else{
...
//                
//            } else {
//                res.json({'Status': 'FAIL', 'Msg': 'Request body is empty', 'Data': ''});
//            }
//        } catch (e) {
//            console.error('Exception', '/sync_contacts/save_prospect_posp_erp_data', e.stack);
...
app.post('/sync_contacts/save_prospect_posp_erp_data', function (req, res) {
        try {
            ...
            if (Object.keys(objRequest).length > 0) {
                ...
                saveObj['Modified_On'] = new Date();
                let prospect_posp_erp_obj = new prospect_posp_erp(saveObj);
                prospect_posp_erp.find(
                    { $or: [{ "Erp_Id": objRequest.Erp_Id}, {"Fba_Id": objRequest.Fba_Id}, {"Erp_Qt": objRequest.Erp_Qt} ]}
                ).exec((err,dbData) => {
                    if(err){
                        res.json({ 'Status': 'FAIL', 'Msg': 'Error occurred during aggregation', 'Data': err });
                    }
