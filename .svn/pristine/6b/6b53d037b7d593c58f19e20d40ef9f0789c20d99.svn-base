...
//To Do the changes in proposal page for date formate - TP Only  
            ...
            if (this.insurer_lm_request['product_id'] === 1) {
                if (config.environment.name.toString() === 'Production')
                {
                    ...
                    this.prepared_request['insurer_integration_agent_code'] = 'TPC00245'//KYC ;//'TPC00026';//Non-KYC
                    this.method_content = this.method_content.replace('___insurer_integration_agent_code___', 'TPC00245');
                }
            ...
            if (this.insurer_lm_request['product_id'] === 10) {
                if (config.environment.name.toString() === 'Production')
                {
                    ...
                    this.prepared_request['insurer_integration_agent_code'] = 'TPC00245';//KYC ;//'TPC00050';//Non-KYC
                    this.method_content = this.method_content.replace('___insurer_integration_agent_code___', 'TPC00245');
                }
                this.processed_request['___insurer_integration_agent_code___'] = this.prepared_request['insurer_integration_agent_code'];
                //this.method_content = this.method_content.replace('___dbmaster_pb_product_sub_category_class_code___', '');
...
//this.method_content = this.method_content.replace('___gender_2___', '');
        ...
        if (this.lm_request['method_type'] === 'Proposal') {
            ...
            if (this.lm_request.hasOwnProperty("kyc_no") && this.lm_request['kyc_no'] && this.lm_request['kyc_no'] !== "" && this.lm_request['kyc_no'] !== "undefined") {
                ...
                this.method_content = this.method_content.replace('___kyc_no___', this.lm_request['kyc_no']);
            } else {
                this.prepared_request['kyc_no'] = "";
                this.processed_request['___kyc_no___'] = this.prepared_request['kyc_no'];
                this.method_content = this.method_content.replace('___kyc_no___', "");
            }
        }
...
//posp start date 9-June-21 Chirag Modi
        ...
        if (parseInt(this.lm_request['product_id']) === 1) {
            ...
            this.method_content = this.method_content.replace('___net_premium___', this.prepared_request['net_premium']);
            this.method_content = this.method_content.replace('___tax___', this.prepared_request['tax']);
            this.method_content = this.method_content.replace('___final_premium___', this.prepared_request['final_premium']);
        }
    } catch (ex) {
...
HDFCErgoMotor.prototype.service_call = function (docLog, objProduct, Insurer_Object, specific_insurer_object, Db_Idv_Item) {
    try {
        ...
        if (specific_insurer_object.method.Method_Type === 'Proposal' && temp_ch_flag && this.lm_request['product_id'] === 1 && this.lm_request['is_breakin'] === "yes" && this.lm_request['is_inspection_done'] === "yes") {
            args = {
    ...
    try {
        if (this.insurer_lm_request['product_id'] === 1) {
            if (objResponseJson.hasOwnProperty('xmlstringResult')) {
                ...
                if ((objResponseJson['WsResult']['WsResultSet'][0]['WsStatus'][0] !== "1") && this.lm_request['product_id'] === 1 && this.lm_request['vehicle_insurance_type'] === 'renew' && this.lm_request['is_breakin'] === 'yes' && (['1CH_0TP', '1OD_0TP'].indexOf(this.lm_request['vehicle_insurance_subtype']) > -1))
                {
                    ...
                    if (this.lm_request['is_inspection_done'] === 'yes') {
                        console.log("==========START========== proposal_response_handler - is_inspection_done - yes ==========START==========");
                        var Proposal_Number = ""; //not_verified_result['PGTransNo'];
                        var Agent_Code = this.processed_request['___insurer_integration_agent_code___'];
                        ...
                        soap.createClient(callingService, function (err, client) {
                            client['GetBreakinProposalDataFinalPremium'](args, function (err1, result, raw, soapHeader) {
                                if (err1) {
                                    console.error('HDFCErgoMotor', 'GetBreakinProposalDataFinalPremium', 'exception', err1);
                                    var objResponseFull = {
                                        'err': err1,
                                        'result': result,
                                        'raw': raw,
                                    ...
                                    console.error('HDFCErgo GetBreakinProposalDataFinalPremium service response :', objResponseFull);
                                } else {
                                    var objResponseJson = {};
                                    var objResponseJsonLength = Object.keys(result).length;
                                    var processedXml = 0;
                                    for (var key in result) {
                                        ...
                                        xml2js.parseString(result[key], function (err2, objXml2Json) {
                                            processedXml++;
                                            if (err2) {
                                                console.error('HDFCErgoMotor', 'GetBreakinProposalDataFinalPremium', 'xml2jsonerror', err2);
                                                var objResponseFull = {
                                                    'err': err2,
                                                    'result': result,
                                                    'raw': raw,
                                                ...
                                                console.error('HDFCErgo GetBreakinProposalDataFinalPremium - xml2js Exception :', objResponseFull);
                                            } else {
                                                objResponseJson[key] = objXml2Json;
                                                if (processedXml === objResponseJsonLength) {
                                                    var objResponseFull = {
                                                    ...
                                                    if (objResponseJson.hasOwnProperty('GetBreakinProposalDataFinalPremiumResult')) {
                                                        //NEW Proposal Request hit again Pls...
                                                        console.log("NEW Proposal Request hit again Pls.........................");
                                                        var args = {
                                                            str: this.method_content,
                                                            VehicleClassCode: this.method_processed_request['product_id']
                                                        };
                                                        var callingServiceProposal = '';
                                                        if (config.environment.name.toString() === 'Production')
                                                        {
                                                            callingServiceProposal = 'https://hewspool.hdfcergo.com/motorcp/service.asmx?WSDL';
                                                        } else {
                                                            callingServiceProposal = 'http://202.191.196.210/uat/onlineproducts/newmotorcp/service.asmx?WSDL';
                                                        }
                                                        soap.createClient(callingServiceProposal, function (err, client) {
                                                            client['xmlstring'](args, function (err1, result, raw, soapHeader) {
                                                                //console.error('HDFCErgoMotor', 'service_call', err1, result);                                                                
                                                                if (err1 || result === null || typeof result === 'undefined') {
                                                                    //console.error('HDFCErgoMotor', 'service_call', 'exception', err1);
                                                                    var objResponseFull = {
                                                                        'err': err1,
                                                                            ...
                                                                            xml2js.parseString(result[key], function (err2, objXml2Json) {
                                                                                ...
                                                                                if (err2) {
                                                                                    ...
                                                                                    if (processedXml === objResponseJsonLength) {
                                                                                        ...
                                                                                        //get New Proposal Number for Inspection Payment
                                                                                        Error_Msg = 'NO_ERR';
                                                                                        proposalno = '';
                                                                                    }
                                                                                }
                        ...
                        console.log("==========END========== proposal_response_handler - is_inspection_done - yes ==========END==========");
                    }
                }
            } else {
