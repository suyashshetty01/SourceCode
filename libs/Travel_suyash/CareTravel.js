    ...
    try {
        ...
        if (objResponseJson !== null && objResponseJson.length > 0 && objResponseJson[0].hasOwnProperty(['_doc'])) {
            ...
            if (objPremiumService.hasOwnProperty('Premium')) {
                const mem_discnt = {1:1, 2: 0.95, 3: 0.90, 4: 0.85, 5: 0.825, 6: 0.80};
                let mem_count = this.lm_request['member_count']-0;
                let base_premium = Math.round((objPremiumService['Premium'] - 0) * mem_discnt[mem_count] * mem_count);
                var premium_breakup = this.get_const_premium_breakup();
                premium_breakup['final_premium'] = Math.ceil(base_premium * 1.18);
                premium_breakup['net_premium'] = Math.ceil(premium_breakup['final_premium'] / 1.18);
                ...
                premium_breakup['travel_insurance_si'] = (objPremiumService.InsurerProduct_Name.includes('Europe') ? "€" : "$") + objPremiumService.SumInsured;
                objServiceHandler.Premium_Breakup = premium_breakup;
            } else {
                Error_Msg = 'LM_MSG:MAIN_NODE_MISSING';
            }
        } else {
            Error_Msg = 'LM_MSG:Data_Unavailable_or_DB_Issue';
        }
