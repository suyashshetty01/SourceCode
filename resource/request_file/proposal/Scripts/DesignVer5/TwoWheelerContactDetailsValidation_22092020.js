...
function checkPuccNumber(input) {
    var pattern = new RegExp('^[A-Za-z]{2}[0-9]{14}$');
    var dvid = "dv" + $(input).attr('id');
    if (pattern.test(input.val()) == false) {
        $('#' + dvid).addClass('Error'); return false;
    } else { $('#' + dvid).removeClass('Error'); return true; }
...
function ValidateVAD(InsName) {
    ...
    //PUCC
    if (is_valid_pucc && is_valid_pucc == "yes") {
        if ($("#PUCC_No_Yes").hasClass('btn-default') && $("#PUCC_No_No").hasClass('btn-default') && InsurerID == 4 && $("#PUCC_valid_yn").val() == "") {
        ...
        if (summary.Quote_Request.vehicle_insurance_type === "renew" && InsurerID == 4 && is_valid_pucc && is_valid_pucc == "no") {
            $('#dvIsPucc').addClass('Error'); ValidationArray.push('PUCC_valid_yn'); ValidateError++;
        } else {
            $('#dvIsPucc').removeClass('Error');
        }
