    ...
    $(document).ready(function () {
        ...
        if (arrPlanID.length > 4) {
            ...
            if (isPlanAlreadyInList) {
                ...
                $.post("/PensionInsuranceIndia/GetReturnValue?PlanIdlst=" + PlanIdlst + "&iSumAssured=" + iSumAssured, null, function (strReturnValu) {
...
function OpenPlanDetails(plan_id) {
    $(document).ready(function () {
        $.post("/PensionInsuranceIndia/OpenPlanDetails?Plan_Id=" + plan_id, null, function (strReturnValu) {
            var _content = $.evalJSON(strReturnValu);
...
function VeiwAdditionDetail(quoteid, Plan_Id, GuaranteedAddition, PremiumPayingYear, PolicyTerm) {
    $(document).ready(function () {
        $.post("/PensionInsuranceIndia/VeiwAdditionDetail?quoteid=" + quoteid + "&Plan_Id=" + Plan_Id + "&GuaranteedAddition=" + GuaranteedAddition + "&PremiumPayingYear=" + PremiumPayingYear + "&PolicyTerm=" + PolicyTerm, null, function (strReturnValu) {
            var _content = $.evalJSON(strReturnValu);
            $("#dialog").html(_content);
            $("#dialog").dialog({
                width: 420,
...
function VeiwBonusDetail(quoteid, Plan_Id, GuaranteedAddition, PremiumPayingYear, PolicyTerm) {
    $(document).ready(function () {
        $.post("/PensionInsuranceIndia/VeiwBonusDetail?quoteid=" + quoteid + "&Plan_Id=" + Plan_Id + "&GuaranteedAddition=" + GuaranteedAddition + "&PremiumPayingYear=" + PremiumPayingYear + "&PolicyTerm=" + PolicyTerm, null, function (strReturnValu) {
            var _content = $.evalJSON(strReturnValu);
            $("#dialog").html(_content);
            $("#dialog").dialog({
                width: 420,
    ...
    buttons: {
        "Send Email": function () {
            ...
            if (_planId.length == 0) {
                ...
                if (_sendAs == "Quote") {
                    $.get("/PensionInsuranceIndia/SendEmailQuotes?planids=" + _planId + "&IsAlternate=" + _IsAlternate, function (response) {
                    ...
                    $.get("/PensionInsuranceIndia/SendEmailQuotesComparison?planids=" + _planId + "&IsAlternate=" + _IsAlternate, function (response) {
                        var _message = $.evalJSON(response);
                        if (_message == "Send") {
...
function GetAllPlansForEmail(sendas) {
    $.get("/PensionInsuranceIndia/GetAllPlans?sendas=" + sendas, function (response) {
        ...
        $("#dialogUpdateContactDetails").dialog({
            ...
            buttons: {
                "Update": function () {
                    ...
                    if (_error.length == 0) {
                        $.get("/PensionInsuranceIndia/UpdateContactDetails?email=" + $("#ContactEmail").val() + "&mobile=" + $("#ContactMobile").val(), function (response) {
...
function RemovePlan(plan_id, iSumAssured) {
    ...
    $(document).ready(function () {
        ...
        { document.getElementById("spanComparisionPlans").innerHTML = ""; }
            ...
            $.post("/PensionInsuranceIndia/GetReturnValue?PlanIdlst=" + nPlanlst + "&iSumAssured=" + iSumAssured, null, function (strReturnValu) {
                var _content = $.evalJSON(strReturnValu);
                document.getElementById("spanComparisionPlans").innerHTML = _content;
                // $("#spanComparisionPlans").val(ss);
            });
