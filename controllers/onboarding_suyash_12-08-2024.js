                                            ...
                                            if (posp_res.hasOwnProperty('POSP') && posp_res.POSP !== "NA") {
                                                ...
                                                if (dbresult.length > 0) {
                                                    ...
                                                    if (objRequest && objRequest.POSP_Sources && ['TESTUSER', 'NPOS', 'GPOS', 'TPOS', 'FPOS'].indexOf(objRequest.POSP_Sources) > -1) {
                                                        ...
                                                        args["Is_Email_Verified"] = "Yes";
                                                        args["Email_Verified_On"] = moment().format("YYYY-MM-DDTHH:mm:ss[Z]");
                                                        ...
                                                        if (err) {
                                                            ...
                                                            if (dbresult2.length > 0) {
                                                                ...
                                                                let objCertificate = {
                                                                    ...
                                                                    '___posp_city___': posp_city && posp_city.toLowerCase() || ''
                                                                };
                ...
                if (User_Id) {
                    ...
                    posp_user.find({'User_Id': User_Id}, function (posp_find_err, posp_find_res) {
                        ...
                        if ((posp_find_res && posp_find_res.length > 0) || objRequest["Email_Id"] || objRequest["Nominee_Relationship"]) {
                            ...
                            posp_email = posp_user_details.Email_Id;
                            nominee_relation = posp_user_details.Nominee_Relationship;
                            let address = [posp_user_details.Present_Add1 , posp_user_details.Present_Add2 , posp_user_details.Present_Add3].filter(Boolean).join(' ');
                            doc_uploaded = posp_user_details.Is_Document_Uploaded;
                            //posp_email ? uploaded_len++ : "";
                            //nominee_relation ? uploaded_len++ : "";
                            //Live profile 
...
//                            }
	...
	app.post('/onboarding/upload_onboarding_documents_test', (req, res) => {
        try {
            let form = new formidable.IncomingForm();
            let posp_obj = {}, posp_doc_log_obj = {};
            let User_Id = 0, uploaded_len = 0;
        ...
        try {
            ...
            if (trxId) {
                ...
                request(options, function (error, response) {
                    ...
                    let kycLinkResultRes = JSON.parse(response.body) || "";
                    objSummary['Status'] = 'FAIL';
                        ...
                        if (videoRef) {
                            ...
                            if (videoRef) {
                                ...
                                objSummary['Data'] = {
                                    "KYC_Video": videoRef,
                                    "KYC_Doc": kycImg,
                                    "KYC_Response": JSON.parse(response.body)
                                };
                            }
                        }
                    }
...
//                });
    ...
    app.get('/onboarding/send_appointment_letter_mail_NIU', function (req, res) {
        let objSummary = {};
        try {
            let Email = require('../models/email');
            let posp_users = require('../models/posp_users');
                    ...
                    if (posp_user_db_err) {
                        ...
                        if (posp_user_db_data.length > 0) {
                            ...
                            let posp_summary_data = {
                                ...
                                '___gender___': posp_user_data.Gender || '',
                                '___qualification___': posp_user_data.Education || '',
                            ...
                            html_web_path_portal='https://download.policyboss.com/onboarding/download/onboarding_docs/156354/Summary_156354.html';
                            ...
                            try {
                                ...
                                let download_url = get_pdf_url;
                                console.log("get_pdf_url : " + get_pdf_url);
                                let file_horizon = fs.createWriteStream(pdf_file_path);

                                let request_horizon = http.get(get_pdf_url, function (response) {
                                    get_pdf_url = file_horizon.path;
                                    response.pipe(file_horizon);
                                    console.error("Appointment Letter::PDF URL" + file_horizon);
                                    try {
                                        ...
                                        console.error('/onboarding/posp_summary Exception', e.stack);
                                    }
                                });
                            } catch (e) {
