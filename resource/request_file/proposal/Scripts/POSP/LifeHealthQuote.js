...
$(document).ready(function () {
        ...
        buttons: {
            "Send Email": function () {
                ...
                if (_planId.length == 0) {
                    ...
                    if (_sendAs == "Quote") {
                        $.get("/LifeHealthInsurance/SendEmailQuotes?planids=" + _planId + "&IsAlternate=" + _IsAlternate, function (response) {
                        ...
                        $.get("/LifeHealthInsurance/SendEmailQuotesComparison?planids=" + _planId + "&IsAlternate=" + _IsAlternate, function (response) {
                            var _message = $.evalJSON(response);
                            if (_message == "Send") {
    ...
    function GetAllPlansForEmail(sendas) {
        $.get("/LifeHealthInsurance/GetAllPlans?sendas=" + sendas, function (response) {
            ...
            $("#dialogUpdateContactDetails").dialog({
                ...
                buttons: {
                    "Update": function () {
                        ...
                        if (_error.length == 0) {
                            $.get("/LifeHealthInsurance/UpdateContactDetails?email=" + $("#ContactEmail").val() + "&mobile=" + $("#ContactMobile").val(), function (response) {
...
function GetPlanDetails(plan_id) {
    $(document).ready(function () {
        $.get('/LifeHealthInsurance/GetPlanDetails?Plan_Id=' + plan_id, function (response) {
...
function AddPlanToCompare(plan_id, plan_name) {
    $(document).ready(function () {
        ...
        if (_id == undefined) {
            ...
            if (count_plans <= 6) {
                $.get('/LifeHealthInsurance/AddPlanToCompare?Plan_Id=' + plan_id, function (response) {
                    var _content = $.evalJSON(response);
