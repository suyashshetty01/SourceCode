...
$('#PUCC_No_Yes').click(function () {
    Select1(this);
    is_valid_pucc = "yes";
    if ($("#PUCC_No_Yes").hasClass("btn-primary")) {
        $("#PUCC_No_No").removeClass('puc_no_active');
    } else {
        $("#PUCC_No_Yes").addClass('puc_no_active');
    }
    $("#PUCC_No_No").removeClass('puc_no_active');
    $("#PUCC_valid_yn").val('yes');
    $(".pucc_valid_yes").css('display', 'block');
});
$('#PUCC_No_No').click(function () {
    Select2(this);
    is_valid_pucc = "no";
    pucc_number = "";
    pucc_end_date = "";
    $("#PUCC_valid_yn").val('no');
    $("#PUCC_no").val('');
    $("#PUCC_date").val('');
    if ($("#PUCC_No_No").hasClass("btn-primary")) {
        $("#PUCC_No_No").addClass('puc_no_active');
    } else {
        $("#PUCC_No_Yes").addClass('puc_no_active');
    }
    $("#PUCC_No_Yes").removeClass('puc_no_active');
    $(".pucc_valid_yes").hide();
});
...
function checkPuccNumber(input) {
    var pattern = new RegExp('^[A-Za-z0-9]{5,20}$');
    var dvid = "dv" + $(input).attr('id');
    if (pattern.test(input.val()) == false) {
        $('#' + dvid).addClass('Error');
        return false;
...
function GetInsurerName(id) {
    ...
    case "11":
        ...
        break;
    case "12":
        InsurerName = "NewIndia";
        break;
    case "13":
        InsurerName = "Oriental";
        break;
    case "14":
        InsurerName = "United";
        break;
...
function ValidateSection(id) {
    ...
    case 'TabPersonalInfo':
        return ValidateCD(InsurerName);
        break;
        //case 'TabCompanyInfo': return ValidateCompanyD(InsurerName); break;
    case 'TabAddInfo':
        return ValidateAD(InsurerName);
        break;
    case 'TabCurrentTPPolicyInfo':
        return ValidateCTPD(InsurerName);
        break;
    case 'TabVehicleAddInfo':
        return ValidateVAD(InsurerName);
        break;
    case 'TabNomineeInfo':
        return ValidateND(InsurerName);
        break;
...
//function AgentValidation() {
    $('.Heading1').click(function (e) {
		...
		if(IsCustomer == "false" &&$(this).attr('id')=="PaymentLink"){
			popup_src="PaymentLink";
		}else if(popup_src == "PaymentLink" && $(this).attr('id')=="proceed_ok_btn"){
			popup_src="PaymentLink";
		}
		else{
			popup_src ="";
		...
		if((IsCustomer == "false" && ($(this).attr('id') !== "PayAgent" && $(this).attr('id') != "PayFromWallet" && $(this).attr('id') != "proceed_confirmation" && $(this).attr('id') != "proceed_ok_btn" && $(this).attr('id') != "RequestForInspection"))||(IsCustomer == "false" && $(this).attr('id') == "proceed_ok_btn" && popup_src == "PaymentLink")){
        ...
        if (data_sent.hasOwnProperty(this.id)) {
            ...
            else if (ValidatePersonalInformation(InsurerName) && ValidateVADAgent(InsurerName)) {
                if ($(this).attr('id') == 'PaymentLink' && $("#IsConfirmed").val() == "0") {
                    $("#confirmation_ok_dv").show();
                    $("#confirmation_spinner").show();
                    if (summary && summary.Quote_Request) {
                        var ncb_slab = {
                            "0": "20",
                            "20": "25",
                            "25": "35",
                        ...
                        var premiumReqData = summary.Quote_Request;
                        $(".confirmation_popup_name").html((premiumReqData.vehicle_registration_type === "individual" ? ($('#Salutation').val()).toUpperCase() : 'Messrs') + ' ' + ($('#ContactName').val()).toUpperCase() + ' ' + ($('#ContactMiddleName').val()).toUpperCase() + ' ' + ($('#ContactLastName').val()).toUpperCase());
                        $(".confirmation_popup_vehicle_no").html(($('#RegistrationNumberPart1').val()).toUpperCase() + '-' + ($('#RegistrationNumberPart1A').val()).toUpperCase() + '-' + ($('#RegistrationNumberPart2').val()).toUpperCase() + '-' + ($('#RegistrationNumberPart3').val()).toUpperCase());
                        $(".confirmation_popup_vehicle_details").html(premiumReqData.vehicle_full);
                        $(".confirmation_popup_policy_duration").html($('#_Policy_Duration').html());
                        $(".confirmation_popup_Vehicle_reg_date").html($('#_Registration_Date').html());
                        if (premiumReqData.vehicle_insurance_type === 'renew') {
                            $(".confirmation_popup_is_prev_policy").show();
                            $(".confirmation_popup_policy_expiry").html($('#_Prev_Expiry_Date').html());
                            $(".confirmation_popup_is_claim").html(premiumReqData.is_claim_exists == 'yes' ? 'Yes' : 'No');
                            $(".confirmation_popup_claim").hide();
                            if (premiumReqData.is_claim_exists == 'no') {
                                $(".confirmation_popup_claim").show();
                                $(".confirmation_popup_ncb").html(ncb_slab[premiumReqData.vehicle_ncb_current]);
                            }
                        }
                        if (premiumReqData.vehicle_insurance_subtype.indexOf('0CH') > -1) {
                            $(".confirmation_popup_isclaim").hide();
                        } else {
                            $(".confirmation_popup_idv").show();
                            $(".confirmation_popup_vehicle_idv").html($('#_IDV').html());
                        }
                        $(".confirmation_popup_vehicle_finance").html($("#IsFinanced").val() == 'True' ? 'Yes' : 'No');
                        $("#IsFinanced").val() == 'True' ? $(".confirmation_popup_financer").show() : $(".confirmation_popup_financer").hide();
                        $("#IsFinanced").val() == 'True' ? $(".confirmation_popup_finance_name").html($("#InstitutionName").val()) : '';
                        $("#confirmation_popup_details").show();
                    }
                    $("#confirmation_popup").show();
                } else {
                    $("#confirmation_paynow_dv").hide();
                    $("#confirmation_ok_dv").hide();
                    $("#confirmation_spinner").hide();
                    $("#confirmation_popup").hide();
                }                    
                if (($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'proceed_ok_btn') && $("#IsConfirmed").val() == "1") {
            ...
            else if (!(ValidatePersonalInformation(InsurerName))) {
...
function ValidatePersonalInformation(InsurerName) {
    ValidateError = 0;
    $('.valerr').remove();
    var ValidationArray = [];
    ...
    if (VehicleRegistrationType == "individual") {
        ...
        if (Salutation.val() == '0' || Salutation.val() == '') {
		...
		if (InsurerName == "Kotak" || InsurerName == "HDFCErgo") {
            if ($('#ContactLastName').val() == "" && ($('#ContactLastName').val().length < 1 || checkTextWithDotSpace($('#ContactLastName')) == false)) {
                $('#dvContactLastName').addClass('Error'); ValidateError++;
            }
...
$('.Heading1').click(function (e) {
	...
	if((IsCustomer == "false" && ($(this).attr('id') == "PayAgent" || $(this).attr('id') == "RequestForInspection" || $(this).attr('id') == "PayFromWallet" || $(this).attr('id') == "proceed_confirmation" || ($(this).attr('id') == "proceed_ok_btn" && popup_src !== "PaymentLink")))||IsCustomer == "true"){
    ...
    $("#accordion").find('.Heading1').each(function (n, i) {
        IDs.push(this.id);
    });
    $("#accordion").find('.panel-collapse').each(function (n, i) {
        ContentIDs.push(this.id);
    });
    var thislength = IDs.indexOf(thisval);
    if (summary.Summary.Insurer_Id === 11 && summary.Summary.Product_Id === 10 && summary.Quote_Request.is_breakin === "yes") {
        $("#TataBrkInNote").show();
    } else {
        $("#TataBrkInNote").hide();
    }
    for (var i = 0; i < thislength; i++) {
        ...
        if (summary.Quote_Request.vehicle_insurance_subtype == "1OD_0TP") {
            if (summary.Quote_Request.channel == "GS" || summary.Quote_Request.channel == "SM" || is_misp || is_kotakoem || summary.Quote_Request.channel == "CC") {
            ...
            //if (summary.Quote_Request.channel == "GS" || summary.Quote_Request.channel == "SM" || is_misp || is_kotakoem || summary.Quote_Request.channel == "CC") {
                if (summary.Quote_Request.channel != "PBS" || is_misp || is_kotakoem) {
                data_sent["TabTermsConditions"] = true;
            }
            if (ValidateSection(IDs[i]) == true) {
                $('#' + IDs[i]).find("i.indicator").removeClass('glyphs');
    ...
    if ($('#FinalSubmit').val() == "1" && ($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'RequestForInspection' || $(this).attr('id') == 'PayAgent' || $(this).attr('id') == 'PayFromWallet') && $("#IsConfirmed").val() == "0") {
        if ($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'PayAgent') {
            $("#confirmation_paynow_dv").show();
        } else {
            $("#confirmation_ok_dv").show();
        }
    ...
    if ($('#FinalSubmit').val() == "1" && ($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'RequestForInspection' || $(this).attr('id') == 'PayAgent' || $(this).attr('id') == 'PayFromWallet' || $(this).attr('id') == 'proceed_confirmation' || $(this).attr('id') == 'proceed_ok_btn' || $(this).attr('id') == 'verifyOtpspan') && $("#IsConfirmed").val() == "1") {
        $('#AlertMsg').hide();
        $('#Hidepopup').hide();
        $('#ProgressStatus').html('');
        ...
        data1.RegisteredPinCode = $("#RegisteredPinCode").val();
        data1.ContactPinCode = $("#ContactPinCode").val();
        data1.RegisteredCityName = $('#RegisteredCityName').val();
        data1.RegisteredStateId = $('#RegisteredStateId').val();
        data1.RegisteredStateName = $('#RegisteredStateName').val();
        data1.StateName = $('#StateName').val();
        data1.RegisteredCityId = $('#RegisteredCityId').val();
        data1.CommunicationStateId = $('#CommunicationStateId').val();
        data1.ContactCityID = $('#ContactCityID').val();
        data1.ContactCityName = $('#ContactCityName').val();
        console.log(data1);
        ...
        var is_customer = SsID != "0" ? false : true;
        ...
        request["final_premium"] = final_premium ;//$("#SelectedQuote_TotalPremium").val();
            request["tax"] = tax ;//$("#SelectedQuote_ServiceTax").val();
        ...
        if (request["is_financed"] == "yes") {
            request["financial_institute_city"] = $("#InstitutionCity").val();
        }
        ...
        //pucc
        if (request["is_valid_pucc"]) {
            request["is_valid_pucc"] = request["is_valid_pucc"].toUpperCase() ? request["is_valid_pucc"].toLowerCase() : request["is_valid_pucc"];
        }
        // Sending Through .net
        ...
        request["proposal_url"] = window.location.href;
        ...
        AddOnSelectedList = $('#ShowAddon .Addons').map(function () {
            return this.id;
        }).get();
        ...
        if (request["vehicle_insurance_subtype"] == "1OD_0TP") {
            ...
            request["saod_tp_tenure"] = parseInt(document.getElementById("TPPolicyTenure").value);
        }
        ...
        if (summary.Quote_Request.product_id == 1 && summary.Quote_Request.is_breakin == "yes" && summary.Quote_Request.is_inspection_done == "yes") {
            if ((((summary.Quote_Request.vehicle_insurance_subtype).split("CH_"))[0]) != '0') {
                ...
                request["iffco_inspection_id"] = $("#Iffco_Breakin_Inspection_Id").val();
                data1["IffcoInspectionId"] = $("#Iffco_Breakin_Inspection_Id").val();
            }
        }

        if ($("#whatsappMblNo").val() !== null && $("#whatsappMblNo").val() !== "" && $("#whatsappMblNo").val() !== undefined) {
            request["whatsapp_mobile"] = $("#whatsappMblNo").val();
            request["is_whatsapp_allow"] = 1;
        } else {
            request["is_whatsapp_allow"] = 0;
            request["whatsapp_mobile"] = "";
        }
        ...
        //Reason for CPA
        if ($('#ReasonforCPA').val() !== "0" && summary.Summary.Insurer_Id === 11) {
            request["valid_driving_license"] = "no";
            request["other_motor_policy"] = "no";
            request["is_standalone_cpa"] = "no";
            if ($('#ReasonforCPA').val() === "valid_driving_license") {
                request["valid_driving_license"] = "yes";
            } else if ($('#ReasonforCPA').val() === "other_motor_policy") {
                request["other_motor_policy"] = "yes";
            } else if ($('#ReasonforCPA').val() === "is_standalone_cpa") {
                request["is_standalone_cpa"] = "yes";
            }
        }
        if ($('#ReasonforCPAChola').val() !== "0" && summary.Summary.Insurer_Id === 3) {
            request["cpa_opt_reason"] = "";
            request["cpa_opt_date"] = "";
            if ($('#ReasonforCPAChola').val() !== "" && $('#ReasonforCPAChola').val() !== "0") {
                request["cpa_opt_reason"] = $('#ReasonforCPAChola').val();
            }
            if ($('#DORofCPA').val() !== "") {
                request["cpa_opt_date"] = $('#DORofCPA').val();
            }
        }

        if ($("#IsPayFromWallet").val() == "1" && ($(this).attr('id') == 'PayFromWallet' || $(this).attr('id') == 'proceed_ok_btn')) {
            request["pay_from"] = 'wallet';
        }
        request['helmet_invoice_date'] = $("#HPCvr_date").val();
        request['is_helmet_cover'] = summary.Quote_Request.is_helmet_cover;
        request['helmet_cover_si'] = summary.Quote_Request.helmet_cover_si;
        ...
        if ($("#NomineeOtherRelation").val() != "" && summary.Summary.Insurer_Id == 46) {
            request["nominee_other_relation"] = $("#NomineeOtherRelation").val();
        }
        console.log('data1', data1);
        request["proposal_url"] = window.location.href;
        request["engine_number"] = $("#EngineNumber").val();
        request["chassis_number"] = $("#ChasisNumber").val();
        var url_param = window.location.href.split("?")[1];
        url_param = url_param.split("&");
        var ARN = url_param[1].toUpperCase();
        ARN = ARN.split("=")[1];

        ClientID = url_param[0];
        ...
        request["middle_name"] = $("#ContactName").val();
        request["marital"] = $("#MaritalStatus").val();
        request["pan"] = $("#PANNo").val();
        request["aadhar"] = $("#AadharNo").val();
        request["occupation"] = $("#ContactOccupationId").val();
        request["gst_no"] = $("#GSTIN").val();
        request["company_name"] = $("#CompanyName").val();
        request["gender"] = $("#Gender").val();
        request["permanent_address_1"] = $("#RegisteredAddress").val();
        request["permanent_address_2"] = $("#RegisteredAddress2").val();
        request["permanent_address_3"] = $("#RegisteredAddress3").val();
        request["permanent_locality_code"] = $("#ddlRegisteredCityId").val();
        request["communication_address_1"] = $("#ContactAddress").val();
        request["communication_address_2"] = $("#Address2").val();
        request["communication_address_3"] = $("#Address3").val();
        request["communication_locality_code"] = $("#ddlContactCityID").val();
        request["registration_no_1"] = $("#RegistrationNumberPart1").val();
        request["registration_no_2"] = $("#RegistrationNumberPart1A").val();
        request["registration_no_3"] = $("#RegistrationNumberPart2").val();
        request["registration_no_4"] = $("#RegistrationNumberPart3").val();
        request["engine_number"] = $("#EngineNumber").val();
        request["chassis_number"] = $("#ChasisNumber").val();
        request["vehicle_color"] = $("#VehicleColor").val();
        request["financial_agreement_type"] = $("#FinancerAgreementType").val();
        request["financial_institute_name"] = $("#InstitutionName").val();
        request["financial_institute_code"] = $("#FinancierCode").val();
        request["financial_institute_city"] = $("#InstitutionCity").val();
        request["is_valid_pucc"] = $("#PUCC_valid_yn").val();
        request["pucc_number"] = $("#PUCC_no").val();
        request["pucc_end_date"] = $("#PUCC_date").val();
        request["nominee_relation"] = $("#NomineeRelationID").val();
        request["nominee_name"] = $("#NomineeName").val();
        request["nominee_birth_date"] = $("#NomineeDOB").val();
        request["tp_policy_number"] = $("#TPPolicyNumber").val();
        request["tp_start_date"] = $("#TPStartDate").val();
        request["tp_end_date"] = $("#TPEndDate").val();
        request["previous_policy_number"] = $("#PolicyNumber").val();
        request["lm_agent_name"] = $("#lm_agent_name").val();
        request["lm_agent_id"] = $("#lm_agent_id").val();
        request["lm_agent_mobile"] = $("#lm_agent_mobile").val();
        request["client_id"] = ClientID;
        request["execution_async"] = "yes";
        request["api_reference_number"] = summary.Summary.Service_Log_Unique_Id;
        request["CustomerReferenceID"] = $('#CustomerReferenceID').val();
        request["search_reference_number"] = summary.Summary.Request_Unique_Id;
        request["secret_key"] = 'SECRET-HZ07QRWY-JIBT-XRMQ-ZP95-J0RWP3DYRACW';
        request["client_key"] = 'CLIENT-CNTP6NYE-CU9N-DUZW-CSPI-SH1IS4DOVHB9';
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.hasOwnProperty('Status') && response.Status != null && response.Status != '' && response.Status == "VALIDATION") {
                    ...
                    $('.CRN').text("CRN : " + CRN);
                    $('#ProgressStatus').html('');
                    $('#ProgressStatus').append(response.Error.Error_Desc);
                    $('#err_msgtxt').html("");
                    $('#Hidepopup').show();
                } else {
...
// addtocartchanges
 $('.Heading2').click(function (e) {
     var a = SsID;
        ...
        if (($('#FinalSubmit').val() == "1" && ($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'RequestForInspection' || $(this).attr('id') == 'PayAgent' || $(this).attr('id') == 'PayFromWallet') && $("#IsConfirmed").val() == "0") || $(this).attr('id') == 'addToCart') {
            var btnSource = "";
			if ($(this).attr('id') == 'addToCart') {
                btnSource = "addToCart";
            }
            if ($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'PayAgent') {
                $("#confirmation_paynow_dv").show();
        ...
        if (($('#FinalSubmit').val() == "1" && ($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'RequestForInspection' || $(this).attr('id') == 'PayAgent' || $(this).attr('id') == 'PayFromWallet' || $(this).attr('id') == 'proceed_confirmation' || $(this).attr('id') == 'proceed_ok_btn' || $(this).attr('id') == 'verifyOtpspan') && $("#IsConfirmed").val() == "1") || $(this).attr('id') == 'addToCart') {
            if ($(this).attr('id') != 'addToCart') {
                $('#AlertMsg').hide();
                $('#Hidepopup').hide();
                $('#ProgressStatus').html('');
        ...
        // addtocartchanges end
            ...
            $.ajax({
                ...
                url: GetUrl() + "/quote/proposal_initiate", //'/TwoWheelerInsurance/Proposal_Initiate',
                data: JSON.stringify(request),
...
function addtocart(data) {
    ...
    //data.response = JSON.stringify(proposal_res);
    console.log('data', data);

    var data2 = Horizon_Method_Convert("/postservicecall/add_to_cart", data, "POST");
    $.ajax({
        url: GetUrl() + "/postservicecall/add_to_cart",//data2['url'],
        type: "POST",
        ...
        success: function (res) {
            ...
            if (res.Msg === "Success") {
                $("#addToCart").hide();
                $(".addToCartMain").show();
                $(".addToCartpopfail").hide();
                $(".addToCartpopSuccess").show();
            }
            else if (res.Msg === "Fail") {
                $(".addToCartMain").show();
                $(".addToCartpopfail").show();
                $(".addToCartpopSuccess").hide();
                $("#addToCartFailMsg").html(res.Msg);
            }
            else {
                $(".addToCartMain").show();
                $(".addToCartpopfail").show();
        ...
        error: function (err) {
            ...
            $(".addToCartpopfail").show();
            $(".addToCartpopSuccess").hide();
            $("#addToCartFailMsg").html(JSON.stringify(err));
        }
    })
}
...
function ValidateCD(InsName) {
    ...
    if (VehicleRegistrationType == "individual") {
		...
		if (InsName != "Bharti") {
            if (InsName == "Kotak" || InsName == "HDFCErgo") {
                if ($('#ContactLastName').val() == ""  && ($('#ContactLastName').val().length < 1 || checkTextWithDotSpace($('#ContactLastName')) == false)) {
                    $('#dvContactLastName').addClass('Error'); ValidationArray.push('ContactLastName'); ValidateError++;
                } else {
        ...
        if (InsName == "Bajaj" || InsName == "Bharti" || InsName == "Cholamandalam" || InsName == "FG" || InsName == "ICICI" || InsName == "IFFCO" || InsName == "Reliance" || InsName == "Royal" || InsName == "Oriental" || InsName == "SBI" || InsName == "Kotak" || InsName == "Acko" || InsName == "Edelweiss" || InsName == "Raheja") {
        ...
        if (SelectedQuote_NetPayablePayablePremium - 0 >= 50000) {
    ...
    if (InsName == "Tata") {
        if (summary.Quote_Request.is_pa_od === "no" && $('#ReasonforCPA').val() == "0") {
            $('#ReasonforCPA').addClass('Error');
            ValidationArray.push('ReasonforCPA');
            ValidateError++;
        } else {
            $('#ReasonforCPA').removeClass('Error');
        }
    }

    if (InsName == "Cholamandalam") {
        if (summary.Quote_Request.is_pa_od === "no" && $('#ReasonforCPAChola').val() == "0") {
            $('#ReasonforCPAChola').addClass('Error');
            ValidationArray.push('ReasonforCPAChola');
            ValidateError++;
        } else {
            if (summary.Quote_Request.is_pa_od === "no" && ($('#ReasonforCPAChola').val() == "Stand-alone CPA" || $('#ReasonforCPAChola').val() == "General Personal Accident cover")) {
                if ($("#DORofCPA").val() == "") {
                    $('#dvDORofCPA').addClass('Error');
                    ValidationArray.push('dvDORofCPA');
                    ValidateError++;
                } else {
                    $('#dvDORofCPA').removeClass('Error');
                }
            } else {
                $('#ReasonforCPAChola').removeClass('Error');
            }
...
function ValidateCompanyD(InsName) {
    ...
    if (VehicleRegistrationType == "corporate") {
        if (document.getElementById('CorporateName').value == "" || (document.getElementById('CorporateName').value.length < 2) || (checkCompanyTextWithSpace(document.getElementById('CorporateName').value) == false)) {
            $('#dvCorporateName').addClass('Error');
            ValidationArray.push('CorporateName');
            ValidateError++;
        } else {
            $('#dvCorporateName').removeClass('Error');
        }

        if ((document.getElementById('GSTNo').value == "" || document.getElementById('GSTNo').value == null)) {
            $("#dvGSTNo").addClass('Error');
            ValidationArray.push('GSTNo');
            ValidateError++;
        } else {
        ...
        if ($('#CompanyContactPersonName').val() == "" || $('#CompanyContactPersonName').val().length < 2 || checkTextWithSpace($('#CompanyContactPersonName')) == false) {
            $('#dvCompanyContactPersonName').addClass('Error');
            ValidationArray.push('CompanyContactPersonName');
            ValidateError++;
        } else {
            $('#dvCompanyContactPersonName').removeClass('Error');
        }

        if ($('#CompanyContactPersonMobile').val().length == 0 || checkMobile($('#CompanyContactPersonMobile')) == false) {
            $('#dvCompanyContactPersonMobile').addClass('Error');
            ValidationArray.push('CompanyContactPersonMobile');
            ValidateError++;
        } else {
            $('#dvCompanyContactPersonMobile').removeClass('Error');
        }

        if ($('#CompanyContactPersonEmail').val().length == 0 || checkEmail($('#CompanyContactPersonEmail')) == false) {
            $('#dvCompanyContactPersonEmail').addClass('Error');
            ValidationArray.push('CompanyContactPersonEmail');
            ValidateError++;
        } else {
            $('#dvCompanyContactPersonEmail').removeClass('Error');
        }
    } else {
        ValidateError = 0;
    }
...
function ValidateAD(InsName) {
    ...
    if ($('#RegisteredAddress').val() == "" || checkAddress($('#RegisteredAddress')) == false) {
        $('#dvRegisteredAddress').addClass('Error');
        ValidationArray.push('RegisteredAddress');
        ValidateError++;
    ...
    //RegisteredAddress2
    if ($('#RegisteredAddress2').val() == "" || checkAddress($('#RegisteredAddress2')) == false) {
        $('#dvRegisteredAddress2').addClass('Error');
        ValidationArray.push('RegisteredAddress2');
        ValidateError++;
    } else {
    ...
    //RegisteredAddress3
    if ($('#RegisteredAddress3').val() == "" || checkAddress($('#RegisteredAddress3')) == false) {
        $('#dvRegisteredAddress3').addClass('Error');
        ValidationArray.push('RegisteredAddress3');
        ValidateError++;
    } else {
    ...
    for (var i = 2; i <= 3; i++) {
        if ($('#RegisteredAddress' + i).val() == "" || checkAddress($('#RegisteredAddress' + i)) == false) {
            $('#dvRegisteredAddress' + i).addClass('Error');
            ValidationArray.push('RegisteredAddress' + i);
            ValidateError++;
            ...
            $('#dvRegisteredAddress' + i).removeClass('Error');
        }
    }

    if ($('#RegisteredPinCode').val().length == 0 || $('#RegisteredPinCode').val() < 110000 || checkPincode($('#RegisteredPinCode')) == false) {
        $('#dvRegisteredPinCode').addClass('Error');
        ValidationArray.push('RegisteredPinCode');
        ValidateError++;
    } else {
    ...
    if ($('#RegisteredCityName').val() == "" || $('#RegisteredCityName').val() == "NO SUCH CITY" || $('#RegisteredCityName').val() == "0" || $('#ContactCityID') == "0" || $('#RegisteredCityName').val() == null) {
        $('#dvRegisteredCityName').addClass('Error');
        ValidationArray.push('RegisteredCityName');
        ValidateError++;
        $('#RegisteredCityName').val('');
        $('#RegisteredCityName').removeClass('used')
    } else {
        $('#dvRegisteredCityName').removeClass('Error');
    }

    if ($('#RegisteredStateName').val() == "" || $('#RegisteredStateName').val() == " ") {
        $('#dvRegisteredStateName').addClass('Error');
        ValidationArray.push('RegisteredStateName');
        ValidateError++;
    ...
    if ((InsName != 'Tata' && InsName != 'NewIndia' && InsName != 'Royal' && InsName != 'United' && InsName != 'ICICI' && InsName != 'Oriental' && InsName != 'Edelweiss' && InsName != 'DHFL') || InsName == 'Cholamandalam' || hdfcergo_breakin_location_flag) {
        if ($('#ddlRegisteredCityId').val() == "0" || $('#ddlRegisteredCityId').val() == 0 || $('#ddlRegisteredCityId').val() == null) {
            $('#ddlRegisteredCityId').addClass('Error');
            ValidationArray.push('ddlRegisteredCityId');
            ValidateError++;
        } else {
    ...
    if (!$('input[id=SameAsCommunication]').is(':checked')) {
        ...
        if ($('#ContactAddress').val() == "" || checkAddress($('#ContactAddress')) == false) {
            ...
            ValidationArray.push('ContactAddress');
            ValidateError++;
            ...
            $('#dvContactAddress').removeClass('Error');
        }

        //Address2
        if ($('#Address2').val() == "" || checkAddress($('#Address2')) == false) {
            $('#dvAddress2').addClass('Error');
            ValidationArray.push('Address2');
            ValidateError++;
        } else {
            $('#dvAddress2').removeClass('Error');
        }

        //Address3
        if ($('#Address3').val() == "" || checkAddress($('#Address3')) == false) {
            $('#dvAddress3').addClass('Error');
            ValidationArray.push('Address3');
            ValidateError++;
        } else {
            $('#dvAddress3').removeClass('Error');
        }

        for (var i = 2; i <= 3; i++) {
            if ($('#Address' + i).val() == "" || checkAddress($('#Address' + i)) == false) {
                $('#dvAddress' + i).addClass('Error');
                ValidationArray.push('Address' + i);
                ValidateError++;
                ...
                $('#dvAddress' + i).removeClass('Error');
            }
        }

        if ($('#ContactPinCode').val().length == 0 || $('#ContactPinCode').val() < 110000 || checkPincode($('#ContactPinCode')) == false) {
            $('#dvContactPinCode').addClass('Error');
            ValidationArray.push('ContactPinCode');
            ValidateError++;
        } else {
        ...
        if ($('#ContactCityName').val() == "" || $('#ContactCityName').val() == "0" || $('#ContactCityID') == "0" || $('#ContactCityName').val() == null) {
            ...
            ValidationArray.push('ContactCityName');
            ValidateError++;
        } else {
        ...
        if ($('#StateName').val() == "") {
            ...
            ValidationArray.push('StateName');
            ValidateError++;
        } else {
        ...
        if ((InsName != 'Tata' && InsName != 'NewIndia' && InsName != 'Royal' && InsName != 'Oriental' && InsName != 'Edelweiss' && InsName != 'DHFL') || InsName == 'Cholamandalam' || hdfcergo_breakin_location_flag) {
            if ($('#ddlContactCityID').val() == "0" || $('#ddlContactCityID').val() == 0 || $('#ddlContactCityID').val() == null) {
                $('#ddlContactCityID').addClass('Error');
                ValidationArray.push('ddlContactCityID');
                ValidateError++;
            } else {
                $('#ddlContactCityID').removeClass('Error');
            }
        }
        //    }
                ...
                if (summary.Quote_Request.is_breakin == "yes") {
                    if (InsurerID == 5) {
                        if ($('#ListOfBreakinLocation').val() == "" || $('#ListOfBreakinLocation').val() == "0" || $('#ListOfBreakinLocation').val() == null) {
                            $('#ListOfBreakinLocation').addClass('Error');
                            ValidationArray.push('ListOfBreakinLocation');
                            ValidateError++;
                        } else {
...
function ValidateCTPD(InsName) {
    ...
    if (summary.Quote_Request.vehicle_insurance_subtype == "1OD_0TP") {
        ...
        if (document.getElementById('TPStartDate').value == "") {
            $('#dvTPStartDate').addClass('Error');
            ValidationArray.push('TPStartDate');
            ValidateError++;
        } else {
            $('#dvTPStartDate').removeClass('Error');
        }

        if (document.getElementById('TPPolicyTenure').value == "" || document.getElementById('TPPolicyTenure').value == "0") {
            $('#TPPolicyTenure').addClass('Error');
            ValidationArray.push('TPPolicyTenure');
            ValidateError++;
            $("#TPEndDate").val("");
        } else {
            $('#TPPolicyTenure').removeClass('Error');
        }

        if (document.getElementById('TPEndDate').value == "") {
            $('#dvTPEndDate').addClass('Error');
            ValidateError++;
        } else {
            $('#dvTPEndDate').removeClass('Error');
        }
...
function ValidateVAD(InsName) {
    ...
    if ($('#AreYouOwnerNo').hasClass('btn-primary')) {
        ...
        var arr = data.split(' ');
        if ($('#OwnerName').val == "" || checkTextWithSpace($('#OwnerName')) == false || arr.length < 2 || arr[arr.length - 1] == "") {
            $('#dvOwnerName').addClass('Error');
            ValidateError++;
        } else {
            $('#dvOwnerName').removeClass('Error');
        ...
        if ($('#OwnerGenderMale').hasClass('active') || $('#OwnerGenderFemale').hasClass('active')) {
            $('#divOwnerGender').removeClass('Error');
        } else {
            $('#divOwnerGender').addClass('Error');
            ValidateError++;
        }
    }
    ...
    if ($EngineNumber.val() == "" || $EngineNumber.val().length < 6 || $EngineNumber.val().length > 20 || pattern.test($EngineNumber.val()) == false || checkText($('#EngineNumber')) == true) {
        ...
        ValidationArray.push('EngineNumber');
        ValidateError++;
    } else {
        $('#dvEngineNumber').removeClass('Error');
    }
    ...
    if ($ChasisNumber.val() == "" || $ChasisNumber.val().length < 6 || $ChasisNumber.val().length > 20 || pattern.test($ChasisNumber.val()) == false || checkText($('#ChasisNumber')) == true) {
        ...
        ValidationArray.push('ChasisNumber');
        ValidateError++;
    } else {
        $('#dvChasisNumber').removeClass('Error');
    }
    ...
    if (InsName == "United") {
    ...
    } else if (InsName == "Tata") {
        if ($('#TwoWheelerType').val() === "NEW") {
    ...
    } else if (InsName == "Kotak" || InsName == "MagmaHDI" || InsName == "Royal") {
        if (InsName == "Kotak") {
            if ($EngineNumber.val() == "" || $EngineNumber.val().length < 6 || $EngineNumber.val().length > 50 || pattern.test($EngineNumber.val()) == false || checkText($('#EngineNumber')) == true) {
                $('#dvEngineNumber').addClass('Error');
                ValidationArray.push('EngineNumber');
                ValidateError++;
            } else {
        ...
        if (summary.Quote_Request.vehicle_insurance_type === "renew" && ($ChasisNumber.val() == "" || ($ChasisNumber.val().length < 8 || $ChasisNumber.val().length > 17) || pattern.test($ChasisNumber.val()) == false || checkText($('#ChasisNumber')) == true)) {
            $('#dvChasisNumber').addClass('Error');
            ValidationArray.push('ChasisNumber');
            ValidateError++;
        } else if (summary.Quote_Request.vehicle_insurance_type === "new" && ($ChasisNumber.val() == "" || $ChasisNumber.val().length !== 17 || pattern.test($ChasisNumber.val()) == false || checkText($('#ChasisNumber')) == true)) {
            $('#dvChasisNumber').addClass('Error');
            ValidationArray.push('ChasisNumber');
            ValidateError++;
        } else {
    ...
    //Is Financed Fields
    console.log($('#IsFinanced').val());
    if ($('#IsFinanced').val() == "True") {
        if ($('#InstitutionName').val() == "" || checkInstName($('#InstitutionName')) == false || $("#FinancierCode").val() == "") {
            $('#dvInstitutionName').addClass('Error');
            ValidationArray.push('InstitutionName');
            ValidateError++;
        } else {
            $('#dvInstitutionName').removeClass('Error');
        }

        if (InsName == "ICICI" || InsName == "Kotak" || InsName == "SBI") {
            if ($('#InstitutionCity').val() == "" || checkCity1($('#InstitutionCity')) == false) {
        ...
        if ($('#FinancerAgreementType').val() == null || $('#FinancerAgreementType').val() == "0" || $('#FinancerAgreementType').val() == "") {
            $('#FinancerAgreementType').addClass('Error');
            ValidationArray.push('FinancerAgreementType');
            ValidateError++;
        } else {
            $('#FinancerAgreementType').removeClass('Error');
        }
    } else {
        $('#dvInstitutionName').removeClass('Error');
        $('#dvInstitutionCity').removeClass('Error');
        $('#InstitutionName').val("");
        $('#FinancerAgreementType').val("0");
    }

    //PUCC
    if (is_valid_pucc && is_valid_pucc == "yes" && summary.Quote_Request.vehicle_insurance_type === "renew") {
        if ($("#PUCC_No_Yes").hasClass('btn-default') && $("#PUCC_valid_yn").val() == "no" && (InsurerID == 2 || InsurerID == 4 || InsurerID == 10 || InsurerID == 19)) {
            $('#dvIsPucc').addClass('Error');
            ValidationArray.push('PUCC_valid_yn');
            ValidateError++;
        } else {
            $('#dvIsPucc').removeClass('Error');
        }

        if ($("#PUCC_valid_yn").val() && $("#PUCC_valid_yn").val() === "yes") {
            if ($("#PUCC_no").val() == "" || checkPuccNumber($('#PUCC_no')) == false) {
                $('#dvPUCC_no').addClass('Error');
                ValidationArray.push('PUCC_no');
                ValidateError++;
            } else {
                //pucc_number = $("#PUCC_no").val();
                //if (pucc_number.length < 5 || pucc_number.length > 20) {
                $('#dvPUCC_no').removeClass('Error');
                //} else {
                //    $('#dvPUCC_no').addClass('Error'); ValidationArray.push('PUCC_no'); ValidateError++;
                //}
            }

            if ($("#PUCC_date").val() == "") {
                $('#dvPUCC_date').addClass('Error');
                ValidationArray.push('PUCC_date');
                ValidateError++;
            } else {
                pucc_end_date = $("#PUCC_date").val();
                $('#dvPUCC_date').removeClass('Error');
            }
        } else {
            $('#dvPUCC_no').removeClass('Error');
            $('#dvPUCC_date').removeClass('Error');
        ...
        if (is_valid_pucc == undefined) {
            $('#dvIsPucc').removeClass('Error');
            $("#PUCC_No_No").removeClass('btn-default');
            $("#PUCC_No_No").addClass('btn-primary');
        } else {
            if (summary.Quote_Request.vehicle_insurance_type === "renew" && is_valid_pucc) {
                if (summary.Quote_Request.vehicle_insurance_type === "renew" && is_valid_pucc && is_valid_pucc == "no" && (InsurerID == 2 || InsurerID == 4 || InsurerID == 10)) {
                    $('#dvIsPucc').addClass('Error');
                    ValidationArray.push('PUCC_valid_yn');
                    ValidateError++;
                } else {
            ...
            } else if (summary.Quote_Request.vehicle_insurance_type === "new") {
                $('#dvIsPucc').removeClass('Error');
            } else {
                $('#dvIsPucc').addClass('Error');
                ValidationArray.push('PUCC_valid_yn');
    ...
    // Helmet Cover validation
	if (summary.Quote_Request.is_helmet_cover == 'yes' && InsurerID == 17 && ProductID == 10) {
        if ($("#HPCvr_date").val() == "") {
            $("#dvHPC_date").addClass('Error');
            ValidationArray.push('HPCvr_date');
            ValidateError++;
        } else {
            $("#dvHPC_date").removeClass('Error');
        }
    } else {
        $("#dvHPC_date").removeClass('Error');
        $("#HPCvr_date").val("");
    ...
    if ($('#TwoWheelerType').val() == "RENEW") {
        if ($('#RegistrationNumberPart1').val() == "" || checkText($('#RegistrationNumberPart1')) == false || $('#RegistrationNumberPart1').val().length < 2) {
            $('#dvRegistrationNumberPart1').addClass('Error');
            ValidateError++;
        } else {
            $('#dvRegistrationNumberPart1').removeClass('Error');
        }

        if (($('#RegistrationNumberPart2').val()).toUpperCase() == 'ZZ') {
            $('#dvRegistrationNumberPart2').addClass('Error');
            ValidationArray.push('RegistrationNumberPart2');
            $('#dvRegistrationNumberPart3').addClass('Error');
            ValidationArray.push('RegistrationNumberPart3');
            ValidateError++;
        } else {
            $('#dvRegistrationNumberPart2').removeClass('Error');
            $('#dvRegistrationNumberPart3').removeClass('Error');
        ...
        if ($('#RegistrationNumberPart1A').val() == "" || (checkifAlphaNumeric($('#RegistrationNumberPart1A')) == false) || checkText($('#RegistrationNumberPart1A')) == true) {
            ...
            ValidationArray.push('RegistrationNumberPart1A');
            ValidateError++;
        } else {
            var Numpattern = new RegExp('^[0-9]+$');
            if (Numpattern.test($('#RegistrationNumberPart1A').val()) == true) { // Numeric
        ...
        if ($('#RegistrationNumberPart3').val() == "" || checkNumeric($('#RegistrationNumberPart3')) == false || $('#RegistrationNumberPart3').val().length < 4 || $('#RegistrationNumberPart3').val() <= 0) {
            $('#dvRegistrationNumberPart3').addClass('Error');
            ValidationArray.push('RegistrationNumberPart3');
            ValidateError++;
        } else {
        ...
        if (summary.Quote_Request.is_policy_exist == "yes" && ($('#PolicyNumber').val() == "" || checkPolicyNumber($('#PolicyNumber')) == false || checkText($('#PolicyNumber')) == true)) {
            $('#dvPolicyNumber').addClass('Error');
            ValidationArray.push('PolicyNumber');
            ValidateError++;
        } else {
            $('#dvPolicyNumber').removeClass('Error');
        }
    ...
    if (InsName == "Bajaj" || InsName == "Bharti" || InsName == "Cholamandalam" || InsName == "ICICI" || InsName == "IFFCO" || InsName == "Royal" || InsName == "Oriental" || InsName == "SBI" || InsName == "Kotak" || InsName == "Edelweiss" || InsName == "HDFCErgo" || InsName == "Tata" || InsName == "NewIndia" || InsName == "United" || InsName == "Universal" || InsName == "Liberty" || InsName == "MagmaHDI" || InsName == "GoDigit" || InsName == "DHFL" || InsName == "Raheja") {
        if ($("#VehicleColor").val() == "" || $("#VehicleColor").val() == 0 || $("#VehicleColor").val() == "0") {
            $('#VehicleColor').addClass('Error');
            ValidationArray.push('VehicleColor');
            ValidateError++;
        } else {
            $('#VehicleColor').removeClass('Error');
        }
    }
...
function EdelweissRelation() {
    if ($('#NomineeRelationID option:selected').text() == "Others" && InsurerName == "Edelweiss") {
        $('#dvNomineeOtherRelation').show();
    } else {
        $('#dvNomineeOtherRelation').hide();
    }
}
...
function ValidateND(InsName) {
    ...
    if (VehicleRegistrationType != "corporate") {
        if ((VehicleRegistrationType == "corporate" && InsName == "Bharti") || VehicleRegistrationType == "individual") {
            ...
            var arr = data.split(' ');
            if (InsName == "Royal") {
                if ($('#NomineeName').val == "" || checkTextWithSpace($('#NomineeName')) == false) {
            ...
            if ($('#NomineeRelationID').val() == "0") {
                ...
                ValidationArray.push('NomineeRelationID');
                ValidateError++;
            } else {
            ...
            if ($('#NomineeDOB').val() == "") {
                ...
                ValidationArray.push('NomineeDOB');
                ValidateError++;
            } else {
            ...
            if (InsName == "Edelweiss" && $('#NomineeRelationID option:selected').text() == "Others") {
                if ($("#NomineeOtherRelation").val() == "" || checkTextWithSpace($('#NomineeOtherRelation')) == false) {
                    $('#dvNomineeOtherRelation').addClass('Error');
                    ValidationArray.push('NomineeOtherRelation');
                    ValidateError++;
                } else {
                    $('#dvNomineeOtherRelation').removeClass('Error');
                }
            }
        }
    ...
    if (ValidateError < 1) {
        if ((VehicleRegistrationType == "individual") && (InsName == "Bharti" || InsName == "HDFCErgo" || InsName == "Reliance")) {
			$("#FinalSubmit").val("1"); 
		}
...
function ValidateTC(InsName) {
    ...
    if (summary.Quote_Request.vehicle_insurance_subtype != null) {
        ...
        if (VIST[0].split('CH')[0] > 0 || VIST[0].split('OD')[0] > 0) {
            is_Comp = "yes";
        }
    }
    ...
    if (InsName == "Tata" && vehicle_ins_type == "renew" && ProductID == 1 && is_Comp == "yes") {
        ...
        if (planename != "GOLD" && planename != "Basic" && planename != "SILVER" && planename != "OD") {
            is_Tata = true;
        }
    } 
    //Somanshu
    ...
    if (true) {
        if ($("#iagree").is(":checked") == false) {
            //if (summary.Quote_Request.channel == 'GS' || summary.Quote_Request.channel == 'SM' || summary.Quote_Request.channel == 'CC' || ([816, 14154, 14155, 14156, 14157, 14158, 17051, 15912, 15253, 11971, 17053].indexOf(summary.Quote_Request.ss_id) > -1 && ProductID === 10) || ([822].indexOf(summary.Quote_Request.ss_id) > -1 && ProductID === 10)) { }
            if (summary.Quote_Request.channel != 'PBS' || ([816, 14154, 14155, 14156, 14157, 14158, 17051, 15912, 15253, 11971, 17053].indexOf(summary.Quote_Request.ss_id) > -1 && ProductID === 10) || ([822].indexOf(summary.Quote_Request.ss_id) > -1 && ProductID === 10)) { }
            else {
        ...
        if (!($(".btn_yes").hasClass('active_Btn') || $(".btn_no").hasClass('active_Btn'))) {
            $('.QuestionPanel_grid').addClass('Errorchack');
            ValidateError++;
            return false;
        } else { $(".QuestionPanel_grid").removeClass('Errorchack'); }
        ...
        if ($(".btn_yes").hasClass('active_Btn')) {
            if ($("#whatsappMblNo").val().length == 0 || checkMobile($('#whatsappMblNo')) == false) {
                $('#errwhatsappMblNo').text("Please Enter valid number").addClass('Errorchack').show();
                ValidateError++;
                return false;
            } else { $('#errwhatsappMblNo').text("").removeClass('Errorchack'); }
        } else { $('#errwhatsappMblNo').text("").removeClass('Errorchack').hide(); }
...
function set_values(request) {
    ...
    if (request.hasOwnProperty("cpa_opt_reason")) {
        $('#ReasonforCPAChola').val(request["cpa_opt_reason"]);
    }
    if (request.hasOwnProperty("cpa_opt_date")) {
        $('#DORofCPA').val(request["cpa_opt_date"]);
        if (request["cpa_opt_reason"] == "Stand-alone CPA" || request["cpa_opt_reason"] == "General Personal Accident cover") {
            $("#divDORofCPA").show();
        } else {
            $("#divDORofCPA").hide();
        }
    ...
    //pucc
    if (request.hasOwnProperty("is_valid_pucc")) {
        if (request.insurer_id == 2 || request.insurer_id == 4 || request.insurer_id == 10) {
            request.is_valid_pucc = "yes";
        }

        if (request.is_valid_pucc == "no") {
            is_valid_pucc = "no";
            pucc_number = "";
            pucc_end_date = "";
            $("#PUCC_valid_yn").val('no');
            ...
            $("#PUCC_No_No").removeClass('btn-default')
            $("#PUCC_No_Yes").removeClass('puc_no_active');
            $("#PUCC_No_Yes").removeClass('btn-primary');
            $("#PUCC_No_No").addClass('btn-primary');
            $("#PUCC_No_No").addClass('puc_no_active');
            $(".pucc_valid_yes").hide();
        } else {
            ...
            pucc_end_date = request.pucc_end_date;
            $("#PUCC_valid_yn").val('no');
            $("#PUCC_no").val(request.pucc_number);
            $("#PUCC_date").val(request.pucc_end_date);
            $("#PUCC_No_Yes").removeClass('btn-default')
            $("#PUCC_No_No").removeClass('puc_no_active');
            $("#PUCC_No_No").removeClass('btn-primary');
            $("#PUCC_No_Yes").addClass('btn-primary');
            $("#PUCC_No_Yes").addClass('puc_no_active');
            $(".pucc_valid_yes").show();
        }
    }
...
var field_mapping = {
    ...
    "PUCC_no": "pucc_number",
    "PUCC_date": "pucc_end_date",
    "PUCC_valid_yn": "is_valid_pucc",
    "TPPolicyTenure": "saod_tp_tenure",
    "HPCvr_date" : "helmet_cover_date"
};
...
var error_msg = {
    ...
    "TPPolicyTenure": "Please select TP Policy Tenure",
    ...
    "PUCC_date": "Enter PUCC end date",
    "PUCC_no": "Enter valid PUCC number",
    "PUCC_valid_yn": "Do you have valid PUCC",
    "ReasonforCPA": "Please select reason for not opt CPA",
    "ReasonforCPAChola": "Please select reason for not opt CPA",
    "dvDORofCPA": "Please select date of registration",
    "HPCvr_date": "Please select date of Helmet Cover"
};
...
function populate_errordetails(ValidationArray) {
    ...
    for (i = 0; i < ValidationArray.length; ++i) {
        ...
        } else if (InsurerID === 11 && ValidationArray[i] === "ChasisNumber" && $('#ChasisNumber').val.length !== 17 && $('#TwoWheelerType').val() === "NEW") {
...
function Salution_Error(ValidationArray_Salutation) {
    ...
    for (i = 0; i < ValidationArray_Salutation.length; ++i) {
        if (ValidationArray_Salutation[i] == "GenderNotMatch") {
            ...
            var err = "<div class='valerr' style='color:red'>" + error_msg['Salutation'] + "</div>";
        }
...
function proposalError() {
    ...
    $.ajax({
        type: "POST",
        data: JSON.stringify(data1),
        url: GetUrl() +'/report/getProposalInsurer_Master',
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (response) {
...
function Raise_Ticket() {
    ...
    $.ajax({
        type: "GET",
        url: GetUrl()+ '/ticket_exist/' + CRN + '/' + error_specification,//obj_horizon_data['url'],
        dataType: "json",
        success: function (data) {
            ...
            if (result === 1 && response['Status'] === "Success") {
                ...
                var objdata = {
                    ...
                    "fba_id":summary.Quote_Request['fba_id'],
				...
				objdata["secret_key"]='SECRET-HZ07QRWY-JIBT-XRMQ-ZP95-J0RWP3DYRACW';
				objdata["client_key"]='CLIENT-CNTP6NYE-CU9N-DUZW-CSPI-SH1IS4DOVHB9';
                ...
                $.ajax({
                    ...
                    url: GetUrl()+"/report/raiseticket", //obj_horizon_data['url'],
                    contentType: "application/json;charset=utf-8",
                    dataType: "json",
                    success: function (response) {
...
function redirect(ARN, product_id, Insurer_name) {
	var url_param = window.location.href.split("?")[1];
		...
		SsID = url_param[3];
		SsID = SsID.split("=")[1];
    _urlLink = ((parseInt(product_id) == 10) ? "/proposal/TwoWheelerContactDetailsNew.html?ClientID=" : "/proposal/TwoWheelerContactDetailsNew.html?ClientID=") + ClientID + "&ARN=" + ARN+"&POSP=NonPOSP&SsID="+SsID;
    ...
    if ($('#SupportsAgentID').val() != "" && $('#SupportsAgentID').val() != "0") {
        send_payment_link2(ARN, Insurer_name)
    } else {
        window.location.href = _urlLink;
    }
}
function send_payment_link2(api_reference_number, Insurer_name) {
    var err = 0;
    var ProdID = summary.Quote_Request.product_id;
    var firstname = $("#ContactName").val();
    var lastname = $("#ContactLastName").val();
    var sal = $("#Salutation").val();
    var email = $("#ContactEmail").val();
    var reg4 = $("#RegistrationNumberPart3").val();
    var _payLink = "";
    var url_param = window.location.href.split("?")[1];
    url_param = url_param.split("&");
    ClientID = url_param[0];
    ...
    if (err == 0) {
        ...
        var obj = {
            ...
            "insurer_id": summary.Summary.Insurer_Id,
        ...
        var SaveDataURL = 'http://qa-horizon.policyboss.com:3000/quote/send_payment_link';
        var obj_horizon_data = Horizon_Method_Convert(SaveDataURL, obj, "POST");
        $.ajax({
            type: "POST",
            data: JSON.stringify(obj),
            url:  GetUrl()+"/quote/send_payment_link", //"http://qa-horizon.policyboss.com:3000/quote/send_payment_link",
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            success: function (response) {
...
function update_user_data(current_div) {
    ...
    if ($('#ReasonforCPA').val() !== "0" && summary.Summary.Insurer_Id === 11) {
        obj["valid_driving_license"] = "no";
        obj["other_motor_policy"] = "no";
        obj["is_standalone_cpa"] = "no";
        if ($('#ReasonforCPA').val() === "valid_driving_license") {
            obj["valid_driving_license"] = "yes";
        } else if ($('#ReasonforCPA').val() === "other_motor_policy") {
            obj["other_motor_policy"] = "yes";
        } else if ($('#ReasonforCPA').val() === "is_standalone_cpa") {
            obj["is_standalone_cpa"] = "yes";
        }
    }
    ...
    if ($('#ReasonforCPAChola').val() !== "0" && summary.Summary.Insurer_Id === 3) {
        ...
        obj["cpa_opt_date"] = "";
        if ($('#ReasonforCPAChola').val() !== "" && $('#ReasonforCPAChola').val() !== "0") {
            obj["cpa_opt_reason"] = $('#ReasonforCPAChola').val();
        }
        if ($('#DORofCPA').val() !== "") {
            obj["cpa_opt_date"] = $('#DORofCPA').val();
        }
    }
	obj["client_id"] = ClientID;
	obj["method_name"]='/quote/save_user_data';
    ...
    $.ajax({
        type: "POST",
        data: JSON.stringify(obj),
        url: GetUrl() + "/quote/save_user_data", //"http://localhost:3000/quote/save_user_data",
        dataType: "json",
        contentType: "application/json;charset=utf-8",
        success: function (response) {
            //console.log('data', data);
...
function update_user_status() {
    ...
    /*$.get(GetUrl() + "/quote/save_user_data?update_data=" + JSON.stringify(obj) + "&ClientID=" + ClientID,
        function (data) {
        //console.log('data', data);
        try {
...
function get_payment_data(arn, count, proposal_id) {
    ...
    var data = {
        search_reference_number: summary.Quote_Request.search_reference_number,
    ...
    $.ajax({
        ...
        success: function (response) {
                ...
                if (response_json.Status === 'complete') {
                    ...
                    var VIST = (summary.Quote_Request.vehicle_insurance_subtype).split("CH_");
                    //QA
                    if (product_id == "1" && (InsurerID == 5 || InsurerID == 9 || InsurerID == 33 || InsurerID == 6 || InsurerID == 7 || InsurerID == 44) && summary.Quote_Request.is_breakin == "yes" && summary.Quote_Request.is_inspection_done == "no" && (VIST[0] != '0')) {
                        //Live
                        ...
                        $('.Insurer_listbox, #RaisedBtn ,#err_msgtxt').hide();
                        if (InspId != null && InspId != '') {
                            ...
                            else if (InsurerID == 44) {
                                var digInspId = InspId.split("#");
                                $('#ProgressStatus').append("<div style='font-size: 13px;'><strong>CRN : </strong>" + CRN + "<br/>This is breakin case. <br/>Insurer will contact for the inspection with inpection id :" + digInspId[0] + " <br/> Once the inspection is done successfully, you will get payment link.<div>");
                            }
                            else {
                            ...
                            if (response_json.Error.Error_Code == "LM206" && response_json.hasOwnProperty('Payment') && response_json.Payment.Premium_Verification.Is_PG_Allowed == true) {
                                $('#ProgressStatus').append("Status:" + response_json.Error.Error_Specific);
                                $('#err_msgtxt').text("");
                                $('#err_msgtxt').text("Other quotes available");
                                //var revised_data = JSON.parse((response_json.Error.Error_Specific.replace("LM_PREMIUM_MISMATCH_", "")));
                                $('#RevisedPremiumBtn').show();
                                //$('.revised_premium_popup').show();
                                //$('.revised_premium_msg').text("Premium is revised from INR " + revised_data.Quote_Amt + " to INR " + revised_data.Proposal_Amt + ". If you wish to continue Payment, please click on 'Continue to Payment' button.");
                                $("#revised_net_premium").val(response_json.Payment.Premium_Verification.Revision_Premium.net_premium);
                                $("#revised_service_tax").val(response_json.Payment.Premium_Verification.Revision_Premium.service_tax);
                                $("#revised_final_premium").val(response_json.Payment.Premium_Verification.Revision_Premium.final_premium);
                            } else if (response_json.Error.Error_Code == "LM001" || response_json.Error.Error_Code == "LM002") {
                                $('#ProgressStatus').append("Error:" + response_json.Error.Error_Specific);
                            } else {
                                $('#ProgressStatus').append("Error:" + response_json.Error.Error_Msg);
                            }
                            $('#Hidepopup').show();
                        }
                        return false;
                    ...
                    if (response_json.Error === null) {
                        ...
                        */
                        var btnSource = "";
                    if ($("#IsAddToCart").val() == 'Yes') {
                        btnSource = "addToCart";
                    }
                    if (btnSource == 'addToCart') {
                        var obj_temp = {
                            "Summary": summary["Summary"],
                            "Quote_Request": summary["Quote_Request"],
                            "Addon_Request": summary["Addon_Request"],
                            "Insurer_Master": summary["Insurer_Master"],
                            "PB_Master": summary["PB_Master"],
                            "Proposal_Request": summary["Proposal_Request"],
                            "Premium_Response": summary["Premium_Response"],
                            "Posp": summary["Posp"],
                            "Last_Premium_Request": summary["Last_Premium_Request"],
                            "Last_Premium_Response": summary["Last_Premium_Response"],
                            "Last_Proposal_Request": summary["Last_Proposal_Request"],
                            "Last_Proposal_Response": summary["Last_Proposal_Response"],
                            "Proposal_Id": proposal_id,
                            "ss_id": summary['Quote_Request']["ss_id"]
                        };
                        addtocart(obj_temp);
                    }
                    else {
...
function ValidateChequeDetails() {
    var ValidateError = 0;
    if ($('#ChequeNumber').val() == "" || checkNumeric($('#ChequeNumber')) == false) {
        $('#dvChequeNumber').addClass('Error');
        ValidateError++;
    } else {
        $('#dvChequeNumber').removeClass('Error');
    }

    if ($('#BankName').val() == "" || checkTextWithSpace($('#BankName')) == false) {
        $('#dvBankName').addClass('Error');
        ValidateError++;
    } else {
        $('#dvBankName').removeClass('Error');
    }

    if ($('#BankCode').val() == "" || checkAlphaNumeric($('#BankCode')) == false) {
        $('#dvBankCode').addClass('Error');
        ValidateError++;
    } else {
        $('#dvBankCode').removeClass('Error');
    }

    if ($('#BankBranch').val() == "" || checkTextWithSpace($('#BankBranch')) == false) {
        $('#dvBankBranch').addClass('Error');
        ValidateError++;
    } else {
        $('#dvBankBranch').removeClass('Error');
    }

    if ($('#ChequeDate').val() == "") {
        $('#dvChequeDate').addClass('Error');
        ValidateError++;
    } else {
        $('#dvChequeDate').removeClass('Error');
    }

    if ($('#Remark').val() != "") {
        if (checkAlphaNumericAndSpace($('#Remark')) == false) {
            $('#dvRemark').addClass('Error');
            ValidateError++;
        } else {
            $('#dvRemark').removeClass('Error');
        }
    }
    if (ValidateError < 1) {
        ...
        request["final_premium"] = final_premium//$("#SelectedQuote_TotalPremium").val();
        request["tax"] = tax//$("#SelectedQuote_ServiceTax").val();
        request["net_premium"] = SelectedQuote_NetPayablePayablePremium;
        request["is_mobile"] = jQuery.browser.mobile == true ? "yes" : "no";
        request["is_financed"] = $("#IsFinanced").val() == "True" ? "yes" : "no";
        request["method_type"] = "Proposal";
        request["execution_async"] = "yes";
...
function send_payment_link() {
    ...
    /*if (firstname == "" || firstname.length < 1 || this.checkTextWithSpace(firstname) == false) {
    document.getElementById('spn_first_name').classList.add('InputError');
    err++;
    }
    else { document.getElementById('spn_first_name').classList.remove('InputError'); }
    if (lastname == '' || lastname.length < 2 || this.CheckPattern(lastname, 'Text') == false) {
    document.getElementById('spn_last_name').classList.add('InputError'); err++;
    }
    else { document.getElementById('spn_last_name').classList.remove('InputError'); }
    if (sal == '' || sal == '0') {
    document.getElementById('salutation').classList.add('dropdownError');
    err++;
    } else { document.getElementById('salutation').classList.remove('dropdownError'); }
    if (email == '') {
    document.getElementById('spn_email').classList.add('InputError'); err++;
    } else {
    var re = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
    if (!re.test(email)) { document.getElementById('spn_email').classList.add('InputError'); err++; }
    else { document.getElementById('spn_email').classList.remove('InputError'); }
    }
    // if (reg4 == "" || this.checkNumeric(reg4) == false || reg4.length < 4 || reg4 <= '0') {
    //   document.getElementById('RegistrationNumberPart3').classList.add('Error'); err++;
    // }
    // else {
    //   document.getElementById('RegistrationNumberPart3').classList.remove('Error');
    // }
    if (err > 0) {
    this.pay_link_data.validDetails = false;
    } else {
    this.update_user_data(data_valid);
    this.pay_link_data.validDetails = true;
    }
     */
    ...
    if (err == 0) {
        ...
        /*
        this.horizon.callAPIPost(obj, method_name, this.client_id).subscribe(
        data => {
        var send_payment_success_id = data['Id'];

        console.log("send_payment_success_id ", send_payment_success_id);
        if (data.hasOwnProperty('Status')) {
        if (data['Status'] == "Success" || data['Status'] == "SUCCESS") {
        this.loader_success = true;
        this.dataStatusSuccess = true;
        this.PaymentLinkSend = true;
        document.getElementsByClassName('btn-submitpolicy')[0].setAttribute("value", "...");
