...
$(document).ready(function () {
    ...
    $('#lblIndividual').click(function () {
        $('#divmsg4').html('');
    });
    $('#lblGroup').click(function () {
        $('#divmsg4').html('');
    });
    ...
    $("#DateofBirthofTraveller").datepicker({
        ...
        yearRange: 'c-82:c',
        dateFormat: 'dd-mm-yy',
        minDate: new Date((new Date()).getFullYear() - 88, (new Date()).getMonth(), 1),
        maxDate: '-18y',
    ...
    $("#DateofTravelStart").datepicker({
        ...
        onSelect: function (dateText) {           
            ...
            var selDate = new Date(parts[2], parts[1] - 1, parts[0]);
            var newDate = new Date(selDate.getFullYear(), selDate.getMonth() + 6, selDate.getDate() );
            $("#DateofTravelEND").datepicker("option", "minDate", $("#DateofTravelStart").val())
            $('#DateofTravelEND').datepicker('option', 'maxDate', newDate);
            $('#divmsg3').html('');
        }
    });
    ...
    $("#Member2DOB").datepicker({
        ...
        maxDate: '-1y',
        onSelect: function (dateText) {
            $("#Member2DOB").removeClass('errorClass1');
            $("#Member2DOB").removeClass('errorClass1');
            $('#divmsg4').html('');
        }
    });
    $("#Member3DOB").datepicker({
        ...
        maxDate: '-1y',
        onSelect: function (dateText) {
            $("#Member3DOB").removeClass('errorClass1');
            $("#Member3DOB").removeClass('errorClass1');
            $('#divmsg4').html('');
        }
    });
    $("#Member4DOB").datepicker({
        ...
        maxDate: '-1y',
        onSelect: function (dateText) {
            $("#Member4DOB").removeClass('errorClass1');
            $("#Member4DOB").removeClass('errorClass1');
            $('#divmsg4').html('');
        }
    });
    $("#Member5DOB").datepicker({
        ...
        maxDate: '-1y',
        onSelect: function (dateText) {
            $("#Member5DOB").removeClass('errorClass1');
            $("#Member5DOB").removeClass('errorClass1');
            $('#divmsg1').html('');
        }
    });
    $("#Member6DOB").datepicker({
        ...
        maxDate: '-1y',
        onSelect: function (dateText) {
            $("#Member6DOB").removeClass('errorClass1');
            $("#Member6DOB").removeClass('errorClass1');
            $('#divmsg1').html('');
        }
    });   
    ...
    function Validatediv1_Continue() {
        ...
        //if ($("#TravelCountry").val() == "" || $("#TravelCountryID").val() == 0) {
    ...
    function Validatediv3_Next2() {
        ...
        if ($('#MaximumDuration').is(':visible')) {
            if ($("#MaximumDuration").val() == 0) {
                $("#MaximumDuration").addClass('errorClass1');

                Err++;
            } else { $("#MaximumDuration").removeClass('errorClass1'); }
        }
    ...
    function Validatediv4_Next3() {
        ...
        if ($('#divMember1').is(':visible')) {
            ...
            if (($('#ProposerRelationId').val() == "") && ($('#PolicyFor').val() == "Family")) {
                Err++;
            }
        }
        for (var k = 2; k <= 6; k++)
        {
            var divMember = "#divMember" + k;
            var MemberDOB = "#Member" + k + "DOB";
            var MemberGender = "#Member" + k + "Gender";
            var MemberRelation = "#ProposerRelationId" + k;
            if ($(divMember).is(':visible')) {
                if ($(MemberDOB).val() == "") {
                    $(MemberDOB).addClass('errorClass1');
                    Err++;
                }
                else {
                    $(MemberDOB).removeClass('errorClass1');
                }
                if ($(MemberGender).val() == "") {
                    Err++;
                }
                if (($(MemberRelation).val() == "") && ($('#PolicyFor').val() == "Family")) {
                    Err++;
                }
            }
        }        
    ...
    function Validatediv5() {
        ...
        if (_ContactName == "" || !nameValid(_ContactName)) { $("#ContactName").addClass('errorClass1'); Err++; }
        else {
            var arr = _ContactName.split(' ');
            if (arr[1] != null && arr[1] != "") { $("#ContactName").removeClass('errorClass1'); }
            else { $("#ContactName").addClass('errorClass1'); Err++; }
            if (arr[1] == " ") { $("#ContactName").addClass('errorClass1'); Err++; }
        }
    ...
    function slideonerror() {
        ...
        switch (test) {
            case "div1":
                ...
                //}
                if (($('#TripType').val() == "")) {
                    $('#divmsg1').html('Please Select trip type.').slideUp().slideDown();
                    return;
                }
                //if (($('#divPlanningforUSAYes').hasClass('active') == false) && ($('#divPlanningforUSANo').hasClass('active') == false)) {
                //    $('#divmsg1').html('Please select if you will travel to USA/Canada.').slideUp().slideDown();
                //    return;
                //}
                
                //if (($("#TravelCountry").val() == "")) {
                //    $('#divmsg1').html('Please Select the country to travel.').slideUp().slideDown();
                //    return;
                //}
                break;
            //case "div2":
            ...
            case "div3":
                ...
                if ($('#MaximumDuration').is(':visible')) {
                    if ($('#MaximumDuration').val() == 0) {
                        $('#divmsg3').html('Please Select the maximum travel days.').slideUp().slideDown();
                        return;
                    }
                }
                break;
            case "div4":
                $('#divmsg4').html('');
                if (($("#IsIndividual").val() == "")) {
                    $('#divmsg4').html('Please Select policy type.').slideUp().slideDown();
                    return;
                }
                if ($('#divMember1').is(':visible')) {
                    if ($('#DateofBirthofTraveller').val() == "") {
                        $('#divmsg4').html('Please Select Member 1 DOB.').slideUp().slideDown();
                        return;
                    }
                    if ($('#InsuredGender').val() == "") {
                        $('#divmsg4').html('Please Select Member 1 Gender.').slideUp().slideDown();
                        return;
                    }
                    if ($('#ProposerRelationId').val() == "" && ($('#PolicyFor').val() == "Family")) {
                        $('#divmsg4').html('Please Select Member 1 Relation.').slideUp().slideDown();
                        return;
                    }
                }
                for (var k = 2; k <= 6; k++) {
                    ...
                    if ($(divMember).is(':visible')) {
                        if ($(MemberDOB).val() == "") {
                            $('#divmsg4').html('Please Select Member'+k+' DOB.').slideUp().slideDown();
                            return;
                        }
                        if ($(MemberGender).val() == "") {
                            $('#divmsg4').html('Please Select Member'+k+' Gender.').slideUp().slideDown();
                            return;
                        }
                        if (($(MemberRelation).val() == "") && ($('#PolicyFor').val()=="Family")) {
                            $('#divmsg4').html('Please Select Member' + k + ' Relation.').slideUp().slideDown();
                            return;
                        }
                    }
                }                
            ...
            case "div5":
                ...
                $('#divmsg5').html('');
                if (($("#ContactName").val() == "" || !nameValid($("#ContactName").val()))) {
                ...
                if (!mobileValid($("#ContactMobile").val())) {
                    $('#divmsg5').html('Please enter proper contact number.').slideUp().slideDown();
                    return;
                }
                if (($("#ContactEmail").val() == "")) {
                ...
                if (!emailValid($("#ContactEmail").val())) {
                    $('#divmsg5').html('Please enter proper Email Id.').slideUp().slideDown();
                    return;
                }
                if ($('#chkAuthorize').attr('checked')!="checked") {
   ...
   // });
    ...
    $("#right1").click(function () { Validatediv1_Continue(); });
    ...
    $("#right3").click(function () { Validatediv3_Next2(); });  
    $("#right4").click(function () { Validatediv4_Next3(); });
   ...
   // $("#btnGetQuote1").click(function () { Validatediv4(); });
    $("#btnGetQuote1").click(function () {
        ...
        if (Validatediv5()) {         
            ...
            //showProgress();
            {
                if ($("#hidIsCallMissCallAlert").val()) {
                    CheckMissCallTravel($("#ContactMobile").val(), "Fresh");                
                }
                else {                 
                    ShowDialogBoxNew(true);
                }
            }
        }
    });
