...
function PopulateErrorCode()
{
       $.ajax({
        type: 'GET',
        url: "/errors", // json datasource
        //dataType: 'json',
        //data: {rollNumber: $('#txtSearchRoll').val()},
        //cache: false,
        success: function (aData) {

            $('#Col_ErrorCode').get(0).options.length = 0;
            $('#Col_ErrorCode').get(0).options[0] = new Option("Select Error Code", "");

            $.each(aData, function (i, item) {
                console.log(i, item);
                $('#Col_ErrorCode').get(0).options[$('#Col_ErrorCode').get(0).options.length] = new Option(item['Error_Code'] + " - " + item['Error_Desc'], item['Error_Code']);
                // Display      Value
            });
            $('#Col_ErrorCode').selectpicker('refresh');
        },
        error: function () {
...
function Default_Service_Log() {
    ...
    var vehicle_grid_cols = [
        {"data": "Action"},
        {"data": "Service_Log_Id"},
        {"data": "Service_Log_Unique_Id"},
        {"data": "Request_Unique_Id"},
        {"data": "Request_Id"},
        {"data": "Client_Id"},
        {"data": "Status"},
        {"data": "Product_Id"},
    ...
    $('#tr_top_vehicle').append('<th class="bg-teal">#</th><th class="bg-teal">Service Log Id</th><th class="bg-teal">ARN</th><th class="bg-teal">SRN</th><th class="bg-teal">Request Id</th><th class="bg-teal">Client Id</th><th class="bg-teal">Status</th><th class="bg-teal">Product Id</th><th class="bg-teal">Insurer Id</th><th class="bg-teal">Plan Id</th><th class="bg-teal">Plan Name</th><th class="bg-teal">Method Type</th>');
    var i = 0;
   ...
   // }
    var filter = {
        ...
        'Request_Unique_Id': $('#Col_SRN').val(),
        'Service_Log_Unique_Id': $('#Col_ARN').val(),
        'Method_Type': $('#Col_MethodType').val(),
        'Error_Code': $('#Col_ErrorCode').val(),
    };
   ...
   // }
    ...
    vehicle_grid = $('.vehicle_grid').DataTable({
        ...
        "ajax": {
            url: "/service_logs", // json datasource
            ...
            dataFilter: function (data) {
               ...
               // console.log(json);
                ...
                //}
                    for (var k in json.docs) {
                        json.docs[k]['Service_Log_Unique_Id'] = '**' + json.docs[k]['Service_Log_Unique_Id'].toString().substr(36);
                        json.docs[k]['Request_Unique_Id'] = '**' + json.docs[k]['Request_Unique_Id'].toString().substr(36);
                        //json.docs[k][arr_insurer[k1] + '(' + k1 + ')'] = '<span class="bg-' + arr_status_color[status_id] + '">' + arr_status[status_id] + '</span>';
                    var tmpVehicleAction = coreVehicleActionModelHtml;
                    tmpVehicleAction = tmpVehicleAction.replace(/___index___/g, k);
                    json.docs[k]['Action'] = tmpVehicleAction;
