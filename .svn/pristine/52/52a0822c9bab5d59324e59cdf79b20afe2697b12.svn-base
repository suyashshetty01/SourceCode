        ...
        if (this.method.Method_Type === 'Pdf') {
            var args = {
                data: {
                    ...
                    'client_secret': ((config.environment.name !== 'Production') ? '3cd96187-ed12-44e1-af46-bdea2c3a6e72' : '0299dec1-fb95-45a8-8770-ca9537fc252c')
                },
                headers: {
            ...
            var tokenservice_url = ((config.environment.name !== 'Production') ? "https://auth.bagicsit2.bajajallianz.com/auth/realms/Bagic/protocol/openid-connect/token" : 'https://auth.bagicprod.bajajallianz.com/auth/realms/Bagic/protocol/openid-connect/token');
            client.post(tokenservice_url, args, function (data, response) {
                ...
                client.get(specific_insurer_object.method_file_url + policy_num, args, function (data, response) {
                    // parsed response body as js object 
                    console.log("BajajAllianzTravel PDF data - ", data);
                    var objResponseFull = {
                        'err': null,
                        'result': data,
...
BajajAllianzTravel.prototype.proposal_response_handler = function (objResponseJson) {
    ...
    try {
        ...
        if (this.lm_request['question_101_details'] === false) {
    ...
    try {
        ...
        if (Error_Msg === 'NO_ERR') {
            if (objResponseJson.downloadedPdf !== undefined && objResponseJson.downloadedPdf !== null && objResponseJson.downloadedPdf !== "") {
                ...
                var pdf_sys_loc_horizon = appRoot + "/tmp/pdf/" + pdf_file_name;
                var pdf_web_path_portal = config.environment.downloadurl + config.pb_config.pdf_web_loc + pdf_file_name;
                var binary = new Buffer(objResponseJson.downloadedPdf, 'base64');
