...
Motor.prototype.product_api_pre = function () {
	...
	if (['0CH_1TP','0CH_3TP'].indexOf(objLMRequest['vehicle_insurance_subtype']) > -1) {
		objLMRequest['own_damage_disc_rate'] = '0';	
	}
    ...
    Db_Coll_Request.findOne(Search_Criteria, function (err, Db_Idv_Item) {
        if (Db_Idv_Item) {
            ...
            if (objInsurerProduct.lm_request.hasOwnProperty('vehicle_expected_idv') && (objInsurerProduct.lm_request['vehicle_expected_idv'] - 0) > 0) {
                ...
                if (objInsurerProduct['insurer_id'] == 35) {
                    Idv = Db_Idv_Item.Idv_Min;
                } else {
                    let idv_diff_amount = Db_Idv_Item.Idv_Max - Db_Idv_Item.Idv_Min;
                    idv_diff_amount = (idv_diff_amount < 0) ? 0 : idv_diff_amount;
                    let idv_adjustment_amount = ( idv_diff_amount < 100 ) ? idv_diff_amount : 100;
                    
                    Idv = Db_Idv_Item.Idv_Min + idv_adjustment_amount; // Chirag Changed 4 Aug 23
                }				
...
Motor.prototype.premium_list_db_handler_version_2 = function (Db_Data_Object) {
    ...
    try {
        ...
        arr_premium_response['Error_Master'] = {
            ...
            'LM002': 'Un-Identified Error',
            'LM003': 'Vehicle Not Mapped',
            'LM004': 'RTO Not Mapped',
			'LM308' : 'Plan declined for SATP',
            'LM005': 'Previous Insurer Not Supported',
            'LM006': 'Vehicle Idv Not available',
            'LM008': 'Vehicle Not Supported By Insurer',
            'LM009': 'Vehicle Underwriting Restriction',
            'LM010': 'RTO Underwriting Restriction',
            'LM013': 'Vehicle RTO Underwriting Restriction',
		...
		'LM337' : 'Policy issuance is currently unavailable for New Vehicles'
        };
        let obj_idv_error = {};
        for (let k in dbLogItems) {
            if (dbLogItems[k]['Method_Type'] === 'Idv' && dbLogItems[k]['Error_Code'] !== '' ){
				if(arr_premium_response['Error_Master'].hasOwnProperty(dbLogItems[k]['Error_Code']) === true) {
					...
					obj_idv_error['Insurer_' + dbLogItems[k]['Insurer_Id']] = dbLogItems[k]['Error']['Error_Msg'] || 'NA';
				}
            }
            ...
            if (dbLogItems[k]['Status'] === 'complete') {
                if (dbLogItems[k]['Error_Code'] === "" && dbLogItems[k]['Premium_Breakup']) {
                    ...
                    if (typeof All_Response['Insurer_' + dbLogItems[k]['Insurer_Id']] === 'undefined') {
                        ...
                        if ((parseInt(dbLogItems[k]['Product_Id']) !== 12) && (parseInt(dbLogItems[k]['Insurer_Id']) === 10) && (dbLogItems[k]['Method_Type'] === "Idv" || dbLogItems[k]['Method_Type'] === "Premium") && dbLogItems[k]['LM_Custom_Request']['vehicle_expected_idv'] > 0) {
                    ...
                    for (let key in Addon) {
                        if (key.indexOf('final') < 0 && (Addon[key] - 0) > 0) {
                            ...
                            if (Addon_Amt > 0 && dbLogItems[k]['Insurer_Id'] == 13 && key === 'addon_zero_dep_cover' && All_Response.Insurer_13.Premium_Breakup && All_Response.Insurer_13.Premium_Breakup.own_damage && All_Response.Insurer_13.Premium_Breakup.own_damage.od_loading > 0) {
                    ...
                    if(([1,10].includes(dbLogItems[k]['Product_Id'] - 0) && [3].includes(dbLogItems[k]['Insurer_Id'] - 0)) || ((dbLogItems[k]['Product_Id'] - 0 === 10) && (dbLogItems[k]['Insurer_Id'] === 11))){//added for tata tw and car-chola
                    ...
                    if(dbLogItems[k]['Product_Id'] - 0 === 12 && ([7, 3, 9].indexOf(dbLogItems[k]['Insurer_Id']) > -1)){
					...
					let Display_Error_Msg = 'NA';
                    if (arr_premium_response['Error_Master'].hasOwnProperty(dbLogItems[k]['Error_Code']) === true) {
						if(dbLogItems[k]['Insurer_Id'] == 46 && dbLogItems[k]['Error_Code'] == 'LM002'){
							dbLogItems[k]['Error_Code'] = 'LM013';
						}
						Display_Error_Msg = dbLogItems[k]['Error_Code'] + ' :: '+ arr_premium_response['Error_Master'][dbLogItems[k]['Error_Code']];
					}
					else{
						Display_Error_Msg = dbLogItems[k]['Error_Code'] + ' :: '+ (dbLogItems[k]['Error']['Error_Msg'] || 'NA');
...
Motor.prototype.idv_changes_royal_sundaram = function (Db_Data_Object) {
    ...
    try {
        ...
        if (Db_Data_Object.LM_Custom_Request['Plan_Name'] !== "TP") {
