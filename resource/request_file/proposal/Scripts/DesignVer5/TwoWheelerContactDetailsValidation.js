...
function hideRevisedPremium() {
    $("div#divRevisedPremium").fadeOut("fast");
};
    ...
    $('.Heading1').click(function (e) {
        ...
        if (data_sent.hasOwnProperty(this.id)) {
            ...
            if ($(this).attr('id') == 'TabViewInput' || $(this).attr('id') == 'TabPersonalInfo' || $(this).attr('id') == 'TabVehicleAddInfo') {
                ...
                if (($(this).attr('id') == 'PaymentLink' || $(this).attr('id') == 'proceed_ok_btn') && $("#IsConfirmed").val() == "1" && $("#IsPayFromWallet") == "0") {
...
//Abhay 
function addtocart(data, proposal_res) {
    ...
    $.ajax({
        url: data2['url'],
        type: "POST",
        data: JSON.stringify(data2['data']),
        dataType: 'json',
        contentType: "application/json;charset=utf-8",
        success: function (res) {
            ...
            if (res.Msg === "Success") {
                ...
                $("#addToCartFailMsg").html("Failed to add to cart");
            }
        },
        error: function (err) {
            $('.spinner').hide();
...
function RevisedPremium() {
    ...
    var is_misp = false;
    var is_kotakoem = false;
    if (([816, 14154, 14155, 14156, 14157, 14158, 17051, 15912, 15253, 11971, 17053, 25678].indexOf(summary.Quote_Request.ss_id) > -1 && ProductID === 10) || (summary.Quote_Request.utm_campaign && summary.Quote_Request.utm_campaign === "matchpoint")) {
        is_misp = true;
    }
    if ([822].indexOf(summary.Quote_Request.ss_id) > -1 && ProductID === 10) {
        is_kotakoem = true;
    }/*
    if (summary.Quote_Request.utm_campaign) {
        if (summary.Quote_Request.utm_campaign === "matchpoint") {
            is_misp = true;
        }
    }*/
    data_sent[thisval] = false;
    ...
    request["nominee_birth_date"] = change_formate($("#NomineeDOB").val());
    request["vehicle_expected_idv"] = summary.Last_Premium_Request.vehicle_expected_idv;
    request["net_premium"] = $("#revised_net_premium").val();
    request["tax"] = $("#revised_service_tax").val();
    request["final_premium"] = $("#revised_final_premium").val();
    request["is_mobile"] = jQuery.browser.mobile == true ? "yes" : "no";
    request["is_financed"] = $("#IsFinanced").val() == "True" ? "yes" : "no";
    request["method_type"] = "Proposal";
    request["insurer_id"] = summary.Summary.Insurer_Id;
    ...
    data1["CustomerReferenceID"] = $('#CustomerReferenceID').val();
    data1["TwoWheelerVariantId"] = request["vehicle_id"];
    request["is_premium_revised"] = "yes";
    data1["is_premium_revised"] = request["is_premium_revised"];
    request["proposal_url"] = window.location.href;
    console.log('request', request);
    var data = {
        request_json: JSON.stringify(request),
...
function ValidateCTPD(InsName) {
    ...
    if (summary.Quote_Request.vehicle_insurance_subtype == "1OD_0TP") {
        ...
        if (document.getElementById('TPPolicyTenure').value == "" || document.getElementById('TPPolicyTenure').value == "0") { $('#TPPolicyTenure').addClass('Error'); ValidationArray.push('TPPolicyTenure'); ValidateError++; $("#TPEndDate").val(""); }
        else { $('#TPPolicyTenure').removeClass('Error'); }

        if (document.getElementById('TPEndDate').value == "") { $('#dvTPEndDate').addClass('Error'); ValidateError++; }
...
function ValidateVAD(InsName) {
    ...
    if (InsName == "United") {
        if ($ChasisNumber.val() == "" || $ChasisNumber.val().length < 3 || $ChasisNumber.val().length > 22 || pattern.test($ChasisNumber.val()) == false || checkText($('#ChasisNumber')) == true) { $('#dvChasisNumber').addClass('Error'); ValidationArray.push('ChasisNumber'); ValidateError++; }
        else { $('#dvChasisNumber').removeClass('Error'); }
    }
    else if (InsName == "Tata") {
        if ($('#TwoWheelerType').val() === "NEW") {
            if ($ChasisNumber.val() == "" || $ChasisNumber.val().length !== 17 || pattern.test($ChasisNumber.val()) == false || checkText($('#ChasisNumber')) == true) { $('#dvChasisNumber').addClass('Error'); ValidationArray.push('ChasisNumber'); ValidateError++; }
            else { $('#dvChasisNumber').removeClass('Error'); }
        }
    }
    ...
    if (is_valid_pucc && is_valid_pucc == "yes" && summary.Quote_Request.vehicle_insurance_type === "renew") {
        if ($("#PUCC_No_Yes").hasClass('btn-default') && $("#PUCC_valid_yn").val() == "no" && (InsurerID == 2 || InsurerID == 4 || InsurerID == 10 || InsurerID == 19)) {
            $('#dvIsPucc').addClass('Error'); ValidationArray.push('PUCC_valid_yn'); ValidateError++;
        } else {
        ...
        if ($("#PUCC_valid_yn").val() && $("#PUCC_valid_yn").val() === "yes" ) {
            ...
            if ($("#PUCC_date").val() == "") {
                $('#dvPUCC_date').addClass('Error'); ValidationArray.push('PUCC_date'); ValidateError++;
            }
            else {
    ...
    if (summary.Quote_Request.is_helmet_cover == 'yes') {
        if ($("#HPCvr_date").val() == "") { $("#dvHPC_date").addClass('Error'); ValidationArray.push('HPCvr_date'); ValidateError++; }
        else { $("#dvHPC_date").removeClass('Error'); }
    } else { $("#dvHPC_date").removeClass('Error'); $("#HPCvr_date").val(""); }
        ...
        if ((VehicleRegistrationType == "corporate" && InsName == "Bharti") || VehicleRegistrationType == "individual") {
            ...
            if (InsName == "Edelweiss" && $('#NomineeRelationID option:selected').text() == "Others") {
                if ($("#NomineeOtherRelation").val() == "" || checkTextWithSpace($('#NomineeOtherRelation')) == false) {
                    $('#dvNomineeOtherRelation').addClass('Error'); ValidationArray.push('NomineeOtherRelation'); ValidateError++;
                }
                else { $('#dvNomineeOtherRelation').removeClass('Error'); }
            }
        }
...
function set_values(request) {
    ...
    { $('#DORofCPA').val(request["cpa_opt_date"]); 
        if (request["cpa_opt_reason"] == "Stand-alone CPA" || request["cpa_opt_reason"] == "General Personal Accident cover") { $("#divDORofCPA").show(); } else { $("#divDORofCPA").hide(); }
    }
    //pucc
