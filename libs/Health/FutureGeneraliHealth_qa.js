...
FutureGeneraliHealth.prototype.const_insurer_suminsured = [50000, 100000, 150000, 200000, 250000, 300000, 350000, 400000, 450000, 500000, 1000000, 1500000, 2000000, 2500000, 5000000, 10000000];
...
FutureGeneraliHealth.prototype.insurer_product_field_process_pre = function () {
    ...
    if (this.lm_request['method_type'] === 'Premium') {
        if (this.lm_request.hasOwnProperty('is_short_term_policy') && this.lm_request['is_short_term_policy'] === 'yes') {
            this.prepared_request['contract_type'] = this.prepared_request['Plan_Id'] === 404 ? 'CKP' : 'CRP';
            this.processed_request['___contract_type___'] = this.prepared_request['contract_type'];
            this.processed_request['___health_insurance_type_2___'] = this.prepared_request['health_insurance_type'] === "individual" ? 'FHI' : 'FHF';
            this.method['Method_Request_File'] = "FutureGenerali_Health_Corona_Premium.xml";
    ...
    if (this.lm_request['method_type'] === 'Proposal') {
        if (this.prepared_request['dbmaster_pb_plan_id'] === 404 || this.prepared_request['dbmaster_pb_plan_id'] === 405) {
            this.prepared_request['contract_type'] = this.prepared_request['dbmaster_pb_plan_id'] === 404 ? 'CKP' : 'CRP';
            this.processed_request['___contract_type___'] = this.prepared_request['contract_type'];
            this.processed_request['___health_insurance_type_2___'] = this.prepared_request['health_insurance_type'] === "individual" ? 'FHI' : 'FHF';
            this.method['Method_Request_File'] = "FutureGenerali_Health_Corona_Proposal.xml";
            var method_content = fs.readFileSync(appRoot + '/resource/request_file/Health/' + this.method.Method_Request_File).toString();
            this.method_content = method_content;
        }
        ...
        for (member = 1; member <= adult; member++) {
                ...
                if (key.indexOf('_type') > -1 && key.indexOf('_question_') > -1) {
                    ...
                    if (this.lm_request[key].toString().toLowerCase() === 'flag') {
                        if (this.lm_request[ques_detail] === false) {
                            ...
                            if (this.prepared_request['dbmaster_pb_plan_id'] !== 404 && ques_detail.includes('8')) {
            ...
            if (this.prepared_request['dbmaster_pb_plan_id'] === 404 || this.prepared_request['dbmaster_pb_plan_id'] === 405) {
                if (this.processed_request['___pre_exist_disease___'] === 'N' && (this.prepared_request['member_' + member + '_question_6_details']) === 'N' || (this.prepared_request['member_' + member + '_question_7_details']) === 'Y' ||
                        (this.prepared_request['member_' + member + '_question_8_details']) === 'Y' || (this.prepared_request['member_' + member + '_question_9_details']) === 'N' ||
                        (this.prepared_request['member_' + member + '_question_10_details']) === 'Y' || (this.prepared_request['member_' + member + '_question_11_details']) === 'N')
                {
...
//        console.log(docLog.Insurer_Request);
        ...
        if (specific_insurer_object.method.Method_Type !== 'Pdf') {
            var product_code = docLog['Plan_Id'] === 404 ? "CoronaKavach" : docLog['Plan_Id'] === 405 ? "CoronaRakshak" : "HealthTotal";
...
FutureGeneraliHealth.prototype.proposal_response_handler = function (objResponseJson) {
    ...
    var Error_Msg = 'NO_ERR';
    var final_premium = this.prepared_request['final_premium'];
    if (this.prepared_request['dbmaster_pb_plan_id'] !== 404 && this.prepared_request['dbmaster_pb_plan_id'] !== 405) {
        ...
        if (this.prepared_request['health_insurance_type'] === "individual" && this.prepared_request['member_1_marital_status'] === "M") {
            Error_Msg = "Marital status selected as Married - if insured is married then covering Spouse is mandatory";
        }
...
FutureGeneraliHealth.prototype.is_valid_plan = function (lm_request, planCode) {
    ...
    var index = -1;
    var age = true;
    if (lm_request.hasOwnProperty('is_short_term_policy') && lm_request['is_short_term_policy'] === 'yes') {
        age = lm_request['member_1_age'] > 65 ? false : true;
        var Plans = [
            {'code': "Rakshak", 'min_si': 45000, 'max_si': 250000},
            {'code': "Kavach", 'min_si': 45000, 'max_si': 500000}
        ];
    } else {
        age = lm_request['member_1_age'] > 50 ? false : true;
        var Plans = [
...
FutureGeneraliHealth.prototype.get_questionaire = function (mem) {
    var Questions = [
        {"QuestionId": "1", "QuestionText": "Hypertension", "Default": "N", "Med_Loading": 25},
        {"QuestionId": "2", "QuestionText": "Diabetes", "Default": "N", "Med_Loading": 25},
        {"QuestionId": "3", "QuestionText": "Asthma", "Default": "N", "Med_Loading": 30},
        {"QuestionId": "4", "QuestionText": "Cancer", "Default": "N", "Med_Loading": 30},
        {"QuestionId": "5", "QuestionText": "Smoking", "Default": "N", "Med_Loading": 30}
    ];
    if (this.lm_request['quick_quote'] === true) {
        for (var i = 0; i < Questions.length; i++) {
            var ques_detail = 'member_' + mem + '_question_' + Questions[i].QuestionId + '_details';
            this.prepared_request[ques_detail] = Questions[i].Default;
            this.processed_request['___' + ques_detail + '___'] = this.prepared_request[ques_detail];
        }
        this.prepared_request['member_' + mem + '_medical_loading'] = 0;
        this.processed_request['___member_' + mem + '_medical_loading___'] = 0;
    } else {
        var member_ped = this.lm_request['member_' + mem + '_ped'];
        if (member_ped) {
            this.prepared_request['member_' + mem + '_medical_loading'] = 0;
            for (var i = 0; i < Questions.length; i++) {
                var ques_detail = 'member_' + mem + '_question_' + Questions[i].QuestionId + '_details';
                this.prepared_request[ques_detail] = member_ped.includes(Questions[i].QuestionText) ? 'Y' : Questions[i].Default;
                this.processed_request['___' + ques_detail + '___'] = this.prepared_request[ques_detail];
                this.prepared_request['member_' + mem + '_medical_loading'] += member_ped.includes(Questions[i].QuestionText) ? Questions[i].Med_Loading : 0;
                this.processed_request['___member_' + mem + '_medical_loading___'] = this.prepared_request['member_' + mem + '_medical_loading'];
            }
        }
    }
};
