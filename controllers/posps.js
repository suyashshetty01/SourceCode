...
module.exports.controller = function (app) {
    ...
    app.post('/posps', LoadSession, function (req, res) {
        ...
        if (req.body['search[value]'] !== '') {
            ...
            if (req.obj_session.user.role_detail.role.indexOf('SuperAdmin') > -1 || req.obj_session.user.role_detail.role.indexOf('Recruiter') > -1 || [7973, 7748, 131349, 7222, 138796, 5418].indexOf(req.obj_session.user.ss_id) > -1) {
...
*/
    ...
    app.post('/posps/rm_mapping_upload', LoadSession, function (req, res) {
        ...
        try {
            ...
            if (req.obj_session.user.role_detail.role.indexOf('SuperAdmin') > -1 || req.obj_session.user.role_detail.role.indexOf('Recruiter') > -1 || req.obj_session.user.ss_id == 7748) {
                allowed_channel = 'ALL';
...
//saving history mapping start											
                                            ...
                                            if (req.query['email'] == 'yes') {												
                                                if(req.query['email_type'] == 'IIB'){
													...
													if (['ERPCODECREATION', 'SIGNUP'].indexOf(req.query['email_type']) > -1 && config.channel.Const_CH_Contact.hasOwnProperty(obj_posp[j]['Channel']) === true){
														...
														if(obj_posp[j]['Channel'] === "DIY"){
															ccList += ',varun.kaushik@policyboss.com,marketing@policyboss.com';
														}
  ...
  return array.indexOf(value) === index;
	...
	app.get('/posps/report/email_consolidate_process_NIU', function (req, res) {
                    ...
                    try {
                        if (posp_res && posp_res.status && posp_res.status === "SUCCESS" && ["POSP", "FOS"].indexOf(posp_res.user_type) > -1 && posp_res["POSP_USER"]) {
                            ...
                            if (Event_Type === "DOCUMENT_VERIFICATION_REQUEST") { // TO VERIFIER	DONE
                                ...
                                arr_to.push('posp.ops@policyboss.com');
                            ...
                            if (['TESTUSER','FPOS','NPOS'].indexOf(posp_user_posp_source) > -1)
                            {
										...
										Fos_Migration.update({"Ss_Id": ss_id}, {$set: {'Status': 'MIGRATED', 'Modified_On': new Date()}}, function (err, objDbRm_Mapping_Job) {
											try{	
												...
												if (req.query['dbg'] == 'yes') {
													...
													objModelEmail.send('notifications@policyboss.com', 'suresh.k@landmarkinsurance.in,posp.ops@policyboss.com', subject, res_report, '', config.environment.notification_email);
												}
                                                                    ...
                                                                    posp_request.query(posp_qry_update, function (update_qry_err, update_recordset) {
                                                                        try {
                                                                            if (update_qry_err) {
                                                                                ...
                                                                                let Email = require('../models/email');
                                                                                let objModelEmail = new Email();
                                                                                objModelEmail.send('notifications@policyboss.com', 'posp.ops@policyboss.com', subject, rm_notification, '', config.environment.notification_email);
                                                                            }
	...
	app.get('/posps/summary/monthly_transact_erpcode_singlemonth', LoadSession, function (req, res) {
		try{
			let Reporting = req.query['Reporting'] || '';
			let Posp_Type = req.query['Posp_Type'] || '';
			let User_Data = require('../models/user_data');			
			let SubVertical = req.query['SubVertical'] || '';
			
			let curr_date = moment().utcOffset("+05:30").startOf('Day').format('YYYY-MM-DD');
			let posp_erp_code_creation_summary_cache_key = 'posp_monthly_transact_erpcode_summary_uid_'+req.obj_session.user.ss_id;	
			...
			if(SubVertical !== '' || Posp_Type !== "" || Reporting !== ""){
			...
			if(Posp_Type !== ''){
				if(Posp_Type == "6_SERIES"){
					cond_posp_code['Channel'] = { "$ne" : "DIRECT"};					
				}
				if(Posp_Type == "6_SERIES_NON_TEST_USER"){
					cond_posp_code['Channel'] = { "$ne" : "DIRECT"};
					cond_posp_code['Is_Test_User'] = 0;	
				}
				if(Posp_Type == "6_SERIES_TEST_USER"){
					cond_posp_code['Channel'] = { "$ne" : "DIRECT"};
					cond_posp_code['Is_Test_User'] = 1;			
				}
				if(Posp_Type == "9_SERIES"){
					cond_posp_code['Channel'] = "DIRECT";
					cond_posp_code['Is_Test_User'] = 0;	
				}					
			}
			if(Reporting !== ''){
			...
			let agt_posp = [
				...
				let Max_Inception_Month = moment().format("YYYYMM") - 0;
			let agt_Offline_Transaction = [
{ $match:{
    "Inception_Year_Month" : {"$gte":202203,"$lte": Max_Inception_Month},
    }},
{ $group:{
    _id : {
        "Inception_Year_Month":"$Inception_Year_Month",
        },
    Nop : {$sum:1},
    Premium : {$sum:"$FinalPremium"},
    "Posp_List" : {"$addToSet":"$AgentID"},
	//'Posp_List_Ss_Id' : {$addToSet:"$Ss_Id"}
    }},
{ $project:{
    _id : 0,
    "Inception_Year_Month":"$_id.Inception_Year_Month",
	//'Posp_List_Ss_Id' : 1
    'Posp_Count' : {$size: "$Posp_List"},
	"Posp_List" : 1,
    "Nop":1,
    "Premium":1
    }},
{ $sort:{
    Inception_Year_Month : 1
    }},
];	
			let obj_posp_summary = {
					"status" : "pending",
			...
			Offline_Transaction.aggregate(agt_Offline_Transaction).exec(function (err, dbOffline_Transactions) {
				try {
					if (err) {
						obj_posp_summary['status'] = 'dberr';
						obj_posp_summary['err'] = err;
					} else {
						obj_posp_summary['status'] = 'success';
						if (dbOffline_Transactions) {
							for(let k in dbOffline_Transactions){
								dbOffline_Transactions[k]['Posp_List'] = dbOffline_Transactions[k]['Posp_List'].map(Number);
							}
							obj_posp_summary['data'] = dbOffline_Transactions;
                ...
                try {
                    ...
                    dispositionObj.save(function (err) {
                        if (err) {
                            ...
                            if (fields["Disposition_Source"] === 'TPOSP' && fields["dsp_substatus"] === 'Interested') {
                                updated_arg['Onboarding_Status'] = "INTERESTED";
                            }
                    ...
                    return res.json({"Status": "Error", "Msg": e.stack});
                }
            });
        } catch (e) {
            return res.json({"Status": "Error", "Msg": e.stack});
	...
	app.get('/posps/dashboard/recruitment_historic_summary', LoadSession, function (req, res, next) {
		try {
			...
			//cache start
			let curr_date = moment().utcOffset("+05:30").startOf('Day').format('YYYY-MM-DD');
			let cache_key = 'recruitment_historic_summary_ss_id_'+req.obj_session.user.ss_id;
			if (req.obj_session.user.role_detail.role.indexOf('SuperAdmin') > -1){
				cache_key = 'recruitment_historic_summary_ss_id_superadmin';
			}			
			if(SubVertical !== '' || Posp_Type !== "" || Reporting !== ""){				
				cache_key = '';
			}
			let is_cached = false;
			let cached_path = appRoot + "/cache/posp_historic_summary/" + cache_key + ".log";
			req.query['cache_key'] = cache_key;
			req.query['cached_path'] = cached_path;
			if (false && cache_key !== '' && fs.existsSync(cached_path) === true) {
				let stats = fs.statSync(cached_path);
				let mtime = moment(stats.mtime).utcOffset("+05:30").format('YYYY-MM-DD HH:mm:ss');
				let mdate = moment(stats.mtime).utcOffset("+05:30").startOf('Day').format('YYYY-MM-DD');
				if(curr_date === mdate){			
			...
			for (let k in obj_Posp_Summary) {
				...
				if(Channel !== ''){
					...
					if(Channel == "6_SERIES_NON_TEST_USER"){
						cond_posp_qry['Channel'] = { "$ne" : "DIRECT"};
						cond_posp_qry['Is_Test_User'] = 0;	
					}
					if(Channel == "6_SERIES_TEST_USER"){
						cond_posp_qry['Channel'] = { "$ne" : "DIRECT"};
						cond_posp_qry['Is_Test_User'] = 1;			
					}
					if(Channel == "9_SERIES"){
	...
	app.post('/posp/posp_users', LoadSession, function (req, res) {
        try {
            ...
            if (req && req.body && req.body['search'] && req.body['search']['value'] !== '') {
                ...
                filter = {'Is_Active': true};
                if ((req.obj_session.user.role_detail.role.indexOf('SuperAdmin') > -1) || ([7973, 7748, 17026].indexOf(req.obj_session.user.ss_id) > -1)) {//added Ronald and Sandeep ssid  to show all data
...
//                            Posps.find({"Pan_No": objRequest.Pan_No}, function (posp_data_err, posp_data) {
	...
	app.get('/posp/test_user/get_unique_ssid', function (req, res) {
		let ss_id = req.query['ss_id'] || 0;
		ss_id = ss_id - 0;
		let Cond_Model = {'POSP_Sources':'TESTUSER','Ss_Id':{'$ne':0}};
		if(ss_id > 0){
			Cond_Model['Ss_Id'] = ss_id;
		}			
		posp_user.distinct('Ss_Id',Cond_Model).exec(function(err,Arr_Posp_Users){
			Arr_Posp_Users = Arr_Posp_Users || [];
			return res.json(Arr_Posp_Users);
		});
	});
                ...
                client.get('http://horizon.policyboss.com:5000/posps/dsas/view/' + Ss_Id, {}, function (posp_res, posp_res_data) {
                    try {
                        if (posp_res && posp_res.status && posp_res.status === "SUCCESS" && ["POSP", "FOS"].indexOf(posp_res.user_type) > -1 && posp_res["POSP_USER"]) {
                            ...
                            let Appointment_Letter_Link = "https://download.policyboss.com/onboarding_appointments/" + Ss_Id + "/AppointmentLetter_" + Ss_Id + ".pdf";
                            let Training_Certificate_Link = "http://download.policyboss.com/onboarding/download/certificate/" + Ss_Id + "/Certificate_" + Ss_Id + ".pdf";
                            let Summary_Link =  "http://download.policyboss.com/onboarding/download/onboarding_docs/" + Ss_Id + "/Summary_" + Ss_Id + ".pdf";
                            ...
                            if (Event_Type === "APPOINTMENT-LETTER") {
                                Email_Template = 'posp/posp-welcome-email-appointment.html';
                                Link_to_Action = "http://download.policyboss.com/onboarding/download/onboarding_docs/" + Ss_Id + "/Summary_" + Ss_Id + ".pdf";
                                Sub = "Congratulations !! | Welcome to the family of Landmark Insurance Brokers ";
                                Attachment = "/onboarding_docs/" + Ss_Id + "/Summary_" + Ss_Id + ".pdf";
                                arr_to.push(Posp_Email);
                            }
                            let Delayed_Days_Text = "";
                            if (Delayed_Days >= 0) {
                            ...
                            objMail = {
                                ...
                                '___training_certificate_link___': Training_Certificate_Link,
                                '___payment_link_style___': 'block',
                                '___Summary_Link___' : Summary_Link
                            };
                            if (posp_user_posp_source === 'TESTUSER')
                            {
                                objMail['___payment_link_style___'] = "none";
                            }
                            if (Event_Type == "DOCUMENT_REJECTED") {
                                ...
                                if (Is_Testing === 'yes' || moment(posp_user_data['Created_On']).isAfter(new Date('2024-07-01'), 'day') && posp_user_data['KYC_Status'] && ['SUCCESS', 'FAIL'].indexOf(posp_user_data['KYC_Status']) > -1) {
                                    try {
                                        ...
                                        objMail['___kyc_rejected_by___'] = obj_document_status["KYC"] && (obj_document_status["KYC"]["Status"].indexOf("Reject") > -1 ? (obj_document_status["KYC"]["Status"].indexOf("V-Reject") > -1 ? "VERIFIER" : "APPROVER") : "NA");
                                        objMail['___kyc_rejected_remarks___'] = obj_document_status["KYC"] && (obj_document_status["KYC"]["Status"].indexOf("Reject") > -1 ? obj_document_status["KYC"]["Remark"] : "");
                                        objMail['___kyc_row_style___'] = "block";
                                    } catch (e) {
                                        console.error('uncaugth exception while creating kyc doc rejection mail obj');
                                    }
                                }
