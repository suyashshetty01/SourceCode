...
function GetInsurerName(id) {
    switch (id) {
        ...
        case "48": InsurerName = "RQBE"; break;
    }
    ...
    $('.Heading1').click(function (e) {
        ...
        for (var i = 0; i < thislength ; i++) {
            ...
            if (summary.Quote_Request.vehicle_insurance_subtype == "1OD_0TP") {
                if (summary.Quote_Request.channel == "GS" || summary.Quote_Request.channel == "SM" || is_misp) {
...
function get_payment_data(arn, count) {
    ...
    $.get('/TwoWheelerInsurance/Proposal_Details?request_unique_id=' + srn + '&service_log_unique_id=' + arn + '&client_id=' + ClientID + '&customerRefNo=' + crn + '&productId=' + product_id + '', function (res) {
        ...
        if (response_json.Status === 'complete') {
            ...
            //QA
            if (product_id == "1" && (InsurerID == 5 || InsurerID == 9 || InsurerID == 33 || InsurerID == 6) && summary.Quote_Request.is_breakin == "yes" && summary.Quote_Request.is_inspection_done == "no" && (VIST[0] != '0')) {
                //Live
...
function ValidateCD(InsName) {
    ...
    if (VehicleRegistrationType == "individual") {
        ...
        if (InsName != "Bharti") {
            if ($('#ContactLastName').val() == "" || $('#ContactLastName').val().length < 1 || checkText($('#ContactLastName')) == false) {
            ...
            if ($('#ContactLastName').val() == "" || $('#ContactLastName').val().length < 2 || checkText($('#ContactLastName')) == false) {
                $('#dvContactLastName').addClass('Error');
                ValidationArray.push('ContactLastName');
                ValidateError++;
            } else {
...
function ValidateVAD(InsName) {
    ...
    //tata 
    if (InsName == "Tata" && ProductID == 1 && $('#TwoWheelerType').val() == "NEW") {
...
function ValidateTC(InsName) {
    ...
    if (InsName == "Tata" && vehicle_ins_type == "renew" && ProductID == 1) {
    ...
    if (true) {
        if ($("#iagree").is(":checked") == false) {
            if (summary.Quote_Request.channel == 'GS' || summary.Quote_Request.channel == 'SM' || ([816, 14154, 14155, 14156, 14157, 14158, 17051, 15912, 15253, 11971, 17053].indexOf(summary.Quote_Request.ss_id) > -1 && ProductID === 10)) { }
            else {
...
if ($('input[id=SameAsCommunication]').is(':checked') === true) {
    ...
    if (summary.Quote_Request.product_id === 1) {
        ...
        if (summary.hasOwnProperty('Quote_Request') && summary.Quote_Request) {
            if ((summary.Quote_Request.hasOwnProperty('is_oslc') && summary.Quote_Request.is_oslc) || (summary.Quote_Request.hasOwnProperty('is_financed') && summary.Quote_Request.is_financed)) {

                if (summary.Quote_Request.is_oslc === "yes" || summary.Quote_Request.is_financed === "yes") {
                    $("#IsFinanced").val("True");
...
function populate_errordetails(ValidationArray) {
    ...
    for (i = 0; i < ValidationArray.length; ++i) {
        ...
        } else if (InsurerID === 11 && ProductID == 1 && $('#TwoWheelerType').val() == "NEW" && $('#ChasisNumber').val.length !== 17) {
...
function proposalError() {
    ...
    $('.ErrInsurerName').text("Insurer Name : " + summary.PB_Master.Insurer.Insurer_Name);
